...
namespace System.Data.Entity.ModelConfiguration.Mappers
{
  internal sealed class TypeMapper
  {
    ...
    public MappingContext MappingContext => this._mappingContext;
    ...
    public EntityType MapEntityType(Type type)
    {
      ...
      if (entityType == null)
      {
        ...
        if (entityType1 == null)
          this._mappingContext.Model.AddEntitySet(entityType.Name, entityType);
        else if (entityType1 == entityType)
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.SimpleNameCollision((object) type.FullName, (object) type.BaseType().FullName, (object) type.Name));
    ...
    private static T GetExistingEdmType<T>(EdmModel model, Type type) where T : EdmType
    {
      ...
      return structuralOrEnumType == null || !(type != structuralOrEnumType.GetClrType()) ? structuralOrEnumType as T : throw new NotSupportedException(System.Data.Entity.Resources.Strings.SimpleNameCollision((object) type.FullName, (object) structuralOrEnumType.GetClrType().FullName, (object) type.Name));
    }
      ...
      where TStructuralTypeConfiguration : StructuralTypeConfiguration
    {
      ...
      List<PropertyInfo> list = new PropertyFilter(this._mappingContext.ModelBuilderVersion).GetProperties(type, false, this._mappingContext.ModelConfiguration.GetConfiguredProperties(type), this._mappingContext.ModelConfiguration.StructuralTypes).ToList<PropertyInfo>();
    ...
    {
      if (this._mappingContext.ModelConfiguration.GetStructuralTypeConfiguration(type) is EntityTypeConfiguration typeConfiguration)
      {
        ...
        foreach (PropertyInfo instanceProperty in type.BaseType().GetInstanceProperties())
        {
          ...
          if (!this._mappingContext.AttributeProvider.GetAttributes(property).OfType<NotMappedAttribute>().Any<NotMappedAttribute>() && typeConfiguration.IgnoredProperties.Any<PropertyInfo>((Func<PropertyInfo, bool>) (p => p.IsSameAs(property))))
            throw System.Data.Entity.Resources.Error.CannotIgnoreMappedBaseProperty((object) property.Name, (object) type, (object) property.DeclaringType);
        }
      }
      List<EdmMember> list = entityType.DeclaredMembers.ToList<EdmMember>();
      HashSet<PropertyInfo> propertyInfoSet = new HashSet<PropertyInfo>(new PropertyFilter(this._mappingContext.ModelBuilderVersion).GetProperties(type, true, this._mappingContext.ModelConfiguration.GetConfiguredProperties(type), this._mappingContext.ModelConfiguration.StructuralTypes));
      foreach (EdmMember edmMember in list)
      {
        ...
        {
          if (edmMember is NavigationProperty navigationProperty4)
            this._mappingContext.Model.RemoveAssociationType(navigationProperty4.Association);
