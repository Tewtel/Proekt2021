...
namespace System.Data.Entity.ModelConfiguration.Configuration.Mapping
{
  internal static class ForeignKeyPrimitiveOperations
  {
    ...
    private static void UpdatePrincipalTables(
      ...
      bool removeFks)
    {
      foreach (AssociationType associationType in databaseMapping.Model.AssociationTypes.Where<AssociationType>((Func<AssociationType, bool>) (at => at.SourceEnd.GetEntityType().Equals((object) entityType) || at.TargetEnd.GetEntityType().Equals((object) entityType))))
      ...
      EntityType et)
    {
      ...
      if (associationType.TryGuessPrincipalAndDependentEnds(out principalEnd, out AssociationEndMember _))
    ...
    {
      return dependentTable.ForeignKeyBuilders.Any<ForeignKeyBuilder>((Func<ForeignKeyBuilder, bool>) (fk => fk.PrincipalTable == principalTable && fk.DependentColumns.SequenceEqual<EdmProperty>(columns)));
    }
    ...
    private static IEnumerable<ForeignKeyBuilder> FindAllForeignKeyConstraintsForColumn(
      ...
      EdmProperty column)
    {
      return fromTable.ForeignKeyBuilders.Where<ForeignKeyBuilder>((Func<ForeignKeyBuilder, bool>) (fk => fk.DependentColumns.Contains<EdmProperty>(column) && fk.DependentColumns.All<EdmProperty>((Func<EdmProperty, bool>) (c => toTable.Properties.Any<EdmProperty>((Func<EdmProperty, bool>) (nc => string.Equals(nc.Name, c.Name, StringComparison.Ordinal) || string.Equals(nc.GetUnpreferredUniqueName(), c.Name, StringComparison.Ordinal)))))));
    }
      ...
      foreach (EdmProperty keyProperty in fromTable.KeyProperties)
        ((IEnumerable<ForeignKeyBuilder>) ForeignKeyPrimitiveOperations.FindAllForeignKeyConstraintsForColumn(fromTable, toTable, keyProperty).ToArray<ForeignKeyBuilder>()).Each<ForeignKeyBuilder>((Action<ForeignKeyBuilder>) (fk =>
        {
          if (fk.GetIsTypeConstraint())
            return;
          ForeignKeyPrimitiveOperations.CopyForeignKeyConstraint(database, toTable, fk);
        }));
    }
