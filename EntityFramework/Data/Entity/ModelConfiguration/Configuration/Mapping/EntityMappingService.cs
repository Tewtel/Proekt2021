...
namespace System.Data.Entity.ModelConfiguration.Configuration.Mapping
{
  internal class EntityMappingService
  {
    ...
    public EntityMappingService(DbDatabaseMapping databaseMapping) => this._databaseMapping = databaseMapping;
            ...
            if (this.FindPropertyEntityTypeMapping(tableMapping, entitySet, entityType, requiresIsTypeOf, out propertiesTypeMapping, out propertiesTypeMappingFragment))
            {
              ...
              foreach (ForeignKeyBuilder foreignKeyBuilder in tableMapping.Table.ForeignKeyBuilders)
              {
                ...
                if (associationType != null && associationType.IsRequiredToNonRequired())
                {
                  ...
                  foreignKeyBuilder.GetAssociationType().TryGuessPrincipalAndDependentEnds(out AssociationEndMember _, out dependentEnd);
      ...
      IEnumerable<EdmProperty> columns)
    {
      IEnumerable<EntityType> source = this._tableMappings[table].EntityTypes.GetEntityTypes(entitySet).Where<EntityType>((Func<EntityType, bool>) (et =>
      {
        if (et == dependentEndEntityType)
          return false;
        return et.IsAncestorOf(dependentEndEntityType) || !dependentEndEntityType.IsAncestorOf(et);
      }));
      ...
      MappingFragment conditionTypeMappingFragment)
    {
      ...
      foreach (var data in tableMapping.ColumnMappings.SelectMany((Func<ColumnMapping, IEnumerable<PropertyMappingSpecification>>) (cm => (IEnumerable<PropertyMappingSpecification>) cm.PropertyMappings), (cm, pm) => new
      {
        cm = cm,
        pm = pm
      }).Where(_param1 => _param1.pm.EntityType == entityType).Select(_param1 => new
      {
        Column = _param1.cm.Column,
        Property = _param1.pm
      }))
      {
      ...
      EntityType entityType)
    {
      if (!entityType.IsRootOfSet(tableMapping.EntityTypes.GetEntityTypes(entitySet)))
        return false;
      return tableMapping.EntityTypes.GetEntityTypes(entitySet).Count<EntityType>() > 1 && tableMapping.EntityTypes.GetEntityTypes(entitySet).Any<EntityType>((Func<EntityType, bool>) (et => et != entityType && !et.Abstract)) || this._tableMappings.Values.Any<TableMapping>((Func<TableMapping, bool>) (tm => tm != tableMapping && tm.Table.ForeignKeyBuilders.Any<ForeignKeyBuilder>((Func<ForeignKeyBuilder, bool>) (fk => fk.GetIsTypeConstraint() && fk.PrincipalTable == tableMapping.Table))));
    }
      ...
      out MappingFragment fragment)
    {
      ...
      var data = this._databaseMapping.GetEntityTypeMappings(entityType).SelectMany((Func<EntityTypeMapping, IEnumerable<MappingFragment>>) (etm => (IEnumerable<MappingFragment>) etm.MappingFragments), (etm, tmf) => new
      {
        etm = etm,
        tmf = tmf
      }).Where(_param1 => _param1.tmf.Table == tableMapping.Table).Select(_param1 => new
      {
        TypeMapping = _param1.etm,
        Fragment = _param1.tmf
      }).SingleOrDefault();
      if (data == null)
        return false;
    ...
    private static bool HasConditions(TableMapping tableMapping, EntityType entityType) => tableMapping.ColumnMappings.SelectMany<ColumnMapping, PropertyMappingSpecification>((Func<ColumnMapping, IEnumerable<PropertyMappingSpecification>>) (cm => (IEnumerable<PropertyMappingSpecification>) cm.PropertyMappings)).Any<PropertyMappingSpecification>((Func<PropertyMappingSpecification, bool>) (pm => pm.EntityType == entityType && pm.Conditions.Count > 0));
