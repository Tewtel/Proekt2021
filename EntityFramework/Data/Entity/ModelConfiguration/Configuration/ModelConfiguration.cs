...
namespace System.Data.Entity.ModelConfiguration.Configuration
{
  internal class ModelConfiguration : ConfigurationBase
  {
    ...
    internal virtual System.Data.Entity.ModelConfiguration.Configuration.ModelConfiguration Clone() => new System.Data.Entity.ModelConfiguration.Configuration.ModelConfiguration(this);

    public virtual IEnumerable<Type> ConfiguredTypes => this._entityConfigurations.Keys.Union<Type>((IEnumerable<Type>) this._complexTypeConfigurations.Keys).Union<Type>((IEnumerable<Type>) this._ignoredTypes);

    internal virtual IEnumerable<Type> Entities => (IEnumerable<Type>) this._entityConfigurations.Keys.Except<Type>((IEnumerable<Type>) this._ignoredTypes).ToList<Type>();

    internal virtual IEnumerable<Type> ComplexTypes => (IEnumerable<Type>) this._complexTypeConfigurations.Keys.Except<Type>((IEnumerable<Type>) this._ignoredTypes).ToList<Type>();

    internal virtual IEnumerable<Type> StructuralTypes => (IEnumerable<Type>) this._entityConfigurations.Keys.Union<Type>((IEnumerable<Type>) this._complexTypeConfigurations.Keys).Except<Type>((IEnumerable<Type>) this._ignoredTypes).ToList<Type>();
    ...
    internal virtual void Add(EntityTypeConfiguration entityTypeConfiguration)
    {
      ...
      if (this._entityConfigurations.TryGetValue(entityTypeConfiguration.ClrType, out existing) && !existing.IsReplaceable || this._complexTypeConfigurations.ContainsKey(entityTypeConfiguration.ClrType))
        throw System.Data.Entity.Resources.Error.DuplicateStructuralTypeConfiguration((object) entityTypeConfiguration.ClrType);
    ...
    internal virtual void Add(ComplexTypeConfiguration complexTypeConfiguration)
    {
      if (this._entityConfigurations.ContainsKey(complexTypeConfiguration.ClrType) || this._complexTypeConfigurations.ContainsKey(complexTypeConfiguration.ClrType))
        throw System.Data.Entity.Resources.Error.DuplicateStructuralTypeConfiguration((object) complexTypeConfiguration.ClrType);
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<Type>(entityType, nameof (entityType));
      ...
      bool explicitEntity)
    {
      if (this._complexTypeConfigurations.ContainsKey(entityType))
        throw System.Data.Entity.Resources.Error.EntityTypeConfigurationMismatch((object) entityType.Name);
      EntityTypeConfiguration typeConfiguration1;
      if (!this._entityConfigurations.TryGetValue(entityType, out typeConfiguration1))
      {
        Dictionary<Type, EntityTypeConfiguration> entityConfigurations = this._entityConfigurations;
        Type key = entityType;
        EntityTypeConfiguration typeConfiguration2 = new EntityTypeConfiguration(entityType);
        typeConfiguration2.IsExplicitEntity = explicitEntity;
        typeConfiguration1 = typeConfiguration2;
        entityConfigurations.Add(key, typeConfiguration2);
      }
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<Type>(complexType, nameof (complexType));
      if (this._entityConfigurations.ContainsKey(complexType))
        throw System.Data.Entity.Resources.Error.ComplexTypeConfigurationMismatch((object) complexType.Name);
      ...
      Type type)
    {
      ...
      return this._complexTypeConfigurations.TryGetValue(type, out typeConfiguration2) ? (StructuralTypeConfiguration) typeConfiguration2 : (StructuralTypeConfiguration) null;
    }
      ...
      Type type)
    {
      ...
      return typeConfiguration == null ? Enumerable.Empty<PropertyInfo>() : typeConfiguration.ConfiguredProperties;
    }
      ...
      EntityType entityType)
    {
      ...
      for (; entityType.BaseType != null; entityType = (EntityType) entityType.BaseType)
      {
        ...
        if (!entityType.BaseType.Abstract && (!this._entityConfigurations.TryGetValue(clrType, out typeConfiguration) || typeConfiguration.ModificationStoredProceduresConfiguration == null))
          throw System.Data.Entity.Resources.Error.BaseTypeNotMappedToFunctions((object) clrType.Name, (object) entityTypeConfiguration.ClrType.Name);
      }
    ...
    private void ConfigureComplexTypes(EdmModel model)
    {
      ...
      {
        System.Data.Entity.Core.Metadata.Edm.ComplexType complexType = model.GetComplexType(typeConfiguration.ClrType);
    ...
    {
      foreach (StructuralTypeConfiguration typeConfiguration in databaseMapping.Model.ComplexTypes.Select<System.Data.Entity.Core.Metadata.Edm.ComplexType, object>((Func<System.Data.Entity.Core.Metadata.Edm.ComplexType, object>) (ct => ct.GetConfiguration())).Cast<StructuralTypeConfiguration>().Where<StructuralTypeConfiguration>((Func<StructuralTypeConfiguration, bool>) (c => c != null)))
        typeConfiguration.ConfigurePropertyMappings((IList<Tuple<ColumnMappingBuilder, EntityType>>) databaseMapping.GetComplexPropertyMappings(typeConfiguration.ClrType).ToList<Tuple<ColumnMappingBuilder, EntityType>>(), providerManifest);
    ...
    private void DiscoverIndirectlyConfiguredComplexTypes() => this.ActiveEntityConfigurations.SelectMany<EntityTypeConfiguration, Type>((Func<EntityTypeConfiguration, IEnumerable<Type>>) (ec => ec.ConfiguredComplexTypes)).Each<Type, ComplexTypeConfiguration>((Func<Type, ComplexTypeConfiguration>) (t => this.ComplexType(t)));
    ...
    private static void RemoveRedundantColumnConditions(DbDatabaseMapping databaseMapping) => databaseMapping.GetEntitySetMappings().Select(esm => new
    {
      Set = esm,
      Fragments = esm.EntityTypeMappings.SelectMany((Func<EntityTypeMapping, IEnumerable<MappingFragment>>) (etm => (IEnumerable<MappingFragment>) etm.MappingFragments), (etm, etmf) => new
      {
        etm = etm,
        etmf = etmf
      }).GroupBy(_param1 => _param1.etmf.Table, _param1 => _param1.etmf).Where<IGrouping<EntityType, MappingFragment>>((Func<IGrouping<EntityType, MappingFragment>, bool>) (g => g.Count<MappingFragment>((Func<MappingFragment, bool>) (x => x.GetDefaultDiscriminator() != null)) == 1)).Select<IGrouping<EntityType, MappingFragment>, MappingFragment>((Func<IGrouping<EntityType, MappingFragment>, MappingFragment>) (g => g.Single<MappingFragment>((Func<MappingFragment, bool>) (x => x.GetDefaultDiscriminator() != null))))
    }).Each(x => x.Fragments.Each<MappingFragment>((Action<MappingFragment>) (f => f.RemoveDefaultDiscriminator(x.Set))));

    private static void RemoveRedundantTables(DbDatabaseMapping databaseMapping) => databaseMapping.Database.EntityTypes.Where<EntityType>((Func<EntityType, bool>) (t => databaseMapping.GetEntitySetMappings().SelectMany<EntitySetMapping, EntityTypeMapping>((Func<EntitySetMapping, IEnumerable<EntityTypeMapping>>) (esm => (IEnumerable<EntityTypeMapping>) esm.EntityTypeMappings)).SelectMany<EntityTypeMapping, MappingFragment>((Func<EntityTypeMapping, IEnumerable<MappingFragment>>) (etm => (IEnumerable<MappingFragment>) etm.MappingFragments)).All<MappingFragment>((Func<MappingFragment, bool>) (etmf => etmf.Table != t)) && databaseMapping.GetAssociationSetMappings().All<AssociationSetMapping>((Func<AssociationSetMapping, bool>) (asm => asm.Table != t)))).ToList<EntityType>().Each<EntityType>((Action<EntityType>) (t =>
    {
      DatabaseName tableName = t.GetTableName();
      if (tableName != null)
        throw System.Data.Entity.Resources.Error.OrphanedConfiguredTableDetected((object) tableName);
      databaseMapping.Database.RemoveEntityType(t);
      databaseMapping.Database.AssociationTypes.Where<AssociationType>((Func<AssociationType, bool>) (at => at.SourceEnd.GetEntityType() == t || at.TargetEnd.GetEntityType() == t)).ToList<AssociationType>().Each<AssociationType>((Action<AssociationType>) (at => databaseMapping.Database.RemoveAssociationType(at)));
    }));

    private IEnumerable<EntityTypeConfiguration> ActiveEntityConfigurations => (IEnumerable<EntityTypeConfiguration>) this._entityConfigurations.Where<KeyValuePair<Type, EntityTypeConfiguration>>((Func<KeyValuePair<Type, EntityTypeConfiguration>, bool>) (keyValuePair => !this._ignoredTypes.Contains(keyValuePair.Key))).Select<KeyValuePair<Type, EntityTypeConfiguration>, EntityTypeConfiguration>((Func<KeyValuePair<Type, EntityTypeConfiguration>, EntityTypeConfiguration>) (keyValuePair => keyValuePair.Value)).ToList<EntityTypeConfiguration>();

    private IEnumerable<ComplexTypeConfiguration> ActiveComplexTypeConfigurations => (IEnumerable<ComplexTypeConfiguration>) this._complexTypeConfigurations.Where<KeyValuePair<Type, ComplexTypeConfiguration>>((Func<KeyValuePair<Type, ComplexTypeConfiguration>, bool>) (keyValuePair => !this._ignoredTypes.Contains(keyValuePair.Key))).Select<KeyValuePair<Type, ComplexTypeConfiguration>, ComplexTypeConfiguration>((Func<KeyValuePair<Type, ComplexTypeConfiguration>, ComplexTypeConfiguration>) (keyValuePair => keyValuePair.Value)).ToList<ComplexTypeConfiguration>();
  }
}
