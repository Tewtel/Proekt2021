...
namespace System.Data.Entity.ModelConfiguration.Edm.Serialization
{
  internal sealed class EdmxSerializer
  {
    ...
    private void WriteEdmxRuntime()
    {
      using (this.Element("Runtime"))
      {
        ...
        using (this.Element("StorageModels"))
          new SsdlSerializer().Serialize(this._databaseMapping.Database, this._databaseMapping.ProviderInfo.ProviderInvariantName, this._databaseMapping.ProviderInfo.ProviderManifestToken, this._xmlWriter);
      }
    }
    ...
    private class EndElement : IDisposable
    {
      ...
      public EndElement(XmlWriter xmlWriter) => this._xmlWriter = xmlWriter;

      public void Dispose() => this._xmlWriter.WriteEndElement();
    }
  }
}
