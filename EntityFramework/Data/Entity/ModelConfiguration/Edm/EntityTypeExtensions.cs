...
namespace System.Data.Entity.ModelConfiguration.Edm
{
  internal static class EntityTypeExtensions
  {
    ...
    public static void SetConfiguration(this EntityType table, object configuration) => table.GetMetadataProperties().SetConfiguration(configuration);

    public static DatabaseName GetTableName(this EntityType table) => (DatabaseName) table.Annotations.GetAnnotation("TableName");

    public static void SetTableName(this EntityType table, DatabaseName tableName) => table.GetMetadataProperties().SetAnnotation("TableName", (object) tableName);

    internal static IEnumerable<EntityType> ToHierarchy(this EntityType edmType) => EdmType.SafeTraverseHierarchy<EntityType>(edmType);
    ...
    public static List<EdmProperty> GetKeyProperties(this EntityType entityType)
    {
      ...
      EntityType visitingType = entityType;
      List<EdmProperty> keyProperties = edmPropertyList;
      EntityTypeExtensions.GetKeyProperties(visitedTypes, visitingType, keyProperties);
    ...
    public static object GetConfiguration(this EntityType entityType) => entityType.Annotations.GetConfiguration();

    public static Type GetClrType(this EntityType entityType) => entityType.Annotations.GetClrType();
      ...
      EdmModel model)
    {
      ...
      {
        foreach (EntityType entityType1 in derivedTypes)
        {
    ...
    public static bool IsRootOfSet(this EntityType entityType, IEnumerable<EntityType> set) => set.All<EntityType>((Func<EntityType, bool>) (et => et == entityType || entityType.IsAncestorOf(et) || et.GetRootType() != entityType.GetRootType()));
  }
}
