...
namespace System.Data.Entity.Migrations.Infrastructure
{
  internal class ModificationCommandTreeGenerator
  {
    ...
    private DbContext CreateContext() => this._connection != null ? (DbContext) new ModificationCommandTreeGenerator.TempDbContext(this._connection, this._compiledModel) : (DbContext) new ModificationCommandTreeGenerator.TempDbContext(this._compiledModel);
      ...
      where TCommandTree : DbCommandTree
    {
      ...
      using (DbContext context = this.CreateContext())
      {
        ...
        object sourceEntity = this.InstantiateAndAttachEntity(entityType1, context);
        EntityType entityType2 = associationType.TargetEnd.GetEntityType();
        object targetEntity = entityType1.GetRootType() == entityType2.GetRootType() ? sourceEntity : this.InstantiateAndAttachEntity(entityType2, context);
        ((IObjectContextAdapter) context).ObjectContext.ObjectStateManager.ChangeRelationshipState(sourceEntity, targetEntity, associationType.FullName, associationType.TargetEnd.Name, state == EntityState.Deleted ? state : EntityState.Added);
        using (CommandTracer commandTracer = new CommandTracer(context))
        {
          context.SaveChanges();
          foreach (TCommandTree commandTree in commandTracer.CommandTrees)
            yield return commandTree;
        }
      }
    }
      ...
      DbSet set)
    {
      ...
      {
        EntityType entityType1 = this._metadataWorkspace.GetItems<EntityType>(DataSpace.CSpace).First<EntityType>((Func<EntityType, bool>) (et => entityType.IsAncestorOf(et) && !et.Abstract));
      ...
      EntityState state)
    {
      ...
      using (DbContext context = this.CreateContext())
      {
        ...
        using (CommandTracer commandTracer = new CommandTracer(context))
        {
          ...
          foreach (DbModificationCommandTree commandTree in commandTracer.CommandTrees)
            yield return commandTree;
        }
      }
    }
      ...
      EntityState state)
    {
      ...
      foreach (AssociationType associationType in this._metadataWorkspace.GetItems<AssociationType>(DataSpace.CSpace).Where<AssociationType>((Func<AssociationType, bool>) (at =>
      {
        if (at.IsForeignKey || at.IsManyToMany())
          return false;
        return at.SourceEnd.GetEntityType().IsAssignableFrom((EdmType) entityType) || at.TargetEnd.GetEntityType().IsAssignableFrom((EdmType) entityType);
      })))
      {
        ...
        if (dependentEnd.GetEntityType().IsAssignableFrom((EdmType) entityType))
        {
          ...
          if (obj1 == null || entity == obj1 && state == EntityState.Added)
          {
    ...
    private void HandleTableSplitting(
      ...
      EntityState state)
    {
      foreach (AssociationType associationType in this._metadataWorkspace.GetItems<AssociationType>(DataSpace.CSpace).Where<AssociationType>((Func<AssociationType, bool>) (at => at.IsForeignKey && at.IsRequiredToRequired() && !at.IsSelfReferencing() && (at.SourceEnd.GetEntityType().IsAssignableFrom((EdmType) entityType) || at.TargetEnd.GetEntityType().IsAssignableFrom((EdmType) entityType)) && this._metadataWorkspace.GetItems<AssociationType>(DataSpace.SSpace).All<AssociationType>((Func<AssociationType, bool>) (fk => fk.Name != at.Name)))))
      {
        AssociationEndMember principalEnd;
        AssociationEndMember dependentEnd;
        if (!associationType.TryGuessPrincipalAndDependentEnds(out principalEnd, out dependentEnd))
