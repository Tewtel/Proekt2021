...
namespace System.Data.Entity.Migrations.Infrastructure
{
  ...
  public abstract class MigratorBase
  {
    ...
    public virtual IEnumerable<string> GetPendingMigrations() => this._this.GetPendingMigrations();

    /// <summary>
    /// Gets the configuration being used for the migrations process.
    /// </summary>
    public virtual DbMigrationsConfiguration Configuration => this._this.Configuration;

    /// <summary>Updates the target database to the latest migration.</summary>
    public void Update() => this.Update((string) null);
    ...
    public virtual void Update(string targetMigration) => this._this.Update(targetMigration);

    internal virtual string GetMigrationId(string migration) => this._this.GetMigrationId(migration);

    /// <summary>
    /// Gets a list of the migrations that are defined in the assembly.
    /// </summary>
    /// <returns> List of migration Ids </returns>
    public virtual IEnumerable<string> GetLocalMigrations() => this._this.GetLocalMigrations();

    /// <summary>
    /// Gets a list of the migrations that have been applied to the database.
    /// </summary>
    /// <returns> List of migration Ids </returns>
    public virtual IEnumerable<string> GetDatabaseMigrations() => this._this.GetDatabaseMigrations();
    ...
    internal virtual void ApplyMigration(DbMigration migration, DbMigration lastMigration) => this._this.ApplyMigration(migration, lastMigration);

    internal virtual void EnsureDatabaseExists(Action mustSucceedToKeepDatabase) => this._this.EnsureDatabaseExists(mustSucceedToKeepDatabase);
    ...
    internal virtual void SeedDatabase() => this._this.SeedDatabase();
    ...
    internal virtual IEnumerable<DbQueryCommandTree> CreateDiscoveryQueryTrees() => this._this.CreateDiscoveryQueryTrees();
    ...
    internal virtual void Downgrade(IEnumerable<string> pendingMigrations) => this._this.Downgrade(pendingMigrations);

    internal virtual void UpgradeHistory(IEnumerable<MigrationOperation> upgradeOperations) => this._this.UpgradeHistory(upgradeOperations);

    internal virtual string TargetDatabase => this._this.TargetDatabase;

    internal virtual bool HistoryExists() => this._this.HistoryExists();
  }
}
