...
namespace System.Data.Entity.Internal
{
  internal class DbSetDiscoveryService
  {
    ...
    public DbSetDiscoveryService(DbContext context) => this._context = context;
    ...
    private Dictionary<Type, List<string>> GetSets()
    {
      ...
      if (!DbSetDiscoveryService._objectSetInitializers.TryGetValue(this._context.GetType(), out initializersPair))
      {
        ...
        foreach (PropertyInfo propertyInfo in this._context.GetType().GetInstanceProperties().Where<PropertyInfo>((Func<PropertyInfo, bool>) (p => p.GetIndexParameters().Length == 0 && p.DeclaringType != typeof (DbContext))))
        {
          ...
          if (setType != (Type) null)
          {
            if (!setType.IsValidStructuralType())
              throw System.Data.Entity.Resources.Error.InvalidEntityType((object) setType);
    ...
    public void RegisterSets(DbModelBuilder modelBuilder)
    {
      ...
      foreach (KeyValuePair<Type, List<string>> keyValuePair in source)
      {
        if (keyValuePair.Value.Count > 1)
          throw System.Data.Entity.Resources.Error.Mapping_MESTNotSupported((object) keyValuePair.Value[0], (object) keyValuePair.Value[1], (object) keyValuePair.Key);
        modelBuilder.Entity(keyValuePair.Key).EntitySetName = keyValuePair.Value[0];
      }
    }

    private static bool DbSetPropertyShouldBeInitialized(PropertyInfo propertyInfo) => !propertyInfo.GetCustomAttributes<SuppressDbSetInitializationAttribute>(false).Any<SuppressDbSetInitializationAttribute>() && !propertyInfo.DeclaringType.GetCustomAttributes<SuppressDbSetInitializationAttribute>(false).Any<SuppressDbSetInitializationAttribute>();
