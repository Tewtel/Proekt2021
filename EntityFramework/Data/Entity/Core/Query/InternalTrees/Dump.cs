...
namespace System.Data.Entity.Core.Query.InternalTrees
{
  internal class Dump : BasicOpVisitor, IDisposable
  {
    ...
    internal static string ToXml(Command itree) => Dump.ToXml(itree.Root);
    ...
    internal void BeginExpression() => this.WriteString("(");

    internal void EndExpression() => this.WriteString(")");

    internal void End() => this._writer.WriteEndElement();

    internal void WriteString(string value) => this._writer.WriteString(value);
    ...
    protected override void VisitNestOp(NestBaseOp op, Node n)
    {
      ...
      if (op is SingleStreamNestOp singleStreamNestOp)
    ...
    public override void Visit(NewEntityOp op, Node n) => this.VisitNewOp((Op) op, n);

    public override void Visit(NewInstanceOp op, Node n) => this.VisitNewOp((Op) op, n);

    public override void Visit(DiscriminatedNewEntityOp op, Node n) => this.VisitNewOp((Op) op, n);

    public override void Visit(NewMultisetOp op, Node n) => this.VisitNewOp((Op) op, n);

    public override void Visit(NewRecordOp op, Node n) => this.VisitNewOp((Op) op, n);
    ...
    internal class ColumnMapDumper : ColumnMapVisitor<Dump>
    {
      ...
      {
        Dump.AutoXml autoXml;
        if (columnMap.ForeignKeys.Length != 0)
        {
          autoXml = new Dump.AutoXml(dumper, "foreignKeys");
          try
          {
        ...
        {
          autoXml = new Dump.AutoXml(dumper, "keys");
          try
          {
        ...
        autoXml = new Dump.AutoXml(dumper, "element");
        try
        {
      ...
      private static Dictionary<string, object> GetAttributes(ColumnMap columnMap) => new Dictionary<string, object>()
      {
        {
          "Type",
    ...
    internal struct AutoXml : IDisposable
    {
      ...
      internal AutoXml(Dump dumper, string nodeName)
        : this(dumper, nodeName, (Dictionary<string, object>) null)
      {
      }
      ...
      public void Dispose() => this._dumper.End();
    }
  }
}
