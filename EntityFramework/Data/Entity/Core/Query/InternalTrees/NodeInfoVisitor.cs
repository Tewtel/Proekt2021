...
namespace System.Data.Entity.Core.Query.InternalTrees
{
  internal class NodeInfoVisitor : BasicOpVisitorOfT<NodeInfo>
  {
    ...
    internal NodeInfoVisitor(Command command) => this.m_command = command;

    private NodeInfo GetNodeInfo(Node n) => n.GetNodeInfo(this.m_command);

    private ExtendedNodeInfo GetExtendedNodeInfo(Node n) => n.GetExtendedNodeInfo(this.m_command);
    ...
    private static bool IsDefinitionNonNullable(Node definition, VarVec nonNullableInputs)
    {
      if (definition.Op.OpType == OpType.Constant || definition.Op.OpType == OpType.InternalConstant || definition.Op.OpType == OpType.NullSentinel)
        return true;
      return definition.Op.OpType == OpType.VarRef && nonNullableInputs.IsSet(((VarRefOp) definition.Op).Var);
    }
    ...
    protected override NodeInfo VisitRelOpDefault(RelOp op, Node n) => this.Unimplemented(n);
    ...
    internal static Dictionary<Var, Var> ComputeVarRemappings(Node varDefListNode)
    {
      ...
      {
        if (child.Child0.Op is VarRefOp op2)
        {
          VarDefOp op = child.Op as VarDefOp;
          dictionary[op2.Var] = op.Var;
        }
      }
      return dictionary;
    }
    ...
    public override NodeInfo Visit(ProjectOp op, Node n)
    {
      ...
      {
        VarVec varVec1 = this.m_command.CreateVarVec(extendedNodeInfo2.Keys.KeyVars).Remap((IDictionary<Var, Var>) NodeInfoVisitor.ComputeVarRemappings(n.Child1));
        VarVec varVec2 = varVec1.Clone();
        varVec1.Minus(this.m_command.CreateVarVec(op.Outputs));
        if (varVec1.IsEmpty)
    ...
    public override NodeInfo Visit(FilterOp op, Node n)
    {
      ...
      extendedNodeInfo1.MaxRows = !(n.Child1.Op is ConstantPredicateOp op1) || !op1.IsFalse ? extendedNodeInfo2.MaxRows : RowCount.Zero;
      return (NodeInfo) extendedNodeInfo1;
    }
    ...
    protected override NodeInfo VisitSetOp(SetOp op, Node n)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          for (int index = 0; index < n.Children.Count; ++index)
          {
            ...
            {
              VarVec other = extendedNodeInfo4.Keys.KeyVars.Remap((IDictionary<Var, Var>) unionAllOp.VarMap[index].GetReverseMap());
              varVec.Or(other);
            }
            else
            {
      ...
      VarVec other1 = extendedNodeInfo2.NonNullableDefinitions.Remap((IDictionary<Var, Var>) op.VarMap[0].GetReverseMap());
      extendedNodeInfo1.NonNullableDefinitions.InitFrom(other1);
      if (op.OpType != OpType.Except)
      {
        VarVec other2 = extendedNodeInfo3.NonNullableDefinitions.Remap((IDictionary<Var, Var>) op.VarMap[1].GetReverseMap());
