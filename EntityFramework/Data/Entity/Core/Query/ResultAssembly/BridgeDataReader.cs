...
namespace System.Data.Entity.Core.Query.ResultAssembly
{
  internal class BridgeDataReader : DbDataReader, IExtendedDataRecord, IDataRecord
  {
    ...
    protected virtual Task EnsureInitializedAsync(CancellationToken cancellationToken)
    {
      ...
      return Interlocked.CompareExchange(ref this._initialized, 1, 0) != 0 ? (Task) Task.FromResult<object>((object) null) : this._initializeAsync(cancellationToken);
    }
      ...
      CancellationToken cancellationToken)
    {
      ...
      if (!this._shaper.DataWaiting)
      {
        Shaper<RecordState> shaper1 = this._shaper;
        shaper1.DataWaiting = await this._shaper.RootEnumerator.MoveNextAsync(cancellationToken).WithCurrentCulture<bool>();
        shaper1 = (Shaper<RecordState>) null;
      }
      this.InitializeHasRows();
    }
    ...
    public override IEnumerator GetEnumerator() => (IEnumerator) new DbEnumerator((IDataReader) this, true);

    public override DataTable GetSchemaTable() => throw new NotSupportedException(Strings.ADP_GetSchemaTableIsNotSupported);
    ...
    public override async Task<bool> ReadAsync(CancellationToken cancellationToken)
    {
      ...
      await this._dataRecord.CloseImplicitlyAsync(cancellationToken).WithCurrentCulture();
      bool hasData = await this.ReadInternalAsync(cancellationToken).WithCurrentCulture<bool>();
      this._dataRecord.SetRecordSource(this._shaper.RootEnumerator.Current, hasData);
      return hasData;
    }
    ...
    private async Task<bool> ReadInternalAsync(CancellationToken cancellationToken)
    {
      ...
      {
        shaper = this._shaper;
        shaper.DataWaiting = await this._shaper.RootEnumerator.MoveNextAsync(cancellationToken).WithCurrentCulture<bool>();
        shaper = (Shaper<RecordState>) null;
      }
      while (this._shaper.DataWaiting && this._shaper.RootEnumerator.Current.CoordinatorFactory != this._coordinatorFactory && this._shaper.RootEnumerator.Current.CoordinatorFactory.Depth > this._coordinatorFactory.Depth)
      {
        shaper = this._shaper;
