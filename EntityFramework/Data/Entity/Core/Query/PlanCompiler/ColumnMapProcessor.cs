...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
  internal class ColumnMapProcessor
  {
    ...
    internal ColumnMap ExpandColumnMap()
    {
      ...
      return this.m_varInfo.Kind == VarInfoKind.PrimitiveTypeVarInfo ? (ColumnMap) new VarRefColumnMap(this.m_columnMap.Var.Type, this.m_columnMap.Name, ((PrimitiveTypeVarInfo) this.m_varInfo).NewVar) : this.CreateColumnMap(this.m_columnMap.Var.Type, this.m_columnMap.Name);
    }
    ...
    private ColumnMap CreateColumnMap(TypeUsage type, string name) => !TypeUtils.IsStructuredType(type) ? (ColumnMap) this.CreateSimpleColumnMap(type, name) : this.CreateStructuralColumnMap(type, name);
      ...
      bool handleRelProperties)
    {
      ...
      else
      {
        ...
        foreach (EdmMember structuralMember in TypeHelpers.GetDeclaredStructuralMembers(typeInfo.Type))
        {
          ...
          {
            EdmProperty key = structuralMember as EdmProperty;
            System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert(key != null, "EntityType key member is not property?");
            dictionary[key] = columnMap;
          }
        }
        foreach (EdmMember keyMember in TypeHelpers.GetEdmType<EntityType>(typeInfo.Type).KeyMembers)
        {
          EdmProperty key = keyMember as EdmProperty;
          System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert(key != null, "EntityType key member is not property?");
          SimpleColumnMap simpleColumnMap = dictionary[key] as SimpleColumnMap;
    ...
    private SimpleColumnMap CreateEntitySetIdColumnMap(EdmProperty prop) => this.CreateSimpleColumnMap(Helper.GetModelTypeUsage((EdmMember) prop), "__EntitySetId");
      ...
      string name)
    {
      ...
      System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert(typedColumnMap1 != null, "Didn't find requested type in polymorphic type hierarchy?");
    ...
    private SimpleColumnMap CreateTypeIdColumnMap(EdmProperty prop) => this.CreateSimpleColumnMap(Helper.GetModelTypeUsage((EdmMember) prop), "__TypeId");
      ...
      SimpleColumnMap[] keyColumnMaps)
    {
      ...
      System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert(entitySet != null, "Expected non-null entitySet when no entity set ID is required. Entity type = " + entityType?.ToString());
