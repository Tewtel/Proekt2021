...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
  internal class KeyPullup : BasicOpVisitor
  {
    ...
    internal KeyPullup(Command command) => this.m_command = command;
    ...
    public override void Visit(ProjectOp op, System.Data.Entity.Core.Query.InternalTrees.Node n)
    {
      ...
      if (!extendedNodeInfo.Keys.NoKeys)
      {
        ...
        VarVec other = extendedNodeInfo.Keys.KeyVars.Remap((IDictionary<Var, Var>) varRemappings);
        varVec.Or(other);
    ...
    public override void Visit(UnionAllOp op, System.Data.Entity.Core.Query.InternalTrees.Node n)
    {
      ...
      for (int index = 0; index < n.Children.Count; ++index)
      {
        ...
        VarVec v = this.m_command.GetExtendedNodeInfo(child).Keys.KeyVars.Remap((IDictionary<Var, Var>) op.VarMap[index]);
      ...
      for (int index1 = 0; index1 < n.Children.Count; ++index1)
      {
        ...
        if (OpType.UnionAll == child.Op.OpType && ((UnionAllOp) child.Op).BranchDiscriminator != null)
        {
          ...
          {
            System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert(index1 == 0, "right branch has a discriminator var that the left branch doesn't have?");
        ...
        {
          System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert((uint) args.Count > 0U, "no new nodes?");
          foreach (Var v in (IEnumerable<Var>) op.VarMap[index1].Values)
            varVec.Set(v);
