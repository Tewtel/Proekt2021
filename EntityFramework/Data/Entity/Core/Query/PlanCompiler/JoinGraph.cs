...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
  internal class JoinGraph
  {
    ...
    private static void GetColumnVars(List<ColumnVar> columnVars, IEnumerable<Var> vec)
    {
      ...
      {
        System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert(var.VarType == VarType.Column, "Expected a columnVar. Found " + var.VarType.ToString());
      ...
      out System.Data.Entity.Core.Query.InternalTrees.Node otherPredicateNode)
    {
      ...
      if (joinNode.Op.OpType == OpType.FullOuterJoin)
        return;
      Predicate predicate1 = new Predicate(this.m_command, joinNode.Child2);
      ...
      VarVec leftTableDefinitions = columnVars1;
      VarVec rightTableDefinitions = columnVars2;
      List<Var> varList1;
      ref List<Var> local1 = ref varList1;
      List<Var> varList2;
      ref List<Var> local2 = ref varList2;
      Predicate predicate2;
      ref Predicate local3 = ref predicate2;
      predicate1.GetEquiJoinPredicates(leftTableDefinitions, rightTableDefinitions, out local1, out local2, out local3);
      otherPredicateNode = predicate2.BuildAndTree();
      JoinGraph.GetColumnVars(leftVars, (IEnumerable<Var>) varList1);
      JoinGraph.GetColumnVars(rightVars, (IEnumerable<Var>) varList2);
    }
    ...
    private static bool GenerateTransitiveEdge(JoinEdge edge1, JoinEdge edge2)
    {
      ...
      while (flag)
      {
        KeyValuePair<ColumnVar, ColumnVar> current = enumerator1.Current;
        ColumnVar key1 = current.Key;
        current = enumerator2.Current;
        ColumnVar key2 = current.Key;
        if (key1 == key2)
        {
          List<ColumnVar> columnVarList3 = columnVarList1;
          current = enumerator1.Current;
          ColumnVar columnVar1 = current.Value;
          columnVarList3.Add(columnVar1);
          List<ColumnVar> columnVarList4 = columnVarList2;
          current = enumerator2.Current;
          ColumnVar columnVar2 = current.Value;
          columnVarList4.Add(columnVar2);
        ...
        else
        {
          if (edge1.JoinKind == JoinKind.LeftOuter)
            return false;
          current = enumerator1.Current;
          int id1 = current.Key.Id;
          current = enumerator2.Current;
          int id2 = current.Key.Id;
          flag = id1 <= id2 ? enumerator1.MoveNext() : enumerator2.MoveNext();
        }
      }
    ...
    {
      foreach (AugmentedJoinNode joinNode in this.m_vertexes.OfType<AugmentedJoinNode>().Where<AugmentedJoinNode>((Func<AugmentedJoinNode, bool>) (j => j.Node.Op.OpType == OpType.LeftOuterJoin && j.JoinEdges.Count > 0)))
      {
    ...
    private bool CanJoinEdgeBeTurnedIntoInnerJoin(AugmentedNode rightNode, JoinEdge joinEdge) => !joinEdge.RestrictedElimination && JoinGraph.AreAllTableRowsPreserved(rightNode, joinEdge.Right) && this.IsConstraintPresentForTurningIntoInnerJoin(joinEdge);
    ...
    private void GenerateTransitiveEdges()
    {
      ...
      {
        if (vertex is AugmentedTableNode augmentedTableNode1)
        {
          for (int index1 = 0; index1 < augmentedTableNode1.JoinEdges.Count; ++index1)
          {
            JoinEdge joinEdge1 = augmentedTableNode1.JoinEdges[index1];
            int index2 = 0;
    ...
    {
      return replacingTable.Id < table.NewLocationId ? JoinGraph.CanBeMovedBasedOnLojParticipation(table, replacingTable) : JoinGraph.CanBeMovedBasedOnLojParticipation(replacingTable, table);
    }
    ...
    private static bool MatchOtherPredicates(System.Data.Entity.Core.Query.InternalTrees.Node x, System.Data.Entity.Core.Query.InternalTrees.Node y)
    {
      ...
      return x.Op is VarRefOp op1 && y.Op is VarRefOp op2 && (op1.Var is ColumnVar var1 && op2.Var is ColumnVar var2) && var1.ColumnMetadata.Name.Equals(var2.ColumnMetadata.Name);
    }
    ...
    private bool QualifiesForStarSelfJoinGroup(JoinEdge joinEdge)
    {
      ...
      if (!varVec.IsEmpty)
        return false;
      return joinEdge.JoinNode == null || joinEdge.JoinNode.OtherPredicate == null || JoinGraph.QualifiesForStarSelfJoinGroup(joinEdge.JoinNode.OtherPredicate, this.m_command.GetExtendedNodeInfo(joinEdge.Right.Node).Definitions);
    }
    ...
    {
      if (!(otherPredicateNode.Op is VarRefOp op) || !(op.Var is ColumnVar var))
        return true;
      return rightTableColumnVars.IsSet((Var) var) && otherPredicateNode.Children.All<System.Data.Entity.Core.Query.InternalTrees.Node>((Func<System.Data.Entity.Core.Query.InternalTrees.Node, bool>) (node => JoinGraph.QualifiesForStarSelfJoinGroup(node, rightTableColumnVars)));
    }
    ...
    private void EliminateSelfJoins()
    {
      foreach (AugmentedNode vertex in this.m_vertexes)
      {
        if (vertex is AugmentedTableNode tableNode1)
        {
          this.EliminateSelfJoins(tableNode1);
          this.EliminateStarSelfJoins(tableNode1);
        }
      }
    }
    ...
    private void EliminateRightTable(JoinEdge joinEdge)
    {
      ...
      System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert((joinEdge.Left.Id < joinEdge.Right.Id ? 1 : 0) != 0, "(left-id, right-id) = (" + joinEdge.Left.Id.ToString() + "," + joinEdge.Right.Id.ToString() + ")");
      this.MarkTableAsEliminated<ColumnVar>(joinEdge.Right, joinEdge.Left, joinEdge.RightVars, joinEdge.LeftVars);
    }

    private static bool HasNonKeyReferences(Table table) => !table.Keys.Subsumes(table.ReferencedColumns);
    ...
    {
      if (joinEdge.JoinKind == JoinKind.LeftOuter && fkConstraint.ChildMultiplicity == RelationshipMultiplicity.Many || !JoinGraph.IsJoinOnFkConstraint(fkConstraint, (IList<ColumnVar>) joinEdge.LeftVars, (IList<ColumnVar>) joinEdge.RightVars, out IList<ColumnVar> _))
        return false;
    ...
    {
      IList<ColumnVar> childForeignKeyVars;
      return JoinGraph.IsJoinOnFkConstraint(fkConstraint, (IList<ColumnVar>) joinEdge.RightVars, (IList<ColumnVar>) joinEdge.LeftVars, out childForeignKeyVars) && (childForeignKeyVars.Count <= 1 || childForeignKeyVars.Where<ColumnVar>((Func<ColumnVar, bool>) (v => v.ColumnMetadata.IsNullable)).Count<ColumnVar>() <= 0) && this.TryEliminateRightTable(joinEdge, fkConstraint.ParentKeys.Count, true);
    }
    ...
    private void EliminateParentChildJoins()
    {
      foreach (AugmentedNode vertex in this.m_vertexes)
      {
        if (vertex is AugmentedTableNode tableNode1 && !tableNode1.IsEliminated)
          this.EliminateParentChildJoins(tableNode1);
      }
    }
    ...
    private System.Data.Entity.Core.Query.InternalTrees.Node BuildNodeTree()
    {
      ...
      foreach (KeyValuePair<Var, Var> var1 in this.m_varMap)
      {
        Var key;
        Var var2;
      ...
      VarVec nonNullableColumns)
    {
      if (nonNullableColumns == null)
        return inputNode;
      VarVec varVec = nonNullableColumns.Remap((IDictionary<Var, Var>) this.m_varMap);
      if (varVec.IsEmpty)
        return inputNode;
      System.Data.Entity.Core.Query.InternalTrees.Node node1 = (System.Data.Entity.Core.Query.InternalTrees.Node) null;
      foreach (Var v in varVec)
      {
        System.Data.Entity.Core.Query.InternalTrees.Node node2 = this.m_command.CreateNode((Op) this.m_command.CreateVarRefOp(v));
        ...
        System.Data.Entity.Core.Query.InternalTrees.Node node4 = this.m_command.CreateNode((Op) this.m_command.CreateConditionalOp(OpType.Not), node3);
        node1 = node1 != null ? this.m_command.CreateNode((Op) this.m_command.CreateConditionalOp(OpType.And), node1, node4) : node4;
      }
    ...
    {
      return predicateNode == null ? inputNode : this.m_command.CreateNode((Op) this.m_command.CreateFilterOp(), inputNode, predicateNode);
    }
      ...
      out int minLocationId)
    {
      ...
      for (int index = 0; index < joinNode.LeftVars.Count; ++index)
      {
        ...
        if (!leftVar.Equals((object) rightVar))
        {
          ...
          System.Data.Entity.Core.Query.InternalTrees.Node node1 = this.m_command.CreateNode((Op) this.m_command.CreateVarRefOp(leftVar));
          System.Data.Entity.Core.Query.InternalTrees.Node node2 = this.m_command.CreateNode((Op) this.m_command.CreateVarRefOp(rightVar));
          System.Data.Entity.Core.Query.InternalTrees.Node node3 = this.m_command.CreateNode((Op) this.m_command.CreateComparisonOp(OpType.EQ), node1, node2);
          predicate2 = predicate2 == null ? node3 : PlanCompilerUtil.CombinePredicates(node3, predicate2, this.m_command);
        }
      }
      return predicate2;
    }
      ...
      out Dictionary<System.Data.Entity.Core.Query.InternalTrees.Node, int> predicates)
    {
      ...
      if (inputNode1 == null && inputNode2 == null)
        return predicateNode == null ? (System.Data.Entity.Core.Query.InternalTrees.Node) null : this.BuildFilterNode(this.m_command.CreateNode((Op) this.m_command.CreateSingleRowTableOp()), predicateNode);
      if (inputNode1 == null)
      ...
      AugmentedTableNode tableNode)
    {
      ...
      return augmentedTableNode.NewLocationId < tableNode.Id ? (System.Data.Entity.Core.Query.InternalTrees.Node) null : this.BuildFilterForNullableColumns(augmentedTableNode.Node, augmentedTableNode.NullableColumns);
    }
      ...
      System.Data.Entity.Core.Query.InternalTrees.Node node2)
    {
      if (node1 == null)
        return node2;
      return node2 == null ? node1 : PlanCompilerUtil.CombinePredicates(node1, node2, this.m_command);
    }
    ...
    private int GetLocationId(Var var, int defaultLocationId)
    {
      ...
      return augmentedTableNode.IsMoved ? augmentedTableNode.NewLocationId : augmentedTableNode.Id;
    }
