...
namespace System.Data.Entity.Core.Query.PlanCompiler
{
  internal class ProjectionPruner : BasicOpVisitorOfNode
  {
    ...
    internal static void Process(System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler compilerState) => compilerState.Command.Root = ProjectionPruner.Process(compilerState, compilerState.Command.Root);
    ...
    private System.Data.Entity.Core.Query.InternalTrees.Node Process(System.Data.Entity.Core.Query.InternalTrees.Node node) => this.VisitNode(node);

    private void AddReference(Var v) => this.m_referencedVars.Set(v);
    ...
    private bool IsReferenced(Var v) => this.m_referencedVars.IsSet(v);

    private bool IsUnreferenced(Var v) => !this.IsReferenced(v);

    private void PruneVarMap(VarMap varMap)
    {
      List<Var> varList = new List<Var>();
      foreach (Var key in (IEnumerable<Var>) varMap.Keys)
      {
    ...
    private void PruneVarSet(VarVec varSet) => varSet.And(this.m_referencedVars);
    ...
    private void RemoveRedundantConstantKeys(VarVec keyVec, VarVec outputVec, System.Data.Entity.Core.Query.InternalTrees.Node varDefListNode)
    {
      ...
      varDefListNode.Children.RemoveAll((Predicate<System.Data.Entity.Core.Query.InternalTrees.Node>) (c => constantKeys.Contains(c) && constantKeyVars.IsSet(((VarDefOp) c.Op).Var)));
      if (keyVec.Count != 0)
        return;
    ...
    public override System.Data.Entity.Core.Query.InternalTrees.Node Visit(ProjectOp op, System.Data.Entity.Core.Query.InternalTrees.Node n)
    {
      this.PruneVarSet(op.Outputs);
      this.VisitChildrenReverse(n);
      return !op.Outputs.IsEmpty ? n : n.Child0;
    }
