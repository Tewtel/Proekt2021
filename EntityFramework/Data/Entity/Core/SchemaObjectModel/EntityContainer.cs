...
namespace System.Data.Entity.Core.SchemaObjectModel
{
  ...
  internal sealed class EntityContainer : SchemaType
  {
    ...
    public EntityContainer ExtendingEntityContainer => this._entityContainerGettingExtended;
    ...
    private void HandleExtendsAttribute(XmlReader reader) => this._unresolvedExtendedEntityContainerName = this.HandleUndottedNameAttribute(reader, this._unresolvedExtendedEntityContainerName);
    ...
    internal override void Validate()
    {
      ...
      foreach (SchemaElement member in this.Members)
      {
        if (member is EntityContainerEntitySet entitySet2 && this.Schema.DataModel == SchemaDataModelOption.ProviderDataModel)
          this.CheckForDuplicateTableMapping(tableKeys, entitySet2);
        member.Validate();
      }
    ...
    private void ValidateOnlyBaseEntitySetTypeDefinesConcurrency()
    {
      ...
      foreach (SchemaElement member in this.Members)
      {
        if (member is EntityContainerEntitySet containerEntitySet1 && !baseEntitySetTypes.ContainsKey(containerEntitySet1.EntityType))
          baseEntitySetTypes.Add(containerEntitySet1.EntityType, containerEntitySet1);
      }
      foreach (SchemaType schemaType in this.Schema.SchemaTypes)
      {
        EntityContainerEntitySet set;
        if (schemaType is SchemaEntityType itemType2 && EntityContainer.TypeIsSubTypeOf(itemType2, baseEntitySetTypes, out set) && EntityContainer.TypeDefinesNewConcurrencyProperties(itemType2))
          this.AddError(ErrorCode.ConcurrencyRedefinedOnSubTypeOfEntitySetType, EdmSchemaErrorSeverity.Error, (object) Strings.ConcurrencyRedefinedOnSubTypeOfEntitySetType((object) itemType2.FQName, (object) set.EntityType.FQName, (object) set.FQName));
      }
    }
    ...
    public override string FQName => this.Name;
