...
namespace System.Data.Entity.Core.Metadata.Edm
{
  ...
  public class TypeUsage : MetadataItem
  {
    ...
    internal static TypeUsage Create(EdmType edmType) => new TypeUsage(edmType);

    internal static TypeUsage Create(EdmType edmType, FacetValues values) => new TypeUsage(edmType, TypeUsage.GetDefaultFacetDescriptionsAndOverrideFacetValues(edmType, values));
    ...
    public static TypeUsage Create(EdmType edmType, IEnumerable<Facet> facets) => new TypeUsage(edmType, facets);

    internal TypeUsage ShallowCopy(FacetValues facetValues) => TypeUsage.Create(this._edmType, TypeUsage.OverrideFacetValues((IEnumerable<Facet>) this.Facets, facetValues));

    internal TypeUsage ShallowCopy(params Facet[] facetValues) => TypeUsage.Create(this._edmType, TypeUsage.OverrideFacetValues((IEnumerable<Facet>) this.Facets, (IEnumerable<Facet>) facetValues));
      ...
      int maxLength)
    {
      ...
      if (primitiveType.PrimitiveTypeKind != PrimitiveTypeKind.String)
        throw new ArgumentException(System.Data.Entity.Resources.Strings.NotStringTypeForTypeUsage);
      ...
      int maxLength)
    {
      ...
      if (primitiveType.PrimitiveTypeKind != PrimitiveTypeKind.Binary)
        throw new ArgumentException(System.Data.Entity.Resources.Strings.NotBinaryTypeForTypeUsage);
      TypeUsage.ValidateMaxLength(maxLength);
      return TypeUsage.Create((EdmType) primitiveType, new FacetValues()
      {
        MaxLength = (FacetValueContainer<int?>) new int?(maxLength),
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<PrimitiveType>(primitiveType, nameof (primitiveType));
      return primitiveType.PrimitiveTypeKind == PrimitiveTypeKind.DateTime ? TypeUsage.Create((EdmType) primitiveType, new FacetValues()
      {
        Precision = (FacetValueContainer<byte?>) precision
      }) : throw new ArgumentException(System.Data.Entity.Resources.Strings.NotDateTimeTypeForTypeUsage);
    }
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<PrimitiveType>(primitiveType, nameof (primitiveType));
      return primitiveType.PrimitiveTypeKind == PrimitiveTypeKind.DateTimeOffset ? TypeUsage.Create((EdmType) primitiveType, new FacetValues()
      {
        Precision = (FacetValueContainer<byte?>) precision
      }) : throw new ArgumentException(System.Data.Entity.Resources.Strings.NotDateTimeOffsetTypeForTypeUsage);
    }
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<PrimitiveType>(primitiveType, nameof (primitiveType));
      return primitiveType.PrimitiveTypeKind == PrimitiveTypeKind.Time ? TypeUsage.Create((EdmType) primitiveType, new FacetValues()
      {
        Precision = (FacetValueContainer<byte?>) precision
      }) : throw new ArgumentException(System.Data.Entity.Resources.Strings.NotTimeTypeForTypeUsage);
    }
      ...
      byte scale)
    {
      ...
      if (primitiveType.PrimitiveTypeKind != PrimitiveTypeKind.Decimal)
        throw new ArgumentException(System.Data.Entity.Resources.Strings.NotDecimalTypeForTypeUsage);
      return TypeUsage.Create((EdmType) primitiveType, new FacetValues()
      {
    ...
    public virtual EdmType EdmType => this._edmType;
      ...
      get
      {
        if (this._modelTypeUsage == null)
        {
          ...
          else if (Helper.IsPrimitiveType(edmType))
          {
            ...
            if (typeUsage == null)
              throw new ProviderIncompatibleException(System.Data.Entity.Resources.Strings.Mapping_ProviderReturnsNullType((object) this.ToString()));
    ...
    /// </param>
    public bool IsSubtypeOf(TypeUsage typeUsage) => this.EdmType != null && typeUsage != null && this.EdmType.IsSubtypeOf(typeUsage.EdmType);

    private IEnumerable<Facet> GetFacets() => this._edmType.GetAssociatedFacetDescriptions().Select<FacetDescription, Facet>((Func<FacetDescription, Facet>) (facetDescription => facetDescription.DefaultValueFacet));

    internal override void SetReadOnly() => base.SetReadOnly();
    ...
    /// </returns>
    public override string ToString() => this.EdmType.ToString();
    ...
    {
      if (maxLength <= 0)
        throw new ArgumentOutOfRangeException(nameof (maxLength), System.Data.Entity.Resources.Strings.InvalidMaxLengthSize);
    }
  }
}
