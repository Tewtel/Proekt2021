...
namespace System.Data.Entity.Core.Metadata.Edm
{
  internal class MetadataOptimization
  {
    ...
    internal MetadataOptimization(MetadataWorkspace workspace) => this._workspace = workspace;

    internal IDictionary<Type, EntitySetTypePair> EntitySetMappingCache => this._entitySetMappingsCache;
        ...
        while (entitySet == null && entityTypeStack.Count > 0)
        {
          ...
          foreach (EntityContainer entityContainer in this._workspace.GetItems<EntityContainer>(DataSpace.CSpace))
          {
            ...
            if (count > 1 || count == 1 && entitySet != null)
              throw System.Data.Entity.Resources.Error.DbContext_MESTNotSupported();
            if (count == 1)
      ...
      EntitySet endEntitySet)
    {
      ...
      if (associationTypeToSets is AssociationSet associationSet1)
        return associationSet1.AssociationSetEnds[endName].EntitySet != endEntitySet ? (AssociationSet) null : associationSet1;
      ...
      int associationTypeCount)
    {
      ...
      foreach (EntityContainer entityContainer in itemCollection.GetItems<EntityContainer>())
      {
        ...
        {
          if (baseEntitySet is AssociationSet newItem2)
          {
            int index = newItem2.ElementType.Index;
            MetadataOptimization.AddItemAtIndex<AssociationSet>(array, index, newItem2);
          }
        }
      }
      return array;
    ...
    private static void AddItemAtIndex<T>(object[] array, int index, T newItem) where T : class
    {
      ...
      else if (obj1 is T obj3)
      {
        array[index] = (object) new T[2]{ obj3, newItem };
      }
      else
      {
        T[] array1 = (T[]) obj1;
