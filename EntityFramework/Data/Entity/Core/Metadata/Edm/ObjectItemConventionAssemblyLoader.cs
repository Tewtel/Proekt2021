...
namespace System.Data.Entity.Core.Metadata.Edm
{
  internal class ObjectItemConventionAssemblyLoader : ObjectItemAssemblyLoader
  {
    ...
    public virtual MutableAssemblyCacheEntry CacheEntry => (MutableAssemblyCacheEntry) base.CacheEntry;
    ...
    internal class ConventionOSpaceTypeFactory : OSpaceTypeFactory
    {
      ...
      public ConventionOSpaceTypeFactory(ObjectItemConventionAssemblyLoader loader) => this._loader = loader;

      public override List<Action> ReferenceResolutions => this._loader._referenceResolutions;

      public override void LogLoadMessage(string message, EdmType relatedType) => this._loader.SessionData.LoadMessageLogger.LogLoadMessage(message, relatedType);

      public override void LogError(string errorMessage, EdmType relatedType) => this._loader.SessionData.EdmItemErrors.Add(new EdmItemError(this._loader.SessionData.LoadMessageLogger.CreateErrorMessageWithTypeSpecificLoadLogs(errorMessage, relatedType)));

      public override void TrackClosure(Type type) => this._loader.TrackClosure(type);

      public override Dictionary<EdmType, EdmType> CspaceToOspace => this._loader.SessionData.CspaceToOspace;

      public override Dictionary<string, EdmType> LoadedTypes => this._loader.SessionData.TypesInLoading;

      public override void AddToTypesInAssembly(EdmType type) => this._loader.CacheEntry.TypesInAssembly.Add(type);
    }
  }
}
