...
namespace System.Data.Entity.Core.Metadata.Edm
{
    ...
    where T : MetadataItem
  {
    ...
    internal MetadataCollection() => this._metadataList = new List<T>();
    ...
    private MetadataCollection(List<T> items) => this._metadataList = items;

    internal static MetadataCollection<T> Wrap(List<T> items) => new MetadataCollection<T>(items);

    public virtual int Count => this._metadataList.Count;

    public virtual T this[int index]
    {
      get => this._metadataList[index];
      set
      {
    ...
    internal void HandleIdentityChange(T item, string initialIdentity) => this.HandleIdentityChange(item, initialIdentity, true);

    private void HandleIdentityChange(T item, string initialIdentity, bool validate)
    {
      T obj;
      if (this._caseSensitiveDictionary != null && (!validate || this._caseSensitiveDictionary.TryGetValue(initialIdentity, out obj) && (object) obj == (object) item))
      {
    ...
    {
      get => this.GetValue(identity, false);
    ...
    public virtual bool TryGetValue(string identity, bool ignoreCase, out T item) => !ignoreCase ? this.FindCaseSensitive(identity, out item) : this.FindCaseInsensitive(identity, out item, false);
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<IEnumerable<T>>(items, nameof (items));
      foreach (T obj in items)
      {
        if ((object) obj == null)
          throw new ArgumentException(Strings.ADP_CollectionParameterElementIsNull((object) nameof (items)));
    ...
    public virtual ReadOnlyCollection<T> AsReadOnly => new ReadOnlyCollection<T>((IList<T>) this._metadataList);

    public virtual ReadOnlyMetadataCollection<T> AsReadOnlyMetadataCollection() => new ReadOnlyMetadataCollection<T>(this);

    public bool IsReadOnly => this._readOnly;

    internal void ResetReadOnly() => this._readOnly = false;
    ...
    void IList<T>.Insert(int index, T item) => throw new InvalidOperationException(Strings.OperationOnReadOnlyCollection);

    bool ICollection<T>.Remove(T item) => throw new InvalidOperationException(Strings.OperationOnReadOnlyCollection);

    void IList<T>.RemoveAt(int index) => throw new InvalidOperationException(Strings.OperationOnReadOnlyCollection);

    void ICollection<T>.Clear() => throw new InvalidOperationException(Strings.OperationOnReadOnlyCollection);

    public bool Contains(T item)
    {
      T obj;
      return this.TryGetValue(item.Identity, false, out obj) && (object) obj == (object) item;
    }

    public virtual bool ContainsIdentity(string identity) => this.ContainsIdentityCaseSensitive(identity);

    public virtual int IndexOf(T item) => this._metadataList.IndexOf(item);

    public virtual void CopyTo(T[] array, int arrayIndex) => this._metadataList.CopyTo(array, arrayIndex);

    public ReadOnlyMetadataCollection<T>.Enumerator GetEnumerator() => new ReadOnlyMetadataCollection<T>.Enumerator((IList<T>) this);
    ...
    internal bool HasCaseSensitiveDictionary => this._caseSensitiveDictionary != null;

    internal bool HasCaseInsensitiveDictionary => this._caseInsensitiveDictionary != null;
    ...
    private bool ContainsIdentityCaseSensitive(string identity)
    {
      ...
      return sensitiveDictionary != null ? sensitiveDictionary.ContainsKey(identity) : this.ListContainsIdentityCaseSensitive(identity);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void RemoveFromCaseSensitiveDictionary(string identity) => this._caseSensitiveDictionary.Remove(identity);
