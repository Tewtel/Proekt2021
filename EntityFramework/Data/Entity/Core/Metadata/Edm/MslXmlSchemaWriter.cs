...
namespace System.Data.Entity.Core.Metadata.Edm
{
  internal class MslXmlSchemaWriter : XmlSchemaWriter
  {
    ...
    private void WriteFunctionImportResultMappingElement(FunctionImportResultMapping resultMapping)
    {
      ...
      {
        if (typeMapping is FunctionImportEntityTypeMapping entityTypeMapping1)
          this.WriteFunctionImportEntityTypeMappingElement(entityTypeMapping1);
        else
      ...
      FunctionImportEntityTypeMappingCondition condition)
    {
      ...
      if (condition is FunctionImportEntityTypeMappingConditionIsNull mappingConditionIsNull)
    ...
    private void WriteFunctionImportEndElement() => this._xmlWriter.WriteEndElement();
      ...
      EntityTypeModificationFunctionMapping modificationFunctionMapping)
    {
      ...
      this.WriteFunctionMapping("InsertFunction", modificationFunctionMapping.InsertFunctionMapping);
      this.WriteFunctionMapping("UpdateFunction", modificationFunctionMapping.UpdateFunctionMapping);
      this.WriteFunctionMapping("DeleteFunction", modificationFunctionMapping.DeleteFunctionMapping);
      this._xmlWriter.WriteEndElement();
    }
      ...
      bool associationSetMapping = false)
    {
      ...
      {
        this.WritePropertyParameterBindings((IEnumerable<ModificationFunctionParameterBinding>) functionMapping.ParameterBindings);
    ...
    private void WritePropertyParameterBindings(
      ...
      int level = 0)
    {
      foreach (IGrouping<EdmMember, ModificationFunctionParameterBinding> grouping in parameterBindings.Where<ModificationFunctionParameterBinding>((Func<ModificationFunctionParameterBinding, bool>) (pm => pm.MemberPath.AssociationSetEnd == null && pm.MemberPath.Members.Count<EdmMember>() > level)).GroupBy<ModificationFunctionParameterBinding, EdmMember>((Func<ModificationFunctionParameterBinding, EdmMember>) (pm => pm.MemberPath.Members.ElementAt<EdmMember>(level))))
      {
    ...
    private void WritePropertyMapping(ScalarPropertyMapping scalarPropertyMapping) => this.WriteScalarPropertyElement(scalarPropertyMapping.Property.Name, scalarPropertyMapping.Column.Name);
    ...
    {
      return isHierarchyMapping ? "IsTypeOf(" + fullyQualifiedEntityTypeName + ")" : fullyQualifiedEntityTypeName;
    }
    ...
    private void WriteIsNullConditionAttribute(bool isNullValue) => this._xmlWriter.WriteAttributeString("IsNull", XmlSchemaWriter.GetLowerCaseStringFromBoolValue(isNullValue));

    private void WriteConditionValue(object conditionValue)
    {
      if (conditionValue is bool flag)
        this._xmlWriter.WriteAttributeString("Value", flag ? "1" : "0");
      else
