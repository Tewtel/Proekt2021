...
namespace System.Data.Entity.Core.Metadata.Edm
{
  internal sealed class EdmSerializationVisitor : EdmModelVisitor
  {
    ...
    public EdmSerializationVisitor(EdmXmlSchemaWriter schemaWriter) => this._schemaWriter = schemaWriter;
    ...
    public void Visit(EdmModel edmModel, string provider, string providerManifestToken) => this.Visit(edmModel, edmModel.Containers.Single<EntityContainer>().Name + "Schema", provider, providerManifestToken);
    ...
    protected override void VisitKeyProperties(EntityType entityType, IList<EdmProperty> properties)
    {
      ...
      foreach (EdmProperty property in (IEnumerable<EdmProperty>) properties)
        this._schemaWriter.WriteDeclaredKeyPropertyRefElement(property);
      this._schemaWriter.WriteEndElement();
    }
    ...
    private static void AppendSchemaErrors(StringBuilder builder, MetadataItem item)
    {
      if (!MetadataItemHelper.HasSchemaErrors(item))
        return;
      builder.Append(System.Data.Entity.Resources.Strings.MetadataItemErrorsFoundDuringGeneration);
    ...
    private void WriteComment(string comment) => this._schemaWriter.WriteComment(comment.Replace("--", "- -"));
  }
}
