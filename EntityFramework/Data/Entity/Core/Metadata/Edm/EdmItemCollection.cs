...
namespace System.Data.Entity.Core.Metadata.Edm
{
  ...
  public sealed class EdmItemCollection : ItemCollection
  {
    ...
    private void Init() => this.LoadEdmPrimitiveTypesAndFunctions();
    ...
    {
      get => this._edmVersion;
      internal set => this._edmVersion = value;
    }

    internal OcAssemblyCache ConventionalOcCache => this._conventionalOcCache;
    ...
    internal static bool IsSystemNamespace(DbProviderManifest manifest, string namespaceName)
    {
      ...
      if (namespaceName == "Transient" || namespaceName == "Edm" || namespaceName == "System")
        return true;
      return manifest != null && namespaceName == manifest.NamespaceName;
    }
      ...
      bool throwOnError)
    {
      ...
      return !(!MetadataHelper.CheckIfAllErrorsAreWarnings(andValidate) & throwOnError) ? andValidate : throw EntityUtil.InvalidSchemaEncountered(Helper.CombineErrorMessage((IEnumerable<EdmSchemaError>) andValidate));
    }
      ...
      ItemCollection itemCollection)
    {
      ...
      foreach (GlobalItem globalItem in globalItems)
      {
        if (globalItem.BuiltInTypeKind == BuiltInTypeKind.EdmFunction && globalItem.DataSpace == DataSpace.SSpace)
        {
          ...
          {
            edmSchemaErrorList.Add(new EdmSchemaError(System.Data.Entity.Resources.Strings.DuplicatedFunctionoverloads((object) edmFunction.FullName, (object) str.Substring(edmFunction.FullName.Length)).Trim(), 174, EdmSchemaErrorSeverity.Error));
            continue;
          }
          stringList.Add(str);
        }
    ...
    /// objects.
    /// </returns>
    public ReadOnlyCollection<PrimitiveType> GetPrimitiveTypes() => this._primitiveTypeMaps.GetTypes();
      ...
      double edmVersion)
    {
      if (edmVersion == 1.0 || edmVersion == 1.1 || edmVersion == 2.0)
        return new ReadOnlyCollection<PrimitiveType>((IList<PrimitiveType>) this._primitiveTypeMaps.GetTypes().Where<PrimitiveType>((Func<PrimitiveType, bool>) (type => !Helper.IsSpatialType(type) && !Helper.IsHierarchyIdType(type))).ToList<PrimitiveType>());
      if (edmVersion == 3.0)
        return this._primitiveTypeMaps.GetTypes();
      throw new ArgumentException(System.Data.Entity.Resources.Strings.InvalidEDMVersion((object) edmVersion.ToString((IFormatProvider) CultureInfo.CurrentCulture)));
    }
    ...
    internal DbLambda GenerateFunctionDefinition(EdmFunction function)
    {
      if (!function.HasUserDefinedBody)
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Cqt_UDF_FunctionHasNoDefinition((object) function.Identity));
      ...
      if (!TypeSemantics.IsStructurallyEqual(function.ReturnParameter.TypeUsage, dbLambda.Body.ResultType))
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Cqt_UDF_FunctionDefinitionResultTypeMismatch((object) function.ReturnParameter.TypeUsage.ToString(), (object) function.FullName, (object) dbLambda.Body.ResultType.ToString()));
      return dbLambda;
    }
      ...
      out IList<EdmSchemaError> errors)
    {
      ...
      return errors == null || errors.Count <= 0 ? edmItemCollection : (EdmItemCollection) null;
    }
  }
}
