...
namespace System.Data.Entity.Core.Metadata.Edm
{
  internal class MetadataArtifactLoaderCompositeResource : MetadataArtifactLoader
  {
    ...
    public override string Path => this._originalPath;

    public override bool IsComposite => true;

    public override List<string> GetOriginalPaths(DataSpace spaceToGet) => this.GetOriginalPaths();
    ...
    internal static string CreateResPath(Assembly assembly, string resourceName) => string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0}{1}{2}{3}", (object) MetadataArtifactLoader.resPathPrefix, (object) assembly.FullName, (object) MetadataArtifactLoader.resPathSeparator, (object) resourceName);

    internal static string[] GetManifestResourceNamesForAssembly(Assembly assembly) => assembly.IsDynamic ? new string[0] : assembly.GetManifestResourceNames();
      ...
      MetadataArtifactAssemblyResolver resolver)
    {
      ...
      int num = assemblyName == null ? 0 : (resourceName == null ? 1 : (assemblyName.Trim() == MetadataArtifactLoader.wildcard ? 1 : 0));
      MetadataArtifactLoaderCompositeResource.ValidateExtension(extensionCheck, validExtension, resourceName);
      return num != 0 ? (MetadataArtifactLoader) new MetadataArtifactLoaderCompositeResource(path, assemblyName, resourceName, uriRegistry, resolver) : (MetadataArtifactLoader) new MetadataArtifactLoaderResource(MetadataArtifactLoaderCompositeResource.ResolveAssemblyName(assemblyName, resolver), resourceName, uriRegistry);
    }
