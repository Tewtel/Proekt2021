...
namespace System.Data.Entity.Core.Metadata.Edm
{
  internal class EdmXmlSchemaWriter : XmlSchemaWriter
  {
    ...
    private static string SyndicationItemPropertyToString(object value) => EdmXmlSchemaWriter._syndicationItemToTargetPath[(int) value];

    private static string SyndicationTextContentKindToString(object value) => EdmXmlSchemaWriter._syndicationTextContentKindToString[(int) value];

    public EdmXmlSchemaWriter() => this._resolver = DbConfiguration.DependencyResolver;
    ...
    public virtual void WriteFunctionParameterHeader(FunctionParameter functionParameter)
    {
      ...
      {
        int? maxLength = functionParameter.MaxLength;
        if (maxLength.HasValue)
        {
          XmlWriter xmlWriter = this._xmlWriter;
          maxLength = functionParameter.MaxLength;
          string str = maxLength.Value.ToString((IFormatProvider) CultureInfo.InvariantCulture);
          xmlWriter.WriteAttributeString("MaxLength", str);
        }
      }
      byte? nullable;
      byte num;
      ...
      {
        nullable = functionParameter.Precision;
        if (nullable.HasValue)
        {
          ...
          xmlWriter.WriteAttributeString("Precision", str);
        }
      }
      if (functionParameter.IsScaleConstant)
        return;
      nullable = functionParameter.Scale;
      if (!nullable.HasValue)
        return;
      XmlWriter xmlWriter1 = this._xmlWriter;
      nullable = functionParameter.Scale;
      num = nullable.Value;
      string str1 = num.ToString((IFormatProvider) CultureInfo.InvariantCulture);
      xmlWriter1.WriteAttributeString("Scale", str1);
    }

    internal virtual void WriteFunctionReturnTypeElementHeader() => this._xmlWriter.WriteStartElement("ReturnType");
    ...
    internal void WriteEntityTypeElementHeader(EntityType entityType)
    {
      ...
      {
        foreach (System.Attribute clrAttribute in (IEnumerable<System.Attribute>) entityType.Annotations.GetClrAttributes())
        {
    ...
    private static void AddAttributeAnnotation(EdmProperty property, System.Attribute a)
    {
      if (property == null)
        return;
      IList<System.Attribute> clrAttributes = property.Annotations.GetClrAttributes();
      if (clrAttributes != null)
      {
        ...
        property.GetMetadataProperties().SetClrAttributes((IList<System.Attribute>) new List<System.Attribute>()
        {
          a
        });
    }
    ...
    internal virtual void WriteCollectionTypeElementHeader() => this._xmlWriter.WriteStartElement("CollectionType");

    internal virtual void WriteRowTypeElementHeader() => this._xmlWriter.WriteStartElement("RowType");
    ...
    internal void WriteReferentialConstraintElementHeader() => this._xmlWriter.WriteStartElement("ReferentialConstraint");

    internal void WriteDeclaredKeyPropertiesElementHeader() => this._xmlWriter.WriteStartElement("Key");
    ...
    internal void WritePropertyElementHeader(EdmProperty property)
    {
      ...
      {
        int num1 = 0;
        foreach (System.Attribute clrAttribute in (IEnumerable<System.Attribute>) property.Annotations.GetClrAttributes())
        {
      ...
      if (!property.IsFixedLengthConstant)
      {
        ...
        if (nullable1.HasValue)
        {
          XmlWriter xmlWriter = this._xmlWriter;
          nullable1 = property.IsFixedLength;
          string stringFromBoolValue = XmlSchemaWriter.GetLowerCaseStringFromBoolValue(nullable1.Value);
          xmlWriter.WriteAttributeString("FixedLength", stringFromBoolValue);
        }
      }
      if (!property.IsUnicodeConstant)
      {
        ...
        if (nullable1.HasValue)
        {
          ...
          nullable1 = property.IsUnicode;
          string stringFromBoolValue = XmlSchemaWriter.GetLowerCaseStringFromBoolValue(nullable1.Value);
          xmlWriter.WriteAttributeString("Unicode", stringFromBoolValue);
        }
      }
      byte? nullable2;
      byte num;
      ...
      if (!property.IsScaleConstant)
      {
        ...
        if (nullable2.HasValue)
        {
          ...
          num = nullable2.Value;
          string str = num.ToString((IFormatProvider) CultureInfo.InvariantCulture);
          xmlWriter.WriteAttributeString("Scale", str);
        }
      }
    ...
    private static string GetTypeReferenceName(EdmProperty property)
    {
      ...
      return property.IsComplexType ? XmlSchemaWriter.GetQualifiedTypeName("Self", property.ComplexType.Name) : XmlSchemaWriter.GetQualifiedTypeName("Self", property.EnumType.Name);
    }
    ...
    internal EdmXmlSchemaWriter Replicate(XmlWriter xmlWriter) => new EdmXmlSchemaWriter(xmlWriter, this._version, this._serializeDefaultNullability);
    ...
    private static string GetTypeName(EdmType type) => type.BuiltInTypeKind == BuiltInTypeKind.CollectionType ? string.Format((IFormatProvider) CultureInfo.InvariantCulture, "Collection({0})", (object) EdmXmlSchemaWriter.GetTypeName(((CollectionType) type).TypeUsage.EdmType)) : (type.BuiltInTypeKind != BuiltInTypeKind.PrimitiveType ? type.FullName : type.Name);
