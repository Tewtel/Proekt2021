...
namespace System.Data.Entity.Core.Objects.Internal
{
  internal class TransactionManager
  {
    ...
    {
      get => this._originalMergeOption;
      set => this._originalMergeOption = value;
    }
    ...
    internal bool IsGraphUpdate => (uint) this._graphUpdateCount > 0U;
    ...
    internal void BeginAlignChanges() => this.IsAlignChanges = true;

    internal void EndAlignChanges() => this.IsAlignChanges = false;

    internal void ResetProcessedEntities() => this.ProcessedEntities.Clear();

    internal void BeginLocalPublicAPI() => this.IsLocalPublicAPI = true;

    internal void EndLocalPublicAPI() => this.IsLocalPublicAPI = false;

    internal void BeginOriginalValuesGetter() => this.IsOriginalValuesGetter = true;

    internal void EndOriginalValuesGetter() => this.IsOriginalValuesGetter = false;
    ...
    internal void BeginRelatedEndAdd() => this.IsRelatedEndAdd = true;

    internal void EndRelatedEndAdd() => this.IsRelatedEndAdd = false;

    internal void BeginGraphUpdate() => ++this._graphUpdateCount;

    internal void EndGraphUpdate() => --this._graphUpdateCount;

    internal void BeginDetaching() => this.IsDetaching = true;

    internal void EndDetaching() => this.IsDetaching = false;

    internal void BeginFixupKeysByReference() => this.IsFixupByReference = true;

    internal void EndFixupKeysByReference() => this.IsFixupByReference = false;
  }
}
