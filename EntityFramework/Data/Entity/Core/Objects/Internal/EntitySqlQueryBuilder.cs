...
namespace System.Data.Entity.Core.Objects.Internal
{
  internal static class EntitySqlQueryBuilder
  {
    ...
    private static ObjectQueryState NewBuilderQuery(
      ...
      IEnumerable<ObjectParameter> enumerableParams)
    {
      switch (enumerableParams)
      {
        case ObjectParameterCollection parameters:
        case null:
        ...
        default:
          ...
          using (IEnumerator<ObjectParameter> enumerator = enumerableParams.GetEnumerator())
          {
      ...
      ObjectParameter[] parameters)
    {
      ...
      string commandText = EntitySqlQueryBuilder.GetCommandText(query);
      int num = ((EntitySqlQueryState) query).AllowsLimitSubclause ? 1 : 0;
      StringBuilder queryText = new StringBuilder(num == 0 ? length + ("SELECT VALUE TOP(\r\n".Length + "\r\n) ".Length + alias.Length + "\r\nFROM (\r\n".Length + commandText.Length + "\r\n) AS ".Length + alias.Length) : length + (commandText.Length + "\r\nLIMIT\r\n".Length));
      if (num != 0)
      {
        queryText.Append(commandText);
        queryText.Append("\r\nLIMIT\r\n");
