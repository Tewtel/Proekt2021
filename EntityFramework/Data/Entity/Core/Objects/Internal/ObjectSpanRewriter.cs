...
namespace System.Data.Entity.Core.Objects.Internal
{
  internal class ObjectSpanRewriter
  {
    ...
    internal static bool EntityTypeEquals(EntityTypeBase entityType1, EntityTypeBase entityType2) => entityType1 == entityType2;
    ...
    internal MetadataWorkspace Metadata => this._tree.MetadataWorkspace;

    internal DbExpression Query => this._toRewrite;

    internal bool RelationshipSpan
    {
      get => this._relationshipSpan;
      set => this._relationshipSpan = value;
    }

    internal SpanIndex SpanIndex => this._spanIndex;

    internal DbExpression RewriteQuery()
    {
      DbExpression dbExpression = this.Rewrite(this._toRewrite);
      return this._toRewrite == dbExpression ? (DbExpression) null : dbExpression;
    }
    ...
    private DbExpression RewriteEntity(DbExpression expression, EntityType entityType)
    {
      ...
      if (relationshipSpanEnds != null)
      {
        ...
        int key = spanTrackingInfo.ColumnDefinitions.Count + 1;
        foreach (KeyValuePair<AssociationEndMember, AssociationEndMember> keyValuePair in relationshipSpanEnds)
        {
          if (spanTrackingInfo.FullSpannedEnds == null || !spanTrackingInfo.FullSpannedEnds.ContainsKey(keyValuePair.Value))
          {
            ...
            spanTrackingInfo.SpannedColumns[key] = keyValuePair.Value;
            ++key;
          }
        }
      }
      ...
      DbNewInstanceExpression instanceExpression = DbExpressionBuilder.NewRow((IEnumerable<KeyValuePair<string, DbExpression>>) spanTrackingInfo.ColumnDefinitions);
      this.AddSpanMap((RowType) instanceExpression.ResultType.EdmType, spanTrackingInfo.SpannedColumns);
    ...
    {
      DbNewInstanceExpression instanceExpression = !(expression is DbLambdaExpression lambdaExpression) ? expression as DbNewInstanceExpression : lambdaExpression.Lambda.Body as DbNewInstanceExpression;
      ...
      for (int index = 0; index < rowType.Properties.Count; ++index)
      {
        ...
        if (!dictionary2.TryGetValue(index, out dbExpression))
          dbExpression = dictionary1[index];
    ...
    private DbExpression RewriteCollection(DbExpression expression)
    {
      ...
      if (projectExpression != null)
      {
        ...
        if (projectExpression.Projection != projection)
    ...
    private void EnterCollection() => this._navSources.Push((ObjectSpanRewriter.NavigationInfo) null);

    private void EnterNavigationCollection(ObjectSpanRewriter.NavigationInfo info) => this._navSources.Push(info);

    private void ExitCollection() => this._navSources.Pop();
      ...
      AssociationEndMember toEnd)
    {
      ...
      return ObjectSpanRewriter.EntityTypeEquals((EntityTypeBase) compareType, (EntityTypeBase) elementType) || TypeSemantics.IsSubTypeOf((EdmType) compareType, (EdmType) elementType) || TypeSemantics.IsSubTypeOf((EdmType) elementType, (EdmType) compareType);
    }
    ...
    private class NavigationInfo
    {
      ...
      public AssociationEndMember SourceEnd => this._sourceEnd;

      public DbExpression Source => this._source;

      public DbVariableReferenceExpression SourceVariable => this._sourceRef;
    }
    ...
    private class RelationshipNavigationVisitor : DefaultExpressionVisitor
    {
      ...
      private RelationshipNavigationVisitor(AliasGenerator aliasGenerator) => this._aliasGenerator = aliasGenerator;

      private DbExpression Find(DbExpression expression) => this.VisitExpression(expression);
      ...
      public override DbExpression Visit(DbFilterExpression expression)
      {
        ...
        return input != expression.Input.Expression ? (DbExpression) input.BindAs(expression.Input.VariableName).Filter(expression.Predicate) : (DbExpression) expression;
      }
      ...
      public override DbExpression Visit(DbProjectExpression expression)
      {
        ...
        {
          DbExpression input = this.Find(expression.Input.Expression);
          if (input != expression.Input.Expression)
      ...
      public override DbExpression Visit(DbSortExpression expression)
      {
        ...
        DbExpression input = this.Find(expression.Input.Expression);
        return input != expression.Input.Expression ? (DbExpression) input.BindAs(expression.Input.VariableName).Sort((IEnumerable<DbSortClause>) expression.SortOrder) : (DbExpression) expression;
      }
      ...
      public override DbExpression Visit(DbSkipExpression expression)
      {
        ...
        DbExpression input = this.Find(expression.Input.Expression);
        return input != expression.Input.Expression ? (DbExpression) input.BindAs(expression.Input.VariableName).Skip((IEnumerable<DbSortClause>) expression.SortOrder, expression.Count) : (DbExpression) expression;
      }
    }
  }
}
