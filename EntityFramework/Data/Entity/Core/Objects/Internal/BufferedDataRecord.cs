...
namespace System.Data.Entity.Core.Objects.Internal
{
  internal abstract class BufferedDataRecord
  {
    ...
    public bool HasRows => this._rowCount > 0;

    public int FieldCount => this._dataTypeNames.Length;
    ...
    public string GetDataTypeName(int ordinal) => this._dataTypeNames[ordinal];

    public Type GetFieldType(int ordinal) => this._fieldTypes[ordinal];

    public string GetName(int ordinal) => this._columnNames[ordinal];

    public int GetOrdinal(string name) => this._fieldNameLookup.Value.GetOrdinal(name);
