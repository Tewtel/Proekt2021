...
namespace System.Data.Entity.Core.Objects
{
  ...
  public class ObjectStateManager : IEntityStateManager
  {
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<MetadataWorkspace>(metadataWorkspace, nameof (metadataWorkspace));
    ...
    internal virtual bool InRelationshipFixup => this._inRelationshipFixup;
    ...
    /// .
    /// </returns>
    public virtual MetadataWorkspace MetadataWorkspace => this._metadataWorkspace;
    ...
    {
      add => this.onObjectStateManagerChangedDelegate += value;
      remove => this.onObjectStateManagerChangedDelegate -= value;
    }

    internal event CollectionChangeEventHandler EntityDeleted
    {
      add => this.onEntityDeletedDelegate += value;
      remove => this.onEntityDeletedDelegate -= value;
    }
    ...
    internal virtual EntityEntry AddKeyEntry(EntityKey entityKey, EntitySet entitySet) => this.FindEntityEntry(entityKey) == null ? this.InternalAddEntityEntry(entityKey, entitySet) : throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_ObjectStateManagerContainsThisEntityKey((object) entitySet.ElementType.Name));

    internal EntityEntry GetOrAddKeyEntry(EntityKey entityKey, EntitySet entitySet)
    {
      EntityEntry entry;
      return this.TryGetEntityEntry(entityKey, out entry) ? entry : this.InternalAddEntityEntry(entityKey, entitySet);
    }
    ...
    private void ValidateProxyType(IEntityWrapper wrappedEntity)
    {
      ...
      if (proxyType == null || proxyType.ProxyType != type)
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityProxyTypeInfo_DuplicateOSpaceType((object) identityType.FullName));
    }
      ...
      bool isAdded)
    {
      ...
      if (isAdded && !entitySet.ElementType.IsAssignableFrom(edmType))
        throw new ArgumentException(System.Data.Entity.Resources.Strings.ObjectStateManager_EntityTypeDoesnotMatchtoEntitySetType((object) wrappedObject.Entity.GetType().Name, (object) TypeHelpers.GetFullName(entitySet.EntityContainer.Name, entitySet.Name)), argumentName);
      EntityKey key = !isAdded ? wrappedObject.EntityKey : wrappedObject.GetEntityKeyFromEntity();
      if ((object) key != null)
      {
        entityKey = key;
        if ((object) entityKey == null)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityKey_UnexpectedNull);
        if (wrappedObject.EntityKey != entityKey)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityKey_DoesntMatchKeyOnEntity((object) wrappedObject.Entity.GetType().FullName));
      }
      ...
      if (isAdded && (key == (EntityKey) null && (entityEntry1 = this.FindEntityEntry(wrappedObject.Entity)) != null || key != (EntityKey) null && (entityEntry1 = this.FindEntityEntry(key)) != null))
      {
        if (entityEntry1.Entity != wrappedObject.Entity)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_ObjectStateManagerContainsThisEntityKey((object) wrappedObject.IdentityType.FullName));
        if (entityEntry1.State != EntityState.Added)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_DoesnotAllowToReAddUnchangedOrModifiedOrDeletedEntity((object) entityEntry1.State));
      ...
      if (!isAdded)
        this.FixupReferencesByForeignKeys(entityEntry2);
      return entityEntry2;
    }
    ...
    internal virtual void RemoveForeignKeyFromIndex(EntityKey foreignKey) => this._danglingForeignKeys.Remove(foreignKey);
      ...
      EntityKey foreignKey)
    {
      ...
      return this._danglingForeignKeys.TryGetValue(foreignKey, out source) ? (IEnumerable<EntityEntry>) source.Select<Tuple<EntityReference, EntityEntry>, EntityEntry>((Func<Tuple<EntityReference, EntityEntry>, EntityEntry>) (e => e.Item2)).ToList<EntityEntry>() : Enumerable.Empty<EntityEntry>();
    }
    ...
    internal virtual bool SomeEntryWithConceptualNullExists() => this._entriesWithConceptualNulls != null && (uint) this._entriesWithConceptualNulls.Count > 0U;

    internal virtual bool EntryHasConceptualNull(EntityEntry entry) => this._entriesWithConceptualNulls != null && this._entriesWithConceptualNulls.Contains(entry);
    ...
    internal virtual void ForgetEntryWithConceptualNull(EntityEntry entry, bool resetAllKeys)
    {
      ...
      {
        if (relationship is EntityReference entityReference1 && ForeignKeyFactory.IsConceptualNullKey(entityReference1.CachedForeignKey))
        {
          if (resetAllKeys)
          {
            entityReference1.SetCachedForeignKey((EntityKey) null, (EntityEntry) null);
          }
          else
          {
            this._entriesWithConceptualNulls.Add(entry);
      ...
      bool keyEntryInitialized)
    {
      ...
      try
      {
        ...
        this.FixupReferencesByForeignKeys(keyEntry);
        flag = false;
      }
      finally
      {
      ...
      bool inKeyEntryPromotion)
    {
        ...
        switch (mergeOption)
        {
          case MergeOption.AppendOnly:
            if (inKeyEntryPromotion && !entityReference.IsEmpty() && entityReference.ReferenceValue.Entity != wrappedTarget.Entity)
              throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_EntityConflictsWithKeyEntry);
            break;
      ...
      if (mergeOption == MergeOption.NoTracking)
      {
        ...
        if (relatedEnd.IsLoaded)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Collections_CannotFillTryDifferentMergeOption((object) relatedEnd.SourceRoleName, (object) relatedEnd.RelationshipName));
      }
      if (relatedEnd == null)
        relatedEnd = relatedEndInternal.GetOtherEndOfRelationship(wrappedTarget);
      ...
      MergeOption mergeOption)
    {
      ...
      if (mergeOption != MergeOption.NoTracking)
        return;
      ((EntityReference) relatedEnd).DetachedEntityKey = wrappedRelatedEntity.EntityKey ?? throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityKey_UnexpectedNull);
    }
        ...
        if (targets != null)
        {
          if (mergeOption == MergeOption.NoTracking)
          {
            ...
            if (!relatedEndInternal.IsEmpty())
              throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Collections_CannotFillTryDifferentMergeOption((object) relatedEndInternal.SourceRoleName, (object) relatedEndInternal.RelationshipName));
          }
          Lazy<ILookup<EntityKey, RelationshipEntry>> lazy = new Lazy<ILookup<EntityKey, RelationshipEntry>>((Func<ILookup<EntityKey, RelationshipEntry>>) (() => ObjectStateManager.GetRelationshipLookup(context.ObjectStateManager, associationSet, sourceMember, sourceKey)));
          foreach (object target in (IEnumerable) targets)
          {
            if (!(target is IEntityWrapper entityWrapper6))
              entityWrapper6 = this.EntityWrapperFactory.WrapEntityUsingContext(target, context);
            ++num;
            if (mergeOption == MergeOption.NoTracking)
            {
              ObjectStateManager.AddEntityToCollectionOrReference(MergeOption.NoTracking, wrappedSource, sourceMember, entityWrapper6, targetMember, setIsLoaded, true, false);
            }
            else
            {
              ObjectStateManager objectStateManager = context.ObjectStateManager;
              EntityKey entityKey = entityWrapper6.EntityKey;
              EntityState newEntryState;
              if (!ObjectStateManager.TryUpdateExistingRelationships(context, mergeOption, associationSet, sourceMember, lazy.Value, wrappedSource, targetMember, entityKey, setIsLoaded, out newEntryState))
              {
                bool flag = true;
                switch (sourceMember.RelationshipMultiplicity)
                {
                  ...
                  case RelationshipMultiplicity.One:
                    ...
                    flag = !ObjectStateManager.TryUpdateExistingRelationships(context, mergeOption, associationSet, targetMember, relationshipLookup, entityWrapper6, sourceMember, sourceKey, setIsLoaded, out newEntryState);
                    break;
                }
                if (flag)
                {
                  ...
                  {
                    ObjectStateManager.AddEntityToCollectionOrReference(mergeOption, wrappedSource, sourceMember, entityWrapper6, targetMember, setIsLoaded, false, false);
                  }
                  else
                  {
    ...
    internal static ILookup<EntityKey, RelationshipEntry> GetRelationshipLookup(
      ObjectStateManager manager,
      AssociationSet associationSet,
      AssociationEndMember sourceMember,
      EntityKey sourceKey)
    {
      List<RelationshipEntry> source = new List<RelationshipEntry>();
      foreach (RelationshipEntry relationshipEntry in manager.FindRelationshipsByKey(sourceKey))
      {
        if (relationshipEntry.IsSameAssociationSetAndRole(associationSet, sourceMember, sourceKey))
          source.Add(relationshipEntry);
      }
      return source.ToLookup<RelationshipEntry, EntityKey>((Func<RelationshipEntry, EntityKey>) (r => r.RelationshipWrapper.GetOtherEntityKey(sourceKey)));
    }
    ...
    internal static bool TryUpdateExistingRelationships(
      ...
      AssociationSet associationSet,
      AssociationEndMember sourceMember,
      ILookup<EntityKey, RelationshipEntry> relationshipLookup,
      IEntityWrapper wrappedSource,
      AssociationEndMember targetMember,
      EntityKey targetKey,
      bool setIsLoaded,
      out EntityState newEntryState)
    {
      ...
      switch (targetMember.RelationshipMultiplicity)
      {
        case RelationshipMultiplicity.ZeroOrOne:
        case RelationshipMultiplicity.One:
          using (IEnumerator<RelationshipEntry> enumerator = relationshipLookup.Where<IGrouping<EntityKey, RelationshipEntry>>((Func<IGrouping<EntityKey, RelationshipEntry>, bool>) (g => g.Key != targetKey)).SelectMany<IGrouping<EntityKey, RelationshipEntry>, RelationshipEntry>((Func<IGrouping<EntityKey, RelationshipEntry>, IEnumerable<RelationshipEntry>>) (re => (IEnumerable<RelationshipEntry>) re)).GetEnumerator())
          {
            while (enumerator.MoveNext())
            {
              RelationshipEntry current = enumerator.Current;
              switch (mergeOption)
              {
                ...
                case MergeOption.OverwriteChanges:
                  ...
                  relationshipEntryList1.Add(current);
                  continue;
                case MergeOption.PreserveChanges:
      ...
      string targetRoleName)
    {
      ...
      if (entityEntry.WrappedEntity.Entity == null || !(entityEntry.WrappedEntity.RelationshipManager.GetRelatedEndInternal(((AssociationSet) relationshipEntry.EntitySet).ElementType.FullName, targetRoleName) is EntityReference relatedEndInternal))
        return;
      ...
      bool forAttach)
    {
      ...
      if (entitySet == null)
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_InvalidKey);
      ...
      for (int index = 0; index < entitySet.ElementType.KeyMembers.Count; ++index)
      {
        ...
        if (!ByValueEqualityComparer.Default.Equals(x, valueByName))
          throw new InvalidOperationException(forAttach ? System.Data.Entity.Resources.Strings.ObjectStateManager_KeyPropertyDoesntMatchValueInKeyForAttach : System.Data.Entity.Resources.Strings.ObjectStateManager_KeyPropertyDoesntMatchValueInKey);
      }
    }
    ...
    private void AddRelationshipEndToLookup(EntityKey key, RelationshipEntry relationship) => this.GetEntityEntry(key).AddRelationshipEnd(relationship);
    ...
    private void DeleteRelationshipEndFromLookup(EntityKey key, RelationshipEntry relationship) => this.GetEntityEntry(key).RemoveRelationshipEnd(relationship);
    ...
    {
      return (object) roleAndKey1.Value == null || (object) roleAndKey2.Value == null ? (RelationshipEntry) null : this.FindRelationship(new RelationshipWrapper((AssociationSet) relationshipSet, roleAndKey1, roleAndKey2));
    }
      ...
      RelationshipWrapper relationshipWrapper)
    {
      ...
      int num = this._unchangedRelationshipStore != null && this._unchangedRelationshipStore.TryGetValue(relationshipWrapper, out relationshipEntry) || this._deletedRelationshipStore != null && this._deletedRelationshipStore.TryGetValue(relationshipWrapper, out relationshipEntry) ? 1 : (this._addedRelationshipStore == null ? 0 : (this._addedRelationshipStore.TryGetValue(relationshipWrapper, out relationshipEntry) ? 1 : 0));
      return relationshipEntry;
    }
    ...
    internal virtual RelationshipEntry[] CopyOfRelationshipsByKey(EntityKey key) => this.FindRelationshipsByKey(key).ToArray();
    ...
    private void ValidateKeylessEntityStore()
    {
      ...
      if (this._keylessEntityStore != null && this._keylessEntityStore.Count == ((IEnumerable<Dictionary<EntityKey, EntityEntry>>) dictionaryArray).Sum<Dictionary<EntityKey, EntityEntry>>((Func<Dictionary<EntityKey, EntityEntry>, int>) (s => s != null ? s.Count : 0)))
        return;
      ...
      foreach (Dictionary<EntityKey, EntityEntry> dictionary in dictionaryArray)
      {
        if (dictionary != null)
        {
          foreach (EntityEntry entityEntry in dictionary.Values)
          {
            if (entityEntry.Entity != null && !(entityEntry.Entity is IEntityWithKey))
              this._keylessEntityStore.TryGetValue(entityEntry.Entity, out EntityEntry _);
          }
        }
      }
    }
    ...
    public virtual IEnumerable<ObjectStateEntry> GetObjectStateEntries(
      EntityState state)
    {
      return (EntityState.Detached & state) == (EntityState) 0 ? this.GetObjectStateEntriesInternal(state) : throw new ArgumentException(System.Data.Entity.Resources.Strings.ObjectStateManager_DetachedObjectStateEntriesDoesNotExistInObjectStateManager);
    }

    IEnumerable<IEntityStateEntry> IEntityStateManager.GetEntityStateEntries(
      EntityState state)
    {
      foreach (IEntityStateEntry entityStateEntry in this.GetObjectStateEntriesInternal(state))
        yield return entityStateEntry;
    }
    ...
    internal virtual bool HasChanges()
    {
      if (this._addedRelationshipStore != null && this._addedRelationshipStore.Count > 0 || this._addedEntityStore != null && this._addedEntityStore.Count > 0 || (this._modifiedEntityStore != null && this._modifiedEntityStore.Count > 0 || this._deletedRelationshipStore != null && this._deletedRelationshipStore.Count > 0))
        return true;
      return this._deletedEntityStore != null && this._deletedEntityStore.Count > 0;
    }
    ...
    internal virtual void FixupKey(EntityEntry entry)
    {
      ...
      {
        throw new ArgumentException(System.Data.Entity.Resources.Strings.ObjectStateManager_ChangeStateFromAddedWithNullKeyIsInvalid, (Exception) ex);
      }
      EntityEntry entityEntry = this.FindEntityEntry(key);
      if (entityEntry != null)
        key = entityEntry.IsKeyEntry ? entityEntry.EntityKey : throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_CannotFixUpKeyToExistingValues((object) entry.WrappedEntity.IdentityType.FullName));
      ...
      if (!keyRelationships)
        return;
      this.FixupReferencesByForeignKeys(entry);
    }
    ...
    private void ResetEntityKey(EntityEntry entry, EntityKey value)
    {
        ...
        if (!value.Equals(entityKey))
        {
          try
          {
            ...
            if (wrappedEntity.EntityKey != value)
              throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityKey_DoesntMatchKeyOnEntity((object) wrappedEntity.Entity.GetType().FullName));
          }
          finally
          {
      ...
      throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_AcceptChangesEntityKeyIsNotValid);
    }
      ...
      EntityState entityState)
    {
      ...
      try
      {
        ...
        if (entityEntry == null)
        {
          ...
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_NoEntryExistsForObject((object) entity.GetType().FullName));
        }
      ...
      EntityState relationshipState)
    {
      ...
      return relationshipState != EntityState.Detached ? (ObjectStateEntry) relationship : (ObjectStateEntry) null;
    }
    ...
    private EntityEntry GetEntityEntryByObjectOrEntityKey(object o)
    {
      ...
      if (entityEntry == null)
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_NoEntryExistsForObject((object) o.GetType().FullName));
      return !entityEntry.IsKeyEntry ? entityEntry : throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_CannotChangeRelationshipStateKeyEntry);
    }
    ...
    public virtual ObjectStateEntry GetObjectStateEntry(EntityKey key)
    {
      ...
      if (!this.TryGetObjectStateEntry(key, out entry))
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_NoEntryExistForEntityKey);
      return entry;
    }
    ...
    internal virtual EntityEntry GetEntityEntry(object entity) => this.FindEntityEntry(entity) ?? throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_NoEntryExistsForObject((object) entity.GetType().FullName));
      ...
      out IEntityStateEntry entry)
    {
      ...
      int num = this.TryGetObjectStateEntry(key, out entry1) ? 1 : 0;
    ...
    {
      EntityEntry entry1;
      int num = this.TryGetEntityEntry(key, out entry1) ? 1 : 0;
    ...
    {
      EntityEntry entityEntry = (EntityEntry) null;
      if (entity is IEntityWithKey entityWithKey)
      {
      ...
      out RelationshipManager relationshipManager)
    {
      ...
      if (entity is IEntityWithRelationships withRelationships)
      {
        ...
        if (relationshipManager == null)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelationshipManager_UnexpectedNull);
        if (relationshipManager.WrappedOwner.Entity != entity)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelationshipManager_InvalidRelationshipManagerOwner);
      }
      else
      {
      ...
      ObjectTypeMapping mapping)
    {
      ...
      if (this._metadataMapping.ContainsKey(key))
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Mapping_CannotMapCLRTypeMultipleTimes((object) managerTypeMetadata.CdmMetadata.EdmType.FullName));
    ...
    internal virtual void Dispose() => this._isDisposed = true;

    internal virtual bool IsDisposed => this._isDisposed;
    ...
    private void DetectConflicts(IList<EntityEntry> entries)
    {
      ...
      foreach (EntityEntry entry in (IEnumerable<EntityEntry>) entries)
      {
        ...
        if (dictionary1 != null && dictionary1.Count > 0 && entry.State == EntityState.Deleted)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelatedEnd_UnableToAddRelationshipWithDeletedEntity);
        if (dictionary2 != null)
        {
          foreach (KeyValuePair<RelatedEnd, HashSet<EntityKey>> keyValuePair in dictionary2)
          {
            if ((entry.State == EntityState.Unchanged || entry.State == EntityState.Modified) && (keyValuePair.Key.IsDependentEndOfReferentialConstraint(true) && keyValuePair.Value.Count > 0))
              throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityReference_CannotChangeReferentialConstraintProperty);
            if (keyValuePair.Key is EntityReference && keyValuePair.Value.Count > 1)
              throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_ConflictingChangesOfRelationshipDetected((object) keyValuePair.Key.RelationshipNavigation.To, (object) keyValuePair.Key.RelationshipNavigation.RelationshipName));
          }
        }
        if (dictionary1 != null)
        {
          ...
          foreach (KeyValuePair<RelatedEnd, HashSet<IEntityWrapper>> keyValuePair in dictionary1)
          {
            if (keyValuePair.Key.IsForeignKey && (entry.State == EntityState.Unchanged || entry.State == EntityState.Modified) && (keyValuePair.Key.IsDependentEndOfReferentialConstraint(true) && keyValuePair.Value.Count > 0))
              throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityReference_CannotChangeReferentialConstraintProperty);
            if (keyValuePair.Key is EntityReference key6)
            {
              ...
              if (keyValuePair.Value.Count == 1)
              {
                ...
                {
                  if (this.GetPermanentKey(entry.WrappedEntity, (RelatedEnd) key6, entityTo) != source.First<EntityKey>())
                    throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectStateManager_ConflictingChangesOfRelationshipDetected((object) key6.RelationshipNavigation.To, (object) key6.RelationshipNavigation.RelationshipName));
                }
                else
                {
                  Dictionary<RelatedEnd, HashSet<EntityKey>> dictionary3;
                ...
                if (entityEntry != null && (entityEntry.State == EntityState.Unchanged || entityEntry.State == EntityState.Modified))
                {
                  ...
                  foreach (ReferentialConstraint referentialConstraint in ((AssociationType) key6.RelationMetadata).ReferentialConstraints)
                  {
                    if (referentialConstraint.ToRole == key6.FromEndMember)
                    {
      ...
      RelatedEnd relatedEnd)
    {
      ...
      return entitySet.Name != entity.EntityKey.EntitySetName ? entitySet : relationshipSet.AssociationSetEnds[1].EntitySet;
    }
      ...
      bool isForeignKeyChange)
    {
      ...
      {
        if (relatedEnd.GetOtherEndOfRelationship(entityToAdd) is EntityReference endOfRelationship2 && this.IsReparentingReference(entityToAdd, endOfRelationship2))
          this.TransactionManager.EntityBeingReparented = endOfRelationship2.GetDependentEndOfReferentialConstraint(endOfRelationship2.ReferenceValue.Entity);
      }
      else if (relatedEnd.IsDependentEndOfReferentialConstraint(false) && relatedEnd is EntityReference reference2 && this.IsReparentingReference(wrappedOwner, reference2))
        this.TransactionManager.EntityBeingReparented = reference2.GetDependentEndOfReferentialConstraint(reference2.ReferenceValue.Entity);
      try
      {
      ...
      foreach (EntityEntry entry1 in (IEnumerable<EntityEntry>) entries)
      {
        if (entry1.State != EntityState.Detached && entry1.State != EntityState.Deleted && !entry1.IsKeyEntry)
        {
          foreach (RelatedEnd relationship in entry1.WrappedEntity.RelationshipManager.Relationships)
          {
            HashSet<EntityKey> entityKeySet = (HashSet<EntityKey>) null;
            Dictionary<RelatedEnd, HashSet<EntityKey>> dictionary1;
            if (relationship is EntityReference reference4 && transactionManager.DeletedRelationshipsByForeignKey.TryGetValue(entry1.WrappedEntity, out dictionary1))
              dictionary1.TryGetValue((RelatedEnd) reference4, out entityKeySet);
            ...
            if (entityKeySet != null)
            {
              foreach (EntityKey key in entityKeySet)
              {
                ...
                else if (reference4 != null && reference4.ReferenceValue != NullEntityWrapper.NullWrapper && (reference4.ReferenceValue.EntityKey.IsTemporary && this.TryGetEntityEntry(reference4.ReferenceValue.EntityKey, out entry2)) && entry2.WrappedEntity.Entity != null)
                {
                ...
                if (entityWrapper != null)
                {
                  ...
                  if (reference4 != null && this.IsReparentingReference(entry1.WrappedEntity, reference4))
                    this.TransactionManager.EntityBeingReparented = reference4.GetDependentEndOfReferentialConstraint(reference4.ReferenceValue.Entity);
                  try
                  {
                ...
                if (reference4 != null && reference4.IsForeignKey && reference4.IsDependentEndOfReferentialConstraint(false))
                  reference4.SetCachedForeignKey(ForeignKeyFactory.CreateKeyFromForeignKeyValues(entry1, (RelatedEnd) reference4), entry1);
              }
            }
      ...
      HashSet<IEntityWrapper> entitiesToDelete)
    {
      ...
      if (!flag)
        return true;
      return flag && !this.HasAddedReference(entity, relatedEnd as EntityReference);
    }
    ...
    internal virtual EntityKey CreateEntityKey(EntitySet entitySet, object entity)
    {
      ...
      for (int index = 0; index < keyMembers.Count; ++index)
      {
        ...
        if (clayerMemberName < 0)
          throw new ArgumentException(System.Data.Entity.Resources.Strings.ObjectStateManager_EntityTypeDoesnotMatchtoEntitySetType((object) entity.GetType().FullName, (object) entitySet.Name), nameof (entity));
        compositeKeyValues[index] = managerTypeMetadata.Member(clayerMemberName).GetValue(entity);
        if (compositeKeyValues[index] == null)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityKey_NullKeyValue((object) name, (object) entitySet.ElementType.Name));
      }
      return compositeKeyValues.Length == 1 ? new EntityKey((EntitySetBase) entitySet, compositeKeyValues[0]) : new EntityKey((EntitySetBase) entitySet, compositeKeyValues);
    }
