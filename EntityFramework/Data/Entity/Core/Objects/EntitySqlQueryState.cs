...
namespace System.Data.Entity.Core.Objects
{
  internal sealed class EntitySqlQueryState : ObjectQueryState
  {
    ...
    internal EntitySqlQueryState(
      ...
      Span span)
      : this(elementType, commandText, (DbExpression) null, allowsLimit, context, parameters, span)
    {
    }
    ...
    internal bool AllowsLimitSubclause => this._allowsLimit;
    ...
    protected override TypeUsage GetResultType() => this.Parse().ResultType;

    internal override ObjectQueryState Include<TElementType>(
      ObjectQuery<TElementType> sourceQuery,
      string includePath)
    {
      ObjectQueryState other = (ObjectQueryState) new EntitySqlQueryState(this.ElementType, this._queryText, this._queryExpression, this._allowsLimit, this.ObjectContext, ObjectParameterCollection.DeepCopy(this.Parameters), Span.IncludeIn(this.Span, includePath));
      ...
      MergeOption? forMergeOption)
    {
      ...
      if (queryExecutionPlan1 == null)
      {
        queryExecutionPlan1 = this._objectQueryExecutionPlanFactory.Prepare(this.ObjectContext, DbQueryCommandTree.FromValidExpression(this.ObjectContext.MetadataWorkspace, DataSpace.CSpace, this.Parse(), true, false), this.ElementType, mergeOption, this.EffectiveStreamingBehavior, this.Span, (IEnumerable<Tuple<ObjectParameter, QueryParameterExpression>>) null, DbExpressionBuilder.AliasGenerator);
        if (key != null)
        {
