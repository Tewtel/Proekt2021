...
namespace System.Data.Entity.Core.Objects
{
    ...
    IDbAsyncEnumerable
  {
    ...
    private static bool IsLinqQuery(ObjectQuery query) => query.QueryState is ELinqQueryState;
    ...
    internal ObjectQuery(EntitySetBase entitySet, ObjectContext context, MergeOption mergeOption)
      : this((ObjectQueryState) new EntitySqlQueryState(typeof (T), ObjectQuery<T>.BuildScanEntitySetEsql(entitySet), (DbExpression) entitySet.Scan(), false, context, (ObjectParameterCollection) null, (Span) null))
    {
      EntityUtil.CheckArgumentMergeOption(mergeOption);
      this.QueryState.UserSpecifiedMergeOption = new MergeOption?(mergeOption);
      context.MetadataWorkspace.ImplicitLoadAssemblyForType(typeof (T), Assembly.GetCallingAssembly());
    ...
    private static string BuildScanEntitySetEsql(EntitySetBase entitySet) => string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0}.{1}", (object) EntityUtil.QuoteIdentifier(entitySet.EntityContainer.Name), (object) EntityUtil.QuoteIdentifier(entitySet.Name));
    ...
    public string Name
    {
      ...
      set
      {
        ...
        this._name = ObjectParameter.ValidateParameterName(value) ? value : throw new ArgumentException(System.Data.Entity.Resources.Strings.ObjectQuery_InvalidQueryName((object) value), nameof (value));
      }
    }
    ...
    /// </returns>
    public Task<ObjectResult<T>> ExecuteAsync(MergeOption mergeOption) => this.ExecuteAsync(mergeOption, CancellationToken.None);
    ...
    /// </returns>
    public ObjectQuery<T> Distinct() => ObjectQuery<T>.IsLinqQuery((ObjectQuery) this) ? (ObjectQuery<T>) this.Distinct<T>() : new ObjectQuery<T>(EntitySqlQueryBuilder.Distinct(this.QueryState));
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<ObjectQuery<T>>(query, nameof (query));
      return ObjectQuery<T>.IsLinqQuery((ObjectQuery) this) || ObjectQuery<T>.IsLinqQuery((ObjectQuery) query) ? (ObjectQuery<T>) this.Except<T>((IEnumerable<T>) query) : new ObjectQuery<T>(EntitySqlQueryBuilder.Except(this.QueryState, query.QueryState));
    }
    ...
    {
      System.Data.Entity.Utilities.Check.NotEmpty(keys, nameof (keys));
      System.Data.Entity.Utilities.Check.NotEmpty(projection, nameof (projection));
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<ObjectQuery<T>>(query, nameof (query));
      return ObjectQuery<T>.IsLinqQuery((ObjectQuery) this) || ObjectQuery<T>.IsLinqQuery((ObjectQuery) query) ? (ObjectQuery<T>) this.Intersect<T>((IEnumerable<T>) query) : new ObjectQuery<T>(EntitySqlQueryBuilder.Intersect(this.QueryState, query.QueryState));
    }
    ...
    public ObjectQuery<TResultType> OfType<TResultType>()
    {
      ...
      if (!this.QueryState.ObjectContext.MetadataWorkspace.GetItemCollection(DataSpace.OSpace).TryGetType(type1.Name, type1.NestingNamespace() ?? string.Empty, out type2) || !Helper.IsEntityType(type2) && !Helper.IsComplexType(type2))
        throw new EntitySqlException(System.Data.Entity.Resources.Strings.ObjectQuery_QueryBuilder_InvalidResultType((object) typeof (TResultType).FullName));
    ...
    {
      System.Data.Entity.Utilities.Check.NotEmpty(keys, nameof (keys));
      System.Data.Entity.Utilities.Check.NotEmpty(count, nameof (count));
      System.Data.Entity.Utilities.Check.NotNull<ObjectParameter[]>(parameters, nameof (parameters));
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<ObjectQuery<T>>(query, nameof (query));
      return ObjectQuery<T>.IsLinqQuery((ObjectQuery) this) || ObjectQuery<T>.IsLinqQuery((ObjectQuery) query) ? (ObjectQuery<T>) this.Union<T>((IEnumerable<T>) query) : new ObjectQuery<T>(EntitySqlQueryBuilder.Union(this.QueryState, query.QueryState));
    }
    ...
    {
      System.Data.Entity.Utilities.Check.NotEmpty(predicate, nameof (predicate));
      System.Data.Entity.Utilities.Check.NotNull<ObjectParameter[]>(parameters, nameof (parameters));
    ...
    internal override IEnumerator GetEnumeratorInternal() => (IEnumerator) ((IEnumerable<T>) this).GetEnumerator();

    internal override IDbAsyncEnumerator GetAsyncEnumeratorInternal() => (IDbAsyncEnumerator) ((IDbAsyncEnumerable<T>) this).GetAsyncEnumerator();

    internal override IList GetIListSourceListInternal() => ((IListSource) this.GetResults(new MergeOption?())).GetList();

    internal override ObjectResult ExecuteInternal(MergeOption mergeOption) => (ObjectResult) this.GetResults(new MergeOption?(mergeOption));
    ...
    internal override Expression GetExpression()
    {
      ...
      MergeOption? specifiedMergeOption = this.QueryState.UserSpecifiedMergeOption;
      if (specifiedMergeOption.HasValue)
      {
        Expression instance = TypeSystem.EnsureType(expression, typeof (ObjectQuery<T>));
        MethodInfo mergeAsMethod = ObjectQuery<T>.MergeAsMethod;
        Expression[] expressionArray = new Expression[1];
        specifiedMergeOption = this.QueryState.UserSpecifiedMergeOption;
        expressionArray[0] = (Expression) Expression.Constant((object) specifiedMergeOption.Value);
        expression = (Expression) Expression.Call(instance, mergeAsMethod, expressionArray);
      }
    ...
    internal ObjectQuery<T> MergeAs(MergeOption mergeOption) => throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ELinq_MethodNotDirectlyCallable);

    internal ObjectQuery<T> IncludeSpan(Span span) => throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ELinq_MethodNotDirectlyCallable);
    ...
    private async Task<ObjectResult<T>> GetResultsAsync(
      ...
      CancellationToken cancellationToken)
    {
      ObjectQuery<T> objectQuery = this;
      MergeOption mergeOption = forMergeOption.HasValue ? forMergeOption.Value : objectQuery.QueryState.EffectiveMergeOption;
      if (mergeOption != MergeOption.NoTracking)
        objectQuery.QueryState.ObjectContext.AsyncMonitor.Enter();
      ...
      {
        if (mergeOption != MergeOption.NoTracking)
          objectQuery.QueryState.ObjectContext.AsyncMonitor.Exit();
      }
      return objectResult;
    }
  }
