...
namespace System.Data.Entity.Core.Objects.ELinq
{
  internal class LinqExpressionNormalizer : EntityExpressionVisitor
  {
    ...
    internal override Expression VisitMethodCall(MethodCallExpression m)
    {
      ...
      if (m.Method.IsStatic)
      {
        ...
        if (m.Method.Name == "Equals" && m.Arguments.Count > 1)
          return (Expression) Expression.Equal(m.Arguments[0], m.Arguments[1], false, m.Method);
        if (m.Method.Name == "CompareString" && (m.Method.DeclaringType.FullName == "Microsoft.VisualBasic.CompilerServices.Operators" || m.Method.DeclaringType.FullName == "Microsoft.VisualBasic.CompilerServices.EmbeddedOperators") || m.Method.Name == "Compare" && m.Arguments.Count > 1 && m.Method.ReturnType == typeof (int))
    ...
    {
      Expression key = (Expression) Expression.Condition((Expression) LinqExpressionNormalizer.CreateRelationalOperator(ExpressionType.Equal, left, right), (Expression) Expression.Constant((object) 0), (Expression) Expression.Condition((Expression) LinqExpressionNormalizer.CreateRelationalOperator(ExpressionType.GreaterThan, left, right), (Expression) Expression.Constant((object) 1), (Expression) Expression.Constant((object) -1)));
      this._patterns[key] = (LinqExpressionNormalizer.Pattern) new LinqExpressionNormalizer.ComparePattern(left, right);
      return key;
    }
    ...
    private sealed class ComparePattern : LinqExpressionNormalizer.Pattern
    {
      ...
      internal override LinqExpressionNormalizer.PatternKind Kind => LinqExpressionNormalizer.PatternKind.Compare;
    }
  }
}
