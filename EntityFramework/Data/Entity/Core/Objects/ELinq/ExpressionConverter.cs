...
namespace System.Data.Entity.Core.Objects.ELinq
{
  internal sealed class ExpressionConverter
  {
    ...
    private EdmItemCollection EdmItemCollection => (EdmItemCollection) this._funcletizer.RootContext.MetadataWorkspace.GetItemCollection(DataSpace.CSpace, true);

    internal DbProviderManifest ProviderManifest => ((StoreItemCollection) this._funcletizer.RootContext.MetadataWorkspace.GetItemCollection(DataSpace.SSpace)).ProviderManifest;

    internal IEnumerable<Tuple<ObjectParameter, QueryParameterExpression>> GetParameters() => this._parameters != null ? (IEnumerable<Tuple<ObjectParameter, QueryParameterExpression>>) this._parameters : (IEnumerable<Tuple<ObjectParameter, QueryParameterExpression>>) null;

    internal MergeOption? PropagatedMergeOption => this._mergeOption;

    internal Span PropagatedSpan => this._span;

    internal Func<bool> RecompileRequired => this._recompileRequired;

    internal int IgnoreInclude
    {
      get => this._ignoreInclude;
      set => this._ignoreInclude = value;
    }

    internal AliasGenerator AliasGenerator => this._aliasGenerator;
    ...
    internal static bool CanFuncletizePropertyInfo(PropertyInfo propertyInfo) => ExpressionConverter.MemberAccessTranslator.CanFuncletizePropertyInfo(propertyInfo);

    internal bool CanIncludeSpanInfo() => this._ignoreInclude == 0;
    ...
    internal void ValidateInitializerMetadata(InitializerMetadata metadata)
    {
      ...
      if (this._initializers != null && this._initializers.TryGetValue(metadata.ClrType, out other))
      {
        if (!metadata.Equals(other))
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedHeterogeneousInitializers((object) ExpressionConverter.DescribeClrType(metadata.ClrType)));
      }
      else
      {
    ...
    private bool IsQueryRoot(Expression Expression) => this._expression == Expression;
    ...
    private void UnifySpanMappings(DbExpression left, DbExpression right, DbExpression to)
    {
      ...
      int num = this.TryGetSpan(left, out span1) ? 1 : 0;
      bool span3 = this.TryGetSpan(right, out span2);
      if (num == 0 && !span3)
        return;
      ...
      bool preserveCastForDateTime)
    {
      ...
      if (fromClrType != (Type) null && TypeSystem.GetNonNullableType(fromClrType).IsEnum && toClrType == typeof (Enum))
        return (TypeUsage) null;
      TypeUsage type;
      return this.TryGetValueLayerType(toClrType, out type) && ExpressionConverter.CanOmitCast(fromType, type, preserveCastForDateTime) ? (TypeUsage) null : ExpressionConverter.ValidateAndAdjustCastTypes(type, fromType, toClrType, fromClrType);
    }
      ...
      Type fromClrType)
    {
      if (toType == null || !TypeSemantics.IsScalarType(toType) || !TypeSemantics.IsScalarType(fromType))
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedCast((object) ExpressionConverter.DescribeClrType(fromClrType), (object) ExpressionConverter.DescribeClrType(toClrType)));
      ...
      if (Helper.AsPrimitive(toType.EdmType).PrimitiveTypeKind == PrimitiveTypeKind.Decimal)
      {
        switch (primitiveTypeKind)
        {
          ...
          case PrimitiveTypeKind.Int64:
            ...
            break;
          default:
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedCastToDecimal);
        }
      }
      return toType;
    }
      ...
      bool preserveCastForDateTime)
    {
      ...
      if (ExpressionConverter.TypeUsageEquals(fromType, toType))
        return true;
      return flag ? fromType.EdmType.EdmEquals((MetadataItem) toType.EdmType) : TypeSemantics.IsSubTypeOf(fromType, toType);
    }
    ...
    {
      TypeUsage type;
      return this.TryGetValueLayerType(toClrType, out type) && (TypeSemantics.IsEntityType(type) || TypeSemantics.IsComplexType(type)) ? type : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedIsOrAs((object) operationType, (object) ExpressionConverter.DescribeClrType(fromClrType), (object) ExpressionConverter.DescribeClrType(toClrType)));
    }
      ...
      Type fromClrType)
    {
      ...
      return castTargetType == null ? source : (DbExpression) source.CastTo(castTargetType);
    }
    ...
    {
      if (input.ExpressionKind == DbExpressionKind.Project && !this.TryGetSpan(input, out Span _))
      {
    ...
    private Expression GetLambdaExpression(Expression argument)
    {
      ...
      if (ExpressionType.Call == argument.NodeType)
      {
        if (typeof (Expression).IsAssignableFrom(argument.Type))
          return this.GetLambdaExpression(Expression.Lambda<Func<Expression>>(argument).Compile()());
      }
      else if (ExpressionType.Invoke == argument.NodeType && typeof (Expression).IsAssignableFrom(argument.Type))
        return this.GetLambdaExpression(Expression.Lambda<Func<Expression>>(argument).Compile()());
      throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ADP_InternalProviderError((object) 1025));
    }

    private DbExpression TranslateSet(Expression linq) => this.NormalizeSetSource(this.TranslateExpression(linq));
    ...
    private DbExpression AlignTypes(DbExpression cqt, Type toClrType)
    {
      ...
      return castTargetType != null ? (DbExpression) cqt.CastTo(castTargetType) : cqt;
    }
    ...
    private void CheckInitializerType(Type type)
    {
      ...
      if (this._funcletizer.RootContext.Perspective.TryGetType(type, out outTypeUsage))
      {
        ...
        if (BuiltInTypeKind.EntityType == builtInTypeKind || BuiltInTypeKind.ComplexType == builtInTypeKind)
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedNominalType((object) outTypeUsage.EdmType.FullName));
      }
      if (TypeSystem.IsSequenceType(type))
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedEnumerableType((object) ExpressionConverter.DescribeClrType(type)));
    }
    ...
    private TypeUsage GetValueLayerType(Type linqType)
    {
      ...
      if (!this.TryGetValueLayerType(linqType, out type))
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedType((object) linqType));
      return type;
    }
      ...
      bool isNullComparison)
    {
      ...
      if (memberPath == null)
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedComparison((object) ExpressionConverter.DescribeClrType(clrType)));
      StringBuilder stringBuilder = new StringBuilder();
      foreach (EdmMember edmMember in memberPath)
        stringBuilder.Append(System.Data.Entity.Resources.Strings.ELinq_UnsupportedRowMemberComparison((object) edmMember.Name));
      stringBuilder.Append(System.Data.Entity.Resources.Strings.ELinq_UnsupportedRowTypeComparison((object) ExpressionConverter.DescribeClrType(clrType)));
      throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedRowComparison((object) stringBuilder.ToString()));
    }
      ...
      bool isNullComparison)
    {
      foreach (EdmMember property in rowType.Properties)
      {
        ...
        ExpressionConverter.VerifyTypeSupportedForComparison(clrType, property.TypeUsage, memberPath, isNullComparison);
        memberPath.Pop();
      }
    }
    ...
    internal static string DescribeClrType(Type clrType)
    {
      if (ExpressionConverter.IsCSharpGeneratedClass(clrType.Name, "DisplayClass") || ExpressionConverter.IsVBGeneratedClass(clrType.Name, "Closure"))
        return System.Data.Entity.Resources.Strings.ELinq_ClosureType;
      return ExpressionConverter.IsCSharpGeneratedClass(clrType.Name, "AnonymousType") || ExpressionConverter.IsVBGeneratedClass(clrType.Name, "AnonymousType") ? System.Data.Entity.Resources.Strings.ELinq_AnonymousType : clrType.FullName;
    }

    private static bool IsCSharpGeneratedClass(string typeName, string pattern) => typeName.Contains("<>") && typeName.Contains("__") && typeName.Contains(pattern);

    private static bool IsVBGeneratedClass(string typeName, string pattern) => typeName.Contains("_") && typeName.Contains("$") && typeName.Contains(pattern);

    private static DbExpression CreateIsNullExpression(
      DbExpression operand,
      Type operandClrType)
    {
      ExpressionConverter.VerifyTypeSupportedForComparison(operandClrType, operand.ResultType, (Stack<EdmMember>) null, true);
    ...
    private DbExpression CreateEqualsExpression(
      ...
      Type rightClrType)
    {
      ExpressionConverter.VerifyTypeSupportedForComparison(leftClrType, left.ResultType, (Stack<EdmMember>) null, false);
      ExpressionConverter.VerifyTypeSupportedForComparison(rightClrType, right.ResultType, (Stack<EdmMember>) null, false);
      TypeUsage resultType1 = left.ResultType;
      TypeUsage resultType2 = right.ResultType;
      if (resultType1.EdmType.BuiltInTypeKind == BuiltInTypeKind.RefType && resultType2.EdmType.BuiltInTypeKind == BuiltInTypeKind.RefType && !TypeSemantics.TryGetCommonType(resultType1, resultType2, out TypeUsage _))
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedRefComparison((object) (left.ResultType.EdmType as RefType).ElementType.FullName, (object) (right.ResultType.EdmType as RefType).ElementType.FullName));
      ...
      ExpressionConverter.EqualsPattern pattern)
    {
      ...
      return !this._funcletizer.RootContext.ContextOptions.UseCSharpNullComparisonBehavior ? this.ImplementEquality(left, right, pattern) : this.ImplementEquality(left, right, ExpressionConverter.EqualsPattern.Store);
    }
      ...
      ExpressionConverter.EqualsPattern pattern)
    {
      switch (pattern)
      {
        ...
        case ExpressionConverter.EqualsPattern.PositiveNullEqualityComposable:
          return !this._funcletizer.RootContext.ContextOptions.UseCSharpNullComparisonBehavior ? (DbExpression) constant.Equal(unknown) : (DbExpression) constant.Equal(unknown).And((DbExpression) unknown.IsNull().Not());
        default:
    ...
    {
      int num = this.ProviderManifest.SupportsEscapingLikeArgument(out char _) ? 1 : 0;
      Expression linq1 = call.Arguments[0];
      Expression linq2 = call.Arguments[1];
      Expression linq3 = call.Arguments.Count > 2 ? call.Arguments[2] : (Expression) null;
      if (num == 0 && linq3 != null)
        throw new ProviderIncompatibleException(System.Data.Entity.Resources.Strings.ProviderDoesNotSupportEscapingLikeArgument);
      DbExpression pattern = this.TranslateExpression(linq2);
      DbExpression escape = linq3 != null ? this.TranslateExpression(linq3) : (DbExpression) null;
      DbExpression dbExpression = this.TranslateExpression(linq1);
      return linq3 == null ? (DbExpression) dbExpression.Like(pattern) : (DbExpression) dbExpression.Like(pattern, escape);
    }
      ...
      Func<ExpressionConverter, MethodCallExpression, DbExpression, DbExpression, DbExpression> defaultTranslator)
    {
      ...
      QueryParameterExpression parameterExpression1 = linq1 as QueryParameterExpression;
      if (flag1 && parameterExpression1 != null)
      {
        flag2 = true;
        MethodInfo method1 = typeof (ExpressionConverter).GetMethod("PreparePattern", BindingFlags.Static | BindingFlags.NonPublic);
        ParameterExpression parameterExpression2 = Expression.Parameter(typeof (string), "input");
        ConstantExpression constantExpression1 = Expression.Constant((object) insertPercentAtStart);
        ConstantExpression constantExpression2 = Expression.Constant((object) insertPercentAtEnd);
        ConstantExpression constantExpression3 = Expression.Constant((object) this.ProviderManifest);
        Expression<Func<string, Tuple<string, bool>>> method2 = (Expression<Func<string, Tuple<string, bool>>>) (parameterExpression => Expression.Call(method1, (Expression) parameterExpression2, (Expression) constantExpression1, (Expression) constantExpression2, (Expression) constantExpression3));
        linq1 = (Expression) parameterExpression1.EscapeParameterForLike(method2);
      }
      ...
      if (flag1 && pattern.ExpressionKind == DbExpressionKind.Constant)
      {
        ...
        Tuple<string, bool> tuple = ExpressionConverter.PreparePattern((string) constantExpression.Value, insertPercentAtStart, insertPercentAtEnd, this.ProviderManifest);
    ...
    private static Tuple<string, bool> PreparePattern(
      string patternValue,
      bool insertPercentAtStart,
      bool insertPercentAtEnd,
      DbProviderManifest providerManifest)
    {
      if (patternValue == null)
        return new Tuple<string, bool>((string) null, false);
      string str = providerManifest.EscapeLikeArgument(patternValue);
      if (str == null)
        throw new ProviderIncompatibleException(System.Data.Entity.Resources.Strings.ProviderEscapeLikeArgumentReturnedNull);
      bool flag = patternValue != str;
      StringBuilder stringBuilder = new StringBuilder();
    ...
    private static void ThrowUnresolvableFunction(Expression Expression)
    {
      if (Expression.NodeType == ExpressionType.Call)
      {
        ...
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnresolvableFunctionForMethod((object) method, (object) method.DeclaringType));
      }
      MemberInfo memberInfo = Expression.NodeType == ExpressionType.MemberAccess ? TypeSystem.PropertyOrField(((MemberExpression) Expression).Member, out string _, out Type _) : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnresolvableFunctionForExpression((object) Expression.NodeType));
      throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnresolvableFunctionForMember((object) memberInfo, (object) memberInfo.DeclaringType));
    }
    ...
    private static void ThrowUnresolvableFunctionOverload(Expression Expression, bool isAmbiguous)
    {
      if (Expression.NodeType == ExpressionType.Call)
      {
        MethodInfo method = ((MethodCallExpression) Expression).Method;
        if (isAmbiguous)
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnresolvableFunctionForMethodAmbiguousMatch((object) method, (object) method.DeclaringType));
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnresolvableFunctionForMethodNotFound((object) method, (object) method.DeclaringType));
      }
      MemberInfo memberInfo = Expression.NodeType == ExpressionType.MemberAccess ? TypeSystem.PropertyOrField(((MemberExpression) Expression).Member, out string _, out Type _) : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnresolvableStoreFunctionForExpression((object) Expression.NodeType));
      throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnresolvableStoreFunctionForMember((object) memberInfo, (object) memberInfo.DeclaringType));
    }
    ...
    private class ParameterReferenceRemover : DefaultExpressionVisitor
    {
      ...
      private ParameterReferenceRemover(ObjectParameterCollection availableParams) => this.objectParameters = availableParams;
      ...
      public override DbExpression Visit(DbParameterReferenceExpression expression)
      {
        ...
        return objectParameter.Value == null ? (DbExpression) expression.ResultType.Null() : (DbExpression) expression.ResultType.Constant(objectParameter.Value);
      }
    }
    ...
    internal sealed class MethodCallTranslator : 
      ExpressionConverter.TypedTranslator<MethodCallExpression>
    {
        ...
        MethodCallExpression linq)
      {
        ...
        {
          case "Contains":
            if (((IEnumerable<ParameterInfo>) linq.Method.GetParameters()).Count<ParameterInfo>() == 1 && linq.Method.ReturnType.Equals(typeof (bool)) && linq.Method.IsImplementationOfGenericInterfaceMethod(typeof (ICollection<>), out Type[] _))
      ...
      private static IEnumerable<ExpressionConverter.MethodCallTranslator.CallTranslator> GetCallTranslators() => (IEnumerable<ExpressionConverter.MethodCallTranslator.CallTranslator>) new ExpressionConverter.MethodCallTranslator.CallTranslator[23]
      {
        ...
        (ExpressionConverter.MethodCallTranslator.CallTranslator) new ExpressionConverter.MethodCallTranslator.LikeFunctionTranslator(),
        ...
        (ExpressionConverter.MethodCallTranslator.CallTranslator) new ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator(),
      ...
      internal sealed class HierarchyIdMethodCallTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
        ...
        {
          yield return ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetStaticMethod<HierarchyId>((Expression<Func<HierarchyId>>) (() => HierarchyId.GetRoot()));
          yield return ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetStaticMethod<HierarchyId>((Expression<Func<HierarchyId>>) (() => HierarchyId.Parse(default (string))));
          yield return ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, HierarchyId>((Expression<Func<HierarchyId, HierarchyId>>) (h => h.GetAncestor(0)));
          yield return ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, HierarchyId>((Expression<Func<HierarchyId, HierarchyId>>) (h => h.GetDescendant(default (HierarchyId), default (HierarchyId))));
          yield return ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, short>((Expression<Func<HierarchyId, short>>) (h => h.GetLevel()));
          yield return ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, bool>((Expression<Func<HierarchyId, bool>>) (h => h.IsDescendantOf(default (HierarchyId))));
          yield return ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, HierarchyId>((Expression<Func<HierarchyId, HierarchyId>>) (h => h.GetReparentedValue(default (HierarchyId), default (HierarchyId))));
        }
        ...
        private static Dictionary<MethodInfo, string> GetRenamedMethodFunctions()
        {
          ...
          dictionary.Add(ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetStaticMethod<HierarchyId>((Expression<Func<HierarchyId>>) (() => HierarchyId.GetRoot())), "HierarchyIdGetRoot");
          dictionary.Add(ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetStaticMethod<HierarchyId>((Expression<Func<HierarchyId>>) (() => HierarchyId.Parse(default (string)))), "HierarchyIdParse");
          dictionary.Add(ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, HierarchyId>((Expression<Func<HierarchyId, HierarchyId>>) (h => h.GetAncestor(0))), "GetAncestor");
          dictionary.Add(ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, HierarchyId>((Expression<Func<HierarchyId, HierarchyId>>) (h => h.GetDescendant(default (HierarchyId), default (HierarchyId)))), "GetDescendant");
          dictionary.Add(ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, short>((Expression<Func<HierarchyId, short>>) (h => h.GetLevel())), "GetLevel");
          dictionary.Add(ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, bool>((Expression<Func<HierarchyId, bool>>) (h => h.IsDescendantOf(default (HierarchyId)))), "IsDescendantOf");
          dictionary.Add(ExpressionConverter.MethodCallTranslator.HierarchyIdMethodCallTranslator.GetInstanceMethod<HierarchyId, HierarchyId>((Expression<Func<HierarchyId, HierarchyId>>) (h => h.GetReparentedValue(default (HierarchyId), default (HierarchyId)))), "GetReparentedValue");
          return dictionary;
        }
          ...
          MethodCallExpression call)
        {
          ...
          return (DbExpression) parent.TranslateIntoCanonicalFunction(name, (Expression) call, array);
        }
      }
      ...
      internal abstract class CallTranslator
      {
        ...
        protected CallTranslator(params MethodInfo[] methods) => this._methods = (IEnumerable<MethodInfo>) methods;

        protected CallTranslator(IEnumerable<MethodInfo> methods) => this._methods = methods;

        internal IEnumerable<MethodInfo> Methods => this._methods;
      ...
      private abstract class ObjectQueryCallTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
        ...
        internal static bool IsCandidateMethod(MethodInfo method)
        {
          ...
          return (method.IsPublic || method.IsAssembly && (method.Name == "MergeAs" || method.Name == "IncludeSpan")) && ((Type) null != declaringType && declaringType.IsGenericType()) && typeof (ObjectQuery<>) == declaringType.GetGenericTypeDefinition();
        }
      ...
      private abstract class ObjectQueryBuilderCallTranslator : 
        ExpressionConverter.MethodCallTranslator.ObjectQueryCallTranslator
      {
      ...
      private sealed class ObjectQueryBuilderUnionTranslator : 
        ExpressionConverter.MethodCallTranslator.ObjectQueryBuilderCallTranslator
      {
      ...
      private sealed class ObjectQueryBuilderToListTranslator : 
        ExpressionConverter.MethodCallTranslator.ObjectQueryBuilderCallTranslator
      {
      ...
      private sealed class ObjectQueryIncludeTranslator : 
        ExpressionConverter.MethodCallTranslator.ObjectQueryCallTranslator
      {
          ...
          MethodCallExpression call)
        {
          ...
          string pathToInclude = dbExpression.ExpressionKind == DbExpressionKind.Constant ? (string) ((DbConstantExpression) dbExpression).Value : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedInclude);
      ...
      private sealed class ObjectQueryMergeAsTranslator : 
        ExpressionConverter.MethodCallTranslator.ObjectQueryCallTranslator
      {
          ...
          MethodCallExpression call)
        {
          if (call.Arguments[0].NodeType != ExpressionType.Constant)
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedMergeAs);
      ...
      private sealed class ObjectQueryIncludeSpanTranslator : 
        ExpressionConverter.MethodCallTranslator.ObjectQueryCallTranslator
      {
      ...
      internal sealed class DefaultTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
        internal override DbExpression Translate(
          ExpressionConverter parent,
          MethodCallExpression call)
        {
          ...
          if (method.DeclaringType.Assembly().FullName == "Microsoft.VisualBasic, Version=10.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a" && method.Name == "Mid")
          {
            if (((IEnumerable<Type>) new Type[2]
            {
              ...
              throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedMethodSuggestedAlternative((object) method, (object) "System.String Mid(System.String, Int32, Int32)"));
          }
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedMethod((object) method));
        }
      ...
      private sealed class FunctionCallTranslator
      {
          ...
          DbFunctionAttribute functionAttribute)
        {
          ...
          DbExpression result = function.IsComposableAttribute ? (DbExpression) function.Invoke((IEnumerable<DbExpression>) list1) : throw new NotSupportedException(System.Data.Entity.Resources.Strings.CannotCallNoncomposableFunction((object) function.FullName));
          ...
          DbExpression argumentExpr)
        {
          ...
          return input1 != null && input1 != argumentExpr ? parent.NormalizeSetSource(input1) : parent.NormalizeSetSource(argumentExpr);
        }
          ...
          bool isElementOfCollection)
        {
          ...
          {
            case BuiltInTypeKind.CollectionType:
              Type type = clrReturnType.IsGenericType() ? clrReturnType.GetGenericTypeDefinition() : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_DbFunctionAttributedFunctionWithWrongReturnType((object) call.Method, (object) call.Method.DeclaringType));
              if (type != typeof (IEnumerable<>) && type != typeof (IQueryable<>))
                throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_DbFunctionAttributedFunctionWithWrongReturnType((object) call.Method, (object) call.Method.DeclaringType));
      ...
      internal sealed class CanonicalFunctionDefaultTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class LikeFunctionTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal abstract class AsUnicodeNonUnicodeBaseFunctionTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
          ...
          MethodCallExpression call)
        {
          ...
          switch (dbExpression.ExpressionKind)
          {
            ...
            default:
              throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedAsUnicodeAndAsNonUnicode((object) call.Method));
          }
        }
      }

      internal sealed class AsUnicodeFunctionTranslator : 
        ExpressionConverter.MethodCallTranslator.AsUnicodeNonUnicodeBaseFunctionTranslator
      {
      ...
      internal sealed class AsNonUnicodeFunctionTranslator : 
        ExpressionConverter.MethodCallTranslator.AsUnicodeNonUnicodeBaseFunctionTranslator
      {
      ...
      internal sealed class HasFlagTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
          ...
          Expression valueExpression)
        {
          ...
          if (dbExpression2.ResultType.EdmType != dbExpression1.ResultType.EdmType)
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_HasFlagArgumentAndSourceTypeMismatch((object) dbExpression1.ResultType.EdmType.Name, (object) dbExpression2.ResultType.EdmType.Name));
      ...
      internal sealed class MathTruncateTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class MathPowerTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class GuidNewGuidTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class StringContainsTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class IndexOfTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class StartsWithTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class EndsWithTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class SubstringTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class RemoveTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
          ...
          MethodCallExpression call)
        {
          ...
          if (call.Arguments.Count == 2)
          {
            ...
            DbExpression dbExpression2 = ExpressionConverter.MethodCallTranslator.RemoveTranslator.IsNonNegativeIntegerConstant(right) ? (DbExpression) left.Plus(right).Plus((DbExpression) DbExpressionBuilder.Constant((object) 1)) : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedStringRemoveCase((object) call.Method, (object) call.Method.GetParameters()[1].Name));
        ...
        private static bool IsNonNegativeIntegerConstant(DbExpression argument) => argument.ExpressionKind == DbExpressionKind.Constant && TypeSemantics.IsPrimitiveType(argument.ResultType, PrimitiveTypeKind.Int32) && (int) ((DbConstantExpression) argument).Value >= 0;
      }

      internal sealed class InsertTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class IsNullOrEmptyTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class StringConcatTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal sealed class ToStringTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
      ...
      internal abstract class TrimBaseTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
          ...
          MethodCallExpression call)
        {
          if (call.Arguments.Count != 0 && !ExpressionConverter.MethodCallTranslator.TrimBaseTranslator.IsEmptyArray(call.Arguments[0]))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedTrimStartTrimEndCase((object) call.Method));
      ...
      internal sealed class TrimTranslator : 
        ExpressionConverter.MethodCallTranslator.TrimBaseTranslator
      {
      ...
      internal sealed class TrimStartTranslator : 
        ExpressionConverter.MethodCallTranslator.TrimBaseTranslator
      {
      ...
      internal sealed class TrimEndTranslator : 
        ExpressionConverter.MethodCallTranslator.TrimBaseTranslator
      {
      ...
      internal sealed class VBCanonicalFunctionDefaultTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
        private const string s_stringsTypeFullName = "Microsoft.VisualBasic.Strings";
      ...
      internal sealed class VBCanonicalFunctionRenameTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
        private const string s_stringsTypeFullName = "Microsoft.VisualBasic.Strings";
      ...
      internal sealed class VBDatePartTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
        private const string s_dateAndTimeTypeFullName = "Microsoft.VisualBasic.DateAndTime";
        ...
        private static IEnumerable<MethodInfo> GetMethods(Assembly vbAssembly)
        {
          yield return vbAssembly.GetType("Microsoft.VisualBasic.DateAndTime").GetDeclaredMethod("DatePart", vbAssembly.GetType("Microsoft.VisualBasic.DateInterval"), typeof (DateTime), vbAssembly.GetType("Microsoft.VisualBasic.FirstDayOfWeek"), vbAssembly.GetType("Microsoft.VisualBasic.FirstWeekOfYear"));
        }
        ...
        internal override DbExpression Translate(
          ...
          MethodCallExpression call)
        {
          if (!(call.Arguments[0] is ConstantExpression constantExpression))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedVBDatePartNonConstantInterval((object) call.Method, (object) call.Method.GetParameters()[0].Name));
          ...
          if (!ExpressionConverter.MethodCallTranslator.VBDatePartTranslator._supportedIntervals.Contains(functionName))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedVBDatePartInvalidInterval((object) call.Method, (object) call.Method.GetParameters()[0].Name, (object) functionName));
      ...
      private abstract class SequenceMethodTranslator
      {
        ...
        protected SequenceMethodTranslator(params SequenceMethod[] methods) => this._methods = (IEnumerable<SequenceMethod>) methods;

        internal IEnumerable<SequenceMethod> Methods => this._methods;
      ...
      private abstract class PagingTranslator : 
        ExpressionConverter.MethodCallTranslator.UnarySequenceMethodTranslator
      {
      ...
      private sealed class TakeTranslator : ExpressionConverter.MethodCallTranslator.PagingTranslator
      {
        ...
        protected override DbExpression TranslatePagingOperator(
          ...
          DbExpression count)
        {
          return count is DbConstantExpression constantExpression && constantExpression.Value.Equals((object) 0) ? parent.Filter(operand.BindAs(parent.AliasGenerator.Next()), (DbExpression) DbExpressionBuilder.False) : parent.Limit(operand, count);
        }
      }
      ...
      private sealed class JoinTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
          ...
          MethodCallExpression call)
        {
          ...
          string rightName;
          InitializerMetadata initializerMetadata;
          int num = ExpressionConverter.MethodCallTranslator.IsTrivialRename(lambdaExpression3, parent, out leftName, out rightName, out initializerMetadata) ? 1 : 0;
          DbExpressionBinding binding1;
          DbExpression left = num != 0 ? parent.TranslateLambda(lambdaExpression1, input1, leftName, out binding1) : parent.TranslateLambda(lambdaExpression1, input1, out binding1);
          DbExpressionBinding binding2;
          DbExpression right = num != 0 ? parent.TranslateLambda(lambdaExpression2, input2, rightName, out binding2) : parent.TranslateLambda(lambdaExpression2, input2, out binding2);
          if (!TypeSemantics.IsEqualComparable(left.ResultType) || !TypeSemantics.IsEqualComparable(right.ResultType))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedKeySelector((object) call.Method.Name));
      ...
      private abstract class BinarySequenceMethodTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
      ...
      private class ConcatTranslator : 
        ExpressionConverter.MethodCallTranslator.BinarySequenceMethodTranslator
      {
      ...
      private sealed class UnionTranslator : 
        ExpressionConverter.MethodCallTranslator.BinarySequenceMethodTranslator
      {
      ...
      private sealed class IntersectTranslator : 
        ExpressionConverter.MethodCallTranslator.BinarySequenceMethodTranslator
      {
      ...
      private sealed class ExceptTranslator : 
        ExpressionConverter.MethodCallTranslator.BinarySequenceMethodTranslator
      {
      ...
      private abstract class AggregateTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
        ...
        internal override DbExpression Translate(
          ...
          MethodCallExpression call)
        {
          int num = 1 == call.Arguments.Count ? 1 : 0;
      ...
      private sealed class MaxTranslator : 
        ExpressionConverter.MethodCallTranslator.AggregateTranslator
      {
          ...
          MethodCallExpression call)
        {
          ...
          return !TypeSemantics.IsEnumerationType(returnType) ? returnType : TypeUsage.Create((EdmType) Helper.GetUnderlyingEdmTypeForEnumType(returnType.EdmType), (IEnumerable<Facet>) returnType.Facets);
        }
      }

      private sealed class MinTranslator : 
        ExpressionConverter.MethodCallTranslator.AggregateTranslator
      {
      ...
      private sealed class AverageTranslator : 
        ExpressionConverter.MethodCallTranslator.AggregateTranslator
      {
      ...
      private sealed class SumTranslator : 
        ExpressionConverter.MethodCallTranslator.AggregateTranslator
      {
      ...
      private sealed class CountTranslator : 
        ExpressionConverter.MethodCallTranslator.CountTranslatorBase
      {
      ...
      private sealed class LongCountTranslator : 
        ExpressionConverter.MethodCallTranslator.CountTranslatorBase
      {
      ...
      private abstract class UnarySequenceMethodTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
      ...
      private sealed class PassthroughTranslator : 
        ExpressionConverter.MethodCallTranslator.UnarySequenceMethodTranslator
      {
        ...
        protected override DbExpression TranslateUnary(
          ...
          MethodCallExpression call)
        {
          return TypeSemantics.IsCollectionType(operand.ResultType) ? operand : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedPassthrough((object) call.Method.Name, (object) operand.ResultType.EdmType.Name));
        }
      }

      private sealed class OfTypeTranslator : 
        ExpressionConverter.MethodCallTranslator.UnarySequenceMethodTranslator
      {
          ...
          MethodCallExpression call)
        {
          ...
          if (!parent.TryGetValueLayerType(genericArgument, out type) || !TypeSemantics.IsEntityType(type) && !TypeSemantics.IsComplexType(type))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_InvalidOfTypeResult((object) ExpressionConverter.DescribeClrType(genericArgument)));
          return parent.OfType(operand, type);
        }
      }

      private sealed class DistinctTranslator : 
        ExpressionConverter.MethodCallTranslator.UnarySequenceMethodTranslator
      {
      ...
      private sealed class AnyTranslator : 
        ExpressionConverter.MethodCallTranslator.UnarySequenceMethodTranslator
      {
      ...
      private abstract class OneLambdaTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
        ...
        internal override DbExpression Translate(
          ...
          MethodCallExpression call)
        {
          return this.Translate(parent, call, out DbExpression _, out DbExpressionBinding _, out DbExpression _);
        }
      ...
      private sealed class AnyPredicateTranslator : 
        ExpressionConverter.MethodCallTranslator.OneLambdaTranslator
      {
      ...
      private sealed class AllTranslator : 
        ExpressionConverter.MethodCallTranslator.OneLambdaTranslator
      {
      ...
      private sealed class WhereTranslator : 
        ExpressionConverter.MethodCallTranslator.OneLambdaTranslator
      {
      ...
      private sealed class SelectTranslator : 
        ExpressionConverter.MethodCallTranslator.OneLambdaTranslator
      {
      ...
      private sealed class DefaultIfEmptyTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
          ...
          Type resultType)
        {
          ...
          return defaultValue != null ? parent.TranslateExpression((Expression) Expression.Constant(defaultValue, elementType)) : (DbExpression) null;
        }
      }

      private sealed class ContainsTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
          ...
          Expression valueExpression)
        {
          ...
          if (input1.ExpressionKind == DbExpressionKind.NewInstance)
          {
            ...
            if (left == null)
              return right;
            return right == null ? left : (DbExpression) left.Or(right);
          }
      ...
      private abstract class FirstTranslatorBase : 
        ExpressionConverter.MethodCallTranslator.UnarySequenceMethodTranslator
      {
      ...
      private sealed class FirstTranslator : 
        ExpressionConverter.MethodCallTranslator.FirstTranslatorBase
      {
        ...
        {
          if (!parent.IsQueryRoot((Expression) call))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedNestedFirst);
          return base.TranslateUnary(parent, operand, call);
        }
      }

      private sealed class FirstOrDefaultTranslator : 
        ExpressionConverter.MethodCallTranslator.FirstTranslatorBase
      {
      ...
      private abstract class SingleTranslatorBase : 
        ExpressionConverter.MethodCallTranslator.FirstTranslatorBase
      {
        ...
        {
          if (!parent.IsQueryRoot((Expression) call))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedNestedSingle);
          return base.TranslateUnary(parent, operand, call);
        }
      ...
      private sealed class SingleTranslator : 
        ExpressionConverter.MethodCallTranslator.SingleTranslatorBase
      {
      ...
      private sealed class SingleOrDefaultTranslator : 
        ExpressionConverter.MethodCallTranslator.SingleTranslatorBase
      {
      ...
      private abstract class FirstPredicateTranslatorBase : 
        ExpressionConverter.MethodCallTranslator.OneLambdaTranslator
      {
      ...
      private sealed class FirstPredicateTranslator : 
        ExpressionConverter.MethodCallTranslator.FirstPredicateTranslatorBase
      {
        ...
        internal override DbExpression Translate(
          ...
          MethodCallExpression call)
        {
          return parent.IsQueryRoot((Expression) call) ? base.Translate(parent, call) : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedNestedFirst);
        }
      }

      private sealed class FirstOrDefaultPredicateTranslator : 
        ExpressionConverter.MethodCallTranslator.FirstPredicateTranslatorBase
      {
      ...
      private abstract class SinglePredicateTranslatorBase : 
        ExpressionConverter.MethodCallTranslator.FirstPredicateTranslatorBase
      {
        ...
        internal override DbExpression Translate(
          ...
          MethodCallExpression call)
        {
          return parent.IsQueryRoot((Expression) call) ? base.Translate(parent, call) : throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedNestedSingle);
        }
      ...
      private sealed class SinglePredicateTranslator : 
        ExpressionConverter.MethodCallTranslator.SinglePredicateTranslatorBase
      {
      ...
      private sealed class SingleOrDefaultPredicateTranslator : 
        ExpressionConverter.MethodCallTranslator.SinglePredicateTranslatorBase
      {
      ...
      private sealed class SelectManyTranslator : 
        ExpressionConverter.MethodCallTranslator.OneLambdaTranslator
      {
      ...
      private sealed class CastMethodTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
          ...
          MethodCallExpression call)
        {
          ...
          string varName = parent.AliasGenerator.Next();
      ...
      private sealed class GroupByTranslator : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
          ...
          SequenceMethod sequenceMethod)
        {
          ...
          if ((sequenceMethod == SequenceMethod.GroupByElementSelector ? 1 : (sequenceMethod == SequenceMethod.GroupByElementSelectorResultSelector ? 1 : 0)) != 0)
          {
          ...
          TypeUsage instanceType = TypeUsage.Create((EdmType) new RowType((IEnumerable<EdmProperty>) new List<EdmProperty>(2)
          {
            new EdmProperty("Key", dbExpressionArray[0].ResultType),
            new EdmProperty("Group", dbExpressionArray[1].ResultType)
          }, InitializerMetadata.CreateGroupingInitializer(parent.EdmItemCollection, TypeSystem.GetElementType(call.Type))));
          ...
          DbExpression result)
        {
          ...
          if (lambdaExpression != null)
          {
            ...
            result = (DbExpression) input.Project(parent.TranslateExpression(lambdaExpression.Body));
            parent._bindingContext.PopBindingScope();
            parent._bindingContext.PopBindingScope();
          }
          return result;
      ...
      private abstract class OrderByTranslatorBase : 
        ExpressionConverter.MethodCallTranslator.OneLambdaTranslator
      {
      ...
      private sealed class OrderByTranslator : 
        ExpressionConverter.MethodCallTranslator.OrderByTranslatorBase
      {
      ...
      private sealed class OrderByDescendingTranslator : 
        ExpressionConverter.MethodCallTranslator.OrderByTranslatorBase
      {
      ...
      private abstract class ThenByTranslatorBase : 
        ExpressionConverter.MethodCallTranslator.SequenceMethodTranslator
      {
        private readonly bool _ascending;
          ...
          MethodCallExpression call)
        {
          ...
          DbSortExpression dbSortExpression = DbExpressionKind.Sort == dbExpression.ExpressionKind ? (DbSortExpression) dbExpression : throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ELinq_ThenByDoesNotFollowOrderBy);
      ...
      private sealed class ThenByTranslator : 
        ExpressionConverter.MethodCallTranslator.ThenByTranslatorBase
      {
      ...
      private sealed class ThenByDescendingTranslator : 
        ExpressionConverter.MethodCallTranslator.ThenByTranslatorBase
      {
      ...
      private sealed class SpatialMethodCallTranslator : 
        ExpressionConverter.MethodCallTranslator.CallTranslator
      {
    ...
    private sealed class OrderByLifter
    {
      ...
      internal OrderByLifter(AliasGenerator aliasGenerator) => this._aliasGenerator = aliasGenerator;

      internal DbExpression Project(DbExpressionBinding input, DbExpression projection) => this.GetLifter(input.Expression).Project(input.Project(projection));

      internal DbExpression Filter(DbExpressionBinding input, DbExpression predicate) => this.GetLifter(input.Expression).Filter(input.Filter(predicate));

      internal DbExpression OfType(DbExpression argument, TypeUsage type) => this.GetLifter(argument).OfType(type);

      internal DbExpression Skip(DbExpressionBinding input, DbExpression skipCount) => this.GetLifter(input.Expression).Skip(skipCount);

      internal DbExpression Limit(DbExpression argument, DbExpression limit) => this.GetLifter(argument).Limit(limit);
      ...
      private abstract class OrderByLifterBase
      {
        ...
        protected static DbLimitExpression SubtractFromLimit(
          ...
          DbExpression minusK)
        {
          DbExpression count = ExpressionConverter.OrderByLifter.OrderByLifterBase.CombineIntegers(limit.Limit, minusK, (Func<int, int, int>) ((l, r) => r <= l ? l - r : 0));
          ...
          Func<int, int, int> combineConstants)
        {
          if (left.ExpressionKind == DbExpressionKind.Constant && right.ExpressionKind == DbExpressionKind.Constant)
          {
            ...
            if (obj1 is int && obj2 is int num2)
              return (DbExpression) left.ResultType.Constant((object) combineConstants((int) obj1, num2));
          }
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ADP_InternalProviderError((object) 1025));
        }
      ...
      private class LimitSkipLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Filter(DbFilterExpression filter) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ApplySkipOrderToSort((DbExpression) filter, this._skip);

        internal override DbExpression Project(DbProjectExpression project) => (DbExpression) project;

        internal override DbExpression Limit(DbExpression k) => this._limit.Limit.ExpressionKind == DbExpressionKind.Constant && k.ExpressionKind == DbExpressionKind.Constant ? (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.MinimumLimit((DbExpression) this._skip, this._limit, k) : (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ApplySkipOrderToSort((DbExpression) this._limit, this._skip).Limit(k);

        internal override DbExpression Skip(DbExpression k) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSkip((DbExpression) this._limit, this._skip, k);
      }
      ...
      private class LimitSortLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Filter(DbFilterExpression filter) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSort((DbExpression) filter, this._sort);

        internal override DbExpression Project(DbProjectExpression project) => (DbExpression) project;

        internal override DbExpression Limit(DbExpression k) => this._limit.Limit.ExpressionKind == DbExpressionKind.Constant && k.ExpressionKind == DbExpressionKind.Constant ? (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.MinimumLimit((DbExpression) this._sort, this._limit, k) : (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSort((DbExpression) this._limit, this._sort).Limit(k);

        internal override DbExpression Skip(DbExpression k) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ApplySortOrderToSkip((DbExpression) this._limit, this._sort, k);
      }
      ...
      private class ProjectLimitSkipLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Filter(DbFilterExpression filter) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ApplySkipOrderToSort((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ComposeFilter((DbExpression) this._skip.Limit(this._limit.Limit), this._project, filter), this._skip), this._project);

        internal override DbExpression Project(DbProjectExpression project) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ComposeProject((DbExpression) this._skip.Limit(this._limit.Limit), this._project, project);

        internal override DbExpression Limit(DbExpression k) => this._limit.Limit.ExpressionKind == DbExpressionKind.Constant && k.ExpressionKind == DbExpressionKind.Constant ? (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.MinimumLimit((DbExpression) this._skip, this._limit, k), this._project) : (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ApplySkipOrderToSort((DbExpression) this._skip.Limit(this._limit.Limit), this._skip).Limit(k), this._project);

        internal override DbExpression Skip(DbExpression k) => this._skip.Count.ExpressionKind == DbExpressionKind.Constant && this._limit.Limit.ExpressionKind == DbExpressionKind.Constant && k.ExpressionKind == DbExpressionKind.Constant ? (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.SubtractFromLimit((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.AddToSkip(this._source, this._skip, k), this._limit, k), this._project) : (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSkip((DbExpression) this._skip.Limit(this._limit.Limit), this._skip, k), this._project);
      }
      ...
      private class ProjectLimitSortLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Project(DbProjectExpression project) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ComposeProject((DbExpression) this._sort.Limit(this._limit.Limit), this._project, project);

        internal override DbExpression Limit(DbExpression k) => this._limit.Limit.ExpressionKind == DbExpressionKind.Constant && k.ExpressionKind == DbExpressionKind.Constant ? (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.MinimumLimit((DbExpression) this._sort, this._limit, k), this._project) : (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSort((DbExpression) this._sort.Limit(this._limit.Limit), this._sort).Limit(k), this._project);

        internal override DbExpression Skip(DbExpression k) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ApplySortOrderToSkip((DbExpression) this._sort.Limit(this._limit.Limit), this._sort, k), this._project);
      }
      ...
      private class ProjectSkipLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Limit(DbExpression k) => (DbExpression) this._root.Limit(k);

        internal override DbExpression Project(DbProjectExpression project) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ComposeProject((DbExpression) this._skip, this._project, project);

        internal override DbExpression Skip(DbExpression k) => this._skip.Count.ExpressionKind == DbExpressionKind.Constant && k.ExpressionKind == DbExpressionKind.Constant ? (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.AddToSkip(this._source, this._skip, k), this._project) : (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSkip((DbExpression) this._skip, this._skip, k), this._project);
      }
      ...
      private class SkipLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Skip(DbExpression k) => this._skip.Count.ExpressionKind == DbExpressionKind.Constant && k.ExpressionKind == DbExpressionKind.Constant ? (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.AddToSkip(this._source, this._skip, k) : (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSkip((DbExpression) this._skip, this._skip, k);
      }
      ...
      private class ProjectSortLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Project(DbProjectExpression project) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ComposeProject((DbExpression) this._sort, this._project, project);

        internal override DbExpression Filter(DbFilterExpression filter) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSort((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ComposeFilter(this._source, this._project, filter), this._sort), this._project);

        internal override DbExpression Limit(DbExpression k) => (DbExpression) this._root.Limit(k);

        internal override DbExpression Skip(DbExpression k) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindProject((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ApplySortOrderToSkip(this._source, this._sort, k), this._project);
      }
      ...
      private class SortLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Filter(DbFilterExpression filter) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindSort((DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.RebindFilter(this._source, filter), this._sort);

        internal override DbExpression Limit(DbExpression k) => (DbExpression) this._root.Limit(k);

        internal override DbExpression Skip(DbExpression k) => (DbExpression) ExpressionConverter.OrderByLifter.OrderByLifterBase.ApplySortOrderToSkip(this._source, this._sort, k);
      }
      ...
      private class PassthroughOrderByLifter : ExpressionConverter.OrderByLifter.OrderByLifterBase
      {
        ...
        internal override DbExpression Filter(DbFilterExpression filter) => (DbExpression) filter;

        internal override DbExpression OfType(TypeUsage type) => (DbExpression) this._root.OfType(type);

        internal override DbExpression Limit(DbExpression k) => (DbExpression) this._root.Limit(k);

        internal override DbExpression Skip(DbExpression k) => throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_SkipWithoutOrder);
      }
    }

    internal sealed class MemberAccessTranslator : 
      ExpressionConverter.TypedTranslator<MemberExpression>
    {
      ...
      {
        MemberInfo clrMember = TypeSystem.PropertyOrField(linq.Member, out string _, out Type _);
        if (linq.Expression != null)
        {
          ...
          {
            Delegate @delegate = Expression.Lambda((Expression) linq).Compile();
        ...
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnrecognizedMember((object) linq.Member.Name));
      }
        ...
        out DbExpression propertyExpression)
      {
        ...
        if (edmType1 != null)
        {
          if (edmType1.Members.TryGetValue(name, false, out EdmMember _))
          {
        ...
        if (definingType.EdmType is StructuralType edmType2)
        {
      ...
      private static IEnumerable<ExpressionConverter.MemberAccessTranslator.PropertyTranslator> GetPropertyTranslators() => (IEnumerable<ExpressionConverter.MemberAccessTranslator.PropertyTranslator>) new ExpressionConverter.MemberAccessTranslator.PropertyTranslator[6]
      {
      ...
      private sealed class SpatialPropertyTranslator : 
        ExpressionConverter.MemberAccessTranslator.PropertyTranslator
      {
      ...
      private sealed class GenericICollectionTranslator : 
        ExpressionConverter.MemberAccessTranslator.PropertyTranslator
      {
        ...
        {
          Type elementType;
          if (ExpressionConverter.MemberAccessTranslator.GenericICollectionTranslator.IsICollection(type, out elementType))
          {
          ...
          else
          {
            ...
            for (int index = 0; index < typeArray.Length; ++index)
            {
              ...
              if (ExpressionConverter.MemberAccessTranslator.GenericICollectionTranslator.IsICollection(type1, out elementType))
                yield return new KeyValuePair<Type, Type>(type1, elementType);
            }
            typeArray = (Type[]) null;
          }
        }
      ...
      internal abstract class PropertyTranslator
      {
        ...
        protected PropertyTranslator(params PropertyInfo[] properties) => this._properties = (IEnumerable<PropertyInfo>) properties;

        protected PropertyTranslator(IEnumerable<PropertyInfo> properties) => this._properties = properties;

        internal IEnumerable<PropertyInfo> Properties => this._properties;
      ...
      internal sealed class DefaultCanonicalFunctionPropertyTranslator : 
        ExpressionConverter.MemberAccessTranslator.PropertyTranslator
      {
        ...
        private static IEnumerable<PropertyInfo> GetProperties() => (IEnumerable<PropertyInfo>) new PropertyInfo[17]
        {
      ...
      internal sealed class RenameCanonicalFunctionPropertyTranslator : 
        ExpressionConverter.MemberAccessTranslator.PropertyTranslator
      {
        ...
        private static IEnumerable<PropertyInfo> GetProperties() => (IEnumerable<PropertyInfo>) new PropertyInfo[7]
        {
      ...
      internal sealed class VBDateAndTimeNowTranslator : 
        ExpressionConverter.MemberAccessTranslator.PropertyTranslator
      {
        private const string s_dateAndTimeTypeFullName = "Microsoft.VisualBasic.DateAndTime";
        ...
        private static PropertyInfo GetProperty(Assembly vbAssembly) => vbAssembly.GetType("Microsoft.VisualBasic.DateAndTime").GetDeclaredProperty("Now");
        ...
        ExpressionConverter.MemberAccessTranslator.PropertyTranslator
      {
        ...
        private static PropertyInfo GetProperty() => typeof (EntityCollection<>).GetDeclaredProperty("Count");
      ...
      internal sealed class NullableHasValueTranslator : 
        ExpressionConverter.MemberAccessTranslator.PropertyTranslator
      {
        ...
        private static PropertyInfo GetProperty() => typeof (Nullable<>).GetDeclaredProperty("HasValue");
      ...
      internal sealed class NullableValueTranslator : 
        ExpressionConverter.MemberAccessTranslator.PropertyTranslator
      {
        ...
        private static PropertyInfo GetProperty() => typeof (Nullable<>).GetDeclaredProperty("Value");
    ...
    internal static class StringTranslatorUtil
    {
        ...
        Expression linqExpression)
      {
        if (linqExpression.Type == typeof (object))
          linqExpression = linqExpression is ConstantExpression constantExpression2 ? (Expression) Expression.Constant(constantExpression2.Value) : linqExpression.RemoveConvert();
        ...
        if (nonNullableType.IsEnum)
        {
          if (Attribute.IsDefined((MemberInfo) nonNullableType, typeof (FlagsAttribute)))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.Elinq_ToStringNotSupportedForEnumsWithFlags);
        ...
        if (TypeSemantics.IsPrimitiveType(expression.ResultType, PrimitiveTypeKind.Boolean))
        {
          ...
          DbComparisonExpression comparisonExpression1 = expression.Equal((DbExpression) DbExpressionBuilder.True);
          DbComparisonExpression comparisonExpression2 = expression.Equal((DbExpression) DbExpressionBuilder.False);
          bool flag = true;
          DbConstantExpression constantExpression3 = DbExpressionBuilder.Constant((object) flag.ToString());
          flag = false;
          DbConstantExpression constantExpression4 = DbExpressionBuilder.Constant((object) flag.ToString());
        ...
        if (!ExpressionConverter.StringTranslatorUtil.SupportsCastToString(expression.ResultType))
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.Elinq_ToStringNotSupportedForType((object) expression.ResultType.EdmType.Name));
    ...
    internal abstract class Translator
    {
      ...
      protected Translator(params ExpressionType[] nodeTypes) => this._nodeTypes = nodeTypes;

      internal IEnumerable<ExpressionType> NodeTypes => (IEnumerable<ExpressionType>) this._nodeTypes;
    ...
    private sealed class ConstantTranslator : ExpressionConverter.TypedTranslator<ConstantExpression>
    {
        ...
        ConstantExpression linq)
      {
        if (linq == parent._funcletizer.RootContextExpression)
          throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedUseOfContextParameter((object) parent._funcletizer.RootContextParameter.Name));
        ...
        {
          if (flag1)
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedNullConstant((object) ExpressionConverter.DescribeClrType(linq.Type)));
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedConstant((object) ExpressionConverter.DescribeClrType(linq.Type)));
        }
        if (flag1)
        ...
        if (Helper.IsPrimitiveType(type2.EdmType))
        {
          ...
          if (nonNullableType.IsEnum())
            obj1 = System.Convert.ChangeType(linq.Value, nonNullableType.GetEnumUnderlyingType(), (IFormatProvider) CultureInfo.InvariantCulture);
        }
    ...
    private sealed class ParameterTranslator : 
      ExpressionConverter.TypedTranslator<ParameterExpression>
    {
      ...
      {
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ELinq_UnboundParameterExpression((object) linq.Name));
      }
    }
    ...
    private sealed class NewTranslator : ExpressionConverter.TypedTranslator<NewExpression>
    {
        ...
        NewExpression linq)
      {
        ...
        if (linq.Arguments.Count == 1 && this._castableTypes.Any<Tuple<Type, Type>>((Func<Tuple<Type, Type>, bool>) (cast => cast.Item1 == linq.Constructor.DeclaringType && cast.Item2 == linq.Arguments[0].Type)))
        ...
        if ((ConstructorInfo) null == linq.Constructor || linq.Arguments.Count != num)
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedConstructor);
        ...
        {
          string name;
          TypeSystem.PropertyOrField(linq.Members[index], out name, out Type _);
    ...
    private sealed class NewArrayInitTranslator : 
      ExpressionConverter.TypedTranslator<NewArrayExpression>
    {
        ...
        NewArrayExpression linq)
      {
        ...
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedType((object) ExpressionConverter.DescribeClrType(linq.Type)));
      }
    }
    ...
    private sealed class ListInitTranslator : ExpressionConverter.TypedTranslator<ListInitExpression>
    {
        ...
        ListInitExpression linq)
      {
        ...
        if (linq.Initializers.Any<ElementInit>((Func<ElementInit, bool>) (i => i.Arguments.Count != 1)))
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedInitializers);
    ...
    private sealed class MemberInitTranslator : 
      ExpressionConverter.TypedTranslator<MemberInitExpression>
    {
        ...
        MemberInitExpression linq)
      {
        ...
        {
          if (!(linq.Bindings[index] is MemberAssignment binding2))
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedBinding);
          string name;
          MemberInfo memberInfo = TypeSystem.PropertyOrField(binding2.Member, out name, out Type _);
          DbExpression dbExpression = parent.TranslateExpression(binding2.Expression);
          stringSet.Add(name);
    ...
    private sealed class ConditionalTranslator : 
      ExpressionConverter.TypedTranslator<ConditionalExpression>
    {
    ...
    private sealed class NotSupportedTranslator : ExpressionConverter.Translator
    {
      ...
      internal override DbExpression Translate(
        ...
        Expression linq)
      {
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedExpressionType((object) linq.NodeType));
      }
    }
    ...
    private sealed class ExtensionTranslator : ExpressionConverter.Translator
    {
      ...
      internal override DbExpression Translate(
        ...
        Expression linq)
      {
        if (!(linq is QueryParameterExpression queryParameter))
          throw new NotSupportedException(System.Data.Entity.Resources.Strings.ELinq_UnsupportedExpressionType((object) linq.NodeType));
    ...
    private sealed class IsTranslator : ExpressionConverter.TypedTranslator<TypeBinaryExpression>
    {
      ...
      {
        return (DbExpression) parent.TranslateExpression(linq.Expression).IsOf(parent.GetIsOrAsTargetType(ExpressionType.TypeIs, linq.TypeOperand, linq.Expression.Type));
      }
    }
    ...
    private sealed class AddTranslator : ExpressionConverter.BinaryTranslator
    {
      ...
      protected override DbExpression TypedTranslate(
        ...
        BinaryExpression linq)
      {
        return linq.IsStringAddExpression() ? ExpressionConverter.StringTranslatorUtil.ConcatArgs(parent, linq) : this.TranslateBinary(parent, parent.TranslateExpression(linq.Left), parent.TranslateExpression(linq.Right), linq);
      }
    ...
    private abstract class BitwiseBinaryTranslator : 
      ExpressionConverter.TypedTranslator<BinaryExpression>
    {
      private readonly string _canonicalFunctionName;
