...
namespace System.Data.Entity.Core.Objects.DataClasses
{
    ...
    where TEntity : class
  {
    ...
    {
      add => this._onAssociationChangedforObjectView += value;
      remove => this._onAssociationChangedforObjectView -= value;
    }
    ...
    internal int CountInternal => this._wrappedRelatedEntities == null ? 0 : this._wrappedRelatedEntities.Count;
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<IEnumerable<TEntity>>(entities, nameof (entities));
      this.CheckOwnerNull();
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<TEntity>(entity, nameof (entity));
    ...
    {
      EntityCollection<TEntity> entityCollection = this;
      cancellationToken.ThrowIfCancellationRequested();
      bool hasResults;
      ObjectQuery<TEntity> objectQuery = entityCollection.ValidateLoad<TEntity>(mergeOption, nameof (EntityCollection<TEntity>), out hasResults);
      entityCollection._suppressEvents = true;
      try
      {
        if (collection == null)
        {
          IEnumerable<TEntity> collection1;
          if (hasResults)
            collection1 = (IEnumerable<TEntity>) await (await objectQuery.ExecuteAsync(objectQuery.MergeOption, cancellationToken).WithCurrentCulture<ObjectResult<TEntity>>()).ToListAsync<TEntity>(cancellationToken).WithCurrentCulture<List<TEntity>>();
          else
            collection1 = Enumerable.Empty<TEntity>();
          entityCollection.Merge<TEntity>(collection1, mergeOption, true);
        }
        else
          entityCollection.Merge<TEntity>(collection, mergeOption, true);
      }
      finally
      {
        entityCollection._suppressEvents = false;
      }
      entityCollection.OnAssociationChanged(CollectionChangeAction.Refresh, (object) null);
    }
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<TEntity>(item, nameof (item));
    ...
    internal override void DisconnectedAdd(IEntityWrapper wrappedEntity)
    {
      if (wrappedEntity.Context != null && wrappedEntity.MergeOption != MergeOption.NoTracking)
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelatedEnd_UnableToAddEntity);
    ...
    {
      if (wrappedEntity.Context != null && wrappedEntity.MergeOption != MergeOption.NoTracking)
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelatedEnd_UnableToRemoveEntity);
      int num = this.RemoveFromCache(wrappedEntity, false, false) ? 1 : 0;
    ...
    internal bool RemoveInternal(TEntity entity) => this.Remove(this.EntityWrapperFactory.WrapEntityUsingContext((object) entity, this.ObjectContext), false);
    ...
    internal override bool CanSetEntityType(IEntityWrapper wrappedEntity) => wrappedEntity.Entity is TEntity;

    internal override void VerifyType(IEntityWrapper wrappedEntity)
    {
      if (!this.CanSetEntityType(wrappedEntity))
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelatedEnd_InvalidContainedType_Collection((object) wrappedEntity.Entity.GetType().FullName, (object) typeof (TEntity).FullName));
    }
    ...
    internal override bool RemoveFromObjectCache(IEntityWrapper wrappedEntity) => this.TargetAccessor.HasProperty && this.WrappedOwner.CollectionRemove((RelatedEnd) this, wrappedEntity.Entity);
    ...
    internal override bool IsEmpty() => this._wrappedRelatedEntities == null || this._wrappedRelatedEntities.Count == 0;

    internal override void VerifyMultiplicityConstraintsForAdd(bool applyConstraints)
    {
    }

    internal override void OnRelatedEndClear() => this._isLoaded = false;

    internal override bool ContainsEntity(IEntityWrapper wrappedEntity) => this._wrappedRelatedEntities != null && this._wrappedRelatedEntities.ContainsKey((TEntity) wrappedEntity.Entity);
    ...
    internal override IEnumerable GetInternalEnumerable() => (IEnumerable) this.WrappedRelatedEntities.Keys;

    internal override IEnumerable<IEntityWrapper> GetWrappedEntities() => (IEnumerable<IEntityWrapper>) this.WrappedRelatedEntities.Values;
    ...
    internal override bool CheckIfNavigationPropertyContainsEntity(IEntityWrapper wrapper)
    {
      ...
      try
      {
        ...
        {
          HashSet<TEntity> entitySet = navigationPropertyValue is IEnumerable<TEntity> source4 ? navigationPropertyValue as HashSet<TEntity> : throw new EntityException(System.Data.Entity.Resources.Strings.ObjectStateEntry_UnableToEnumerateCollection((object) this.TargetAccessor.PropertyName, (object) this.WrappedOwner.Entity.GetType().FullName));
          return !wrapper.OverridesEqualsOrGetHashCode || entitySet != null && entitySet.Comparer is ObjectReferenceEqualityComparer ? source4.Contains<TEntity>((TEntity) wrapper.Entity) : source4.Any<TEntity>((Func<TEntity, bool>) (o => (object) o == wrapper.Entity));
        }
      }
      finally
      {
    ...
    public void OnSerializing(StreamingContext context)
    {
      if (!(this.WrappedOwner.Entity is IEntityWithRelationships))
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelatedEnd_CannotSerialize((object) nameof (EntityCollection<TEntity>)));
    ...
    {
      this.CheckOwnerNull();
      return this.CreateSourceQuery<TEntity>(this.DefaultMergeOption, out bool _);
    }

    internal override IEnumerable CreateSourceQueryInternal() => (IEnumerable) this.CreateSourceQuery();

    internal override void AddToLocalCache(IEntityWrapper wrappedEntity, bool applyConstraints) => this.WrappedRelatedEntities[(TEntity) wrappedEntity.Entity] = wrappedEntity;
