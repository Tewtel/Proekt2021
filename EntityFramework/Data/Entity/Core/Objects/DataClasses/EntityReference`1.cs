...
namespace System.Data.Entity.Core.Objects.DataClasses
{
  ...
  public class EntityReference<TEntity> : EntityReference where TEntity : class
  {
    ...
    /// class instead.
    /// </remarks>
    public EntityReference() => this._wrappedCachedValue = NullEntityWrapper.NullWrapper;
    ...
    public TEntity Value
    {
      ...
      set => this.ReferenceValue = this.EntityWrapperFactory.WrapEntityUsingContext((object) value, this.ObjectContext);
    }

    internal override IEntityWrapper CachedValue => this._wrappedCachedValue;
    ...
    {
      EntityReference<TEntity> entityReference = this;
      entityReference.CheckOwnerNull();
      cancellationToken.ThrowIfCancellationRequested();
      bool hasResults;
      ObjectQuery<TEntity> objectQuery = entityReference.ValidateLoad<TEntity>(mergeOption, nameof (EntityReference<TEntity>), out hasResults);
      entityReference._suppressEvents = true;
      try
      {
        IList<TEntity> refreshedValue = (IList<TEntity>) null;
        if (hasResults)
          refreshedValue = (IList<TEntity>) await (await objectQuery.ExecuteAsync(objectQuery.MergeOption, cancellationToken).WithCurrentCulture<ObjectResult<TEntity>>()).ToListAsync<TEntity>(cancellationToken).WithCurrentCulture<List<TEntity>>();
        entityReference.HandleRefreshedValue(mergeOption, refreshedValue);
      }
      finally
      {
        entityReference._suppressEvents = false;
      }
      entityReference.OnAssociationChanged(CollectionChangeAction.Refresh, (object) null);
    }
    ...
    private void HandleRefreshedValue(MergeOption mergeOption, IList<TEntity> refreshedValue)
    {
      if (refreshedValue == null || !refreshedValue.Any<TEntity>())
      {
        if (!((AssociationType) this.RelationMetadata).IsForeignKey && this.ToEndMember.RelationshipMultiplicity == RelationshipMultiplicity.One)
          throw System.Data.Entity.Resources.Error.EntityReference_LessThanExpectedRelatedEntitiesFound();
        if (mergeOption == MergeOption.OverwriteChanges || mergeOption == MergeOption.PreserveChanges)
          this.ObjectContext.ObjectStateManager.RemoveRelationships(mergeOption, (AssociationSet) this.RelationshipSet, this.WrappedOwner.EntityKey ?? throw System.Data.Entity.Resources.Error.EntityKey_UnexpectedNull(), (AssociationEndMember) this.FromEndMember);
        this._isLoaded = true;
      }
      else
      {
        if (refreshedValue.Count<TEntity>() != 1)
          throw System.Data.Entity.Resources.Error.EntityReference_MoreThanExpectedRelatedEntitiesFound();
    ...
    private void IncludeEntityKey(bool doAttach)
    {
      ...
      RelationshipWrapper wrapper = new RelationshipWrapper((AssociationSet) this.RelationshipSet, this.RelationshipNavigation.From, this.WrappedOwner.EntityKey ?? throw System.Data.Entity.Resources.Error.EntityKey_UnexpectedNull(), this.RelationshipNavigation.To, this.DetachedEntityKey);
    ...
    internal override void VerifyType(IEntityWrapper wrappedEntity)
    {
      if (!this.CanSetEntityType(wrappedEntity))
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelatedEnd_InvalidContainedType_Reference((object) wrappedEntity.Entity.GetType().FullName, (object) typeof (TEntity).FullName));
    }

    internal override void DisconnectedAdd(IEntityWrapper wrappedEntity) => this.CheckOwnerNull();
    ...
    internal override bool IsEmpty() => this._wrappedCachedValue.Entity == null;

    internal override void VerifyMultiplicityConstraintsForAdd(bool applyConstraints)
    {
      if (applyConstraints && !this.IsEmpty())
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.EntityReference_CannotAddMoreThanOneEntityToEntityReference((object) this.RelationshipNavigation.To, (object) this.RelationshipNavigation.RelationshipName));
    }

    internal override void OnRelatedEndClear() => this._isLoaded = false;

    internal override bool ContainsEntity(IEntityWrapper wrappedEntity) => this._wrappedCachedValue.Entity != null && this._wrappedCachedValue.Entity == wrappedEntity.Entity;
    ...
    internal override bool CheckIfNavigationPropertyContainsEntity(IEntityWrapper wrapper) => this.TargetAccessor.HasProperty && this.WrappedOwner.GetNavigationPropertyValue((RelatedEnd) this) == wrapper.Entity;
    ...
    public void OnRefDeserialized(StreamingContext context) => this._wrappedCachedValue = this.EntityWrapperFactory.WrapEntityUsingContext((object) this._cachedValue, this.ObjectContext);
    ...
    public void OnSerializing(StreamingContext context)
    {
      if (!(this.WrappedOwner.Entity is IEntityWithRelationships))
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.RelatedEnd_CannotSerialize((object) nameof (EntityReference<TEntity>)));
    }
