...
namespace System.Data.Entity.Core.Common.Internal
{
  internal static class DbTypeMap
  {
    ...
    private static TypeUsage CreateType(PrimitiveTypeKind type) => DbTypeMap.CreateType(type, new FacetValues());

    private static TypeUsage CreateType(PrimitiveTypeKind type, FacetValues facets) => TypeUsage.Create((EdmType) EdmProviderManifest.Instance.GetPrimitiveType(type), facets);
  }
}
