  ...
  internal abstract class Shaper
  {
      ...
      else
      {
        ...
        if (entityEntry != null && !entityEntry.IsKeyEntry)
        {
          if (typeof (TEntity) != entityEntry.WrappedEntity.IdentityType)
          {
            ...
            throw new NotSupportedException(System.Data.Entity.Resources.Strings.Materializer_RecyclingEntity((object) TypeHelpers.GetFullName(entityKey1.EntityContainerName, entityKey1.EntitySetName), (object) typeof (TEntity).FullName, (object) entityEntry.WrappedEntity.IdentityType.FullName));
          }
          entityWrapper = EntityState.Added != entityEntry.State ? entityEntry.WrappedEntity : throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Materializer_AddedEntityAlreadyExists((object) typeof (TEntity).FullName));
        }
        else
        {
          entityWrapper = constructEntityDelegate(this);
      ...
      AssociationEndMember targetMember)
    {
      ...
      if ((object) targetKey != null)
      {
        ...
        ILookup<EntityKey, RelationshipEntry> relationshipLookup1 = ObjectStateManager.GetRelationshipLookup(this.Context.ObjectStateManager, cspaceAssociationSet, otherAssociationEnd, entityKey);
        EntityState newEntryState;
        if (!ObjectStateManager.TryUpdateExistingRelationships(this.Context, this.MergeOption, cspaceAssociationSet, otherAssociationEnd, relationshipLookup1, wrappedEntity, targetMember, targetKey, true, out newEntryState))
        {
          ...
          switch (otherAssociationEnd.RelationshipMultiplicity)
          {
            ...
            case RelationshipMultiplicity.One:
              ILookup<EntityKey, RelationshipEntry> relationshipLookup2 = ObjectStateManager.GetRelationshipLookup(this.Context.ObjectStateManager, cspaceAssociationSet, targetMember, targetKey);
              flag = !ObjectStateManager.TryUpdateExistingRelationships(this.Context, this.MergeOption, cspaceAssociationSet, targetMember, relationshipLookup2, entityEntry.WrappedEntity, otherAssociationEnd, entityKey, true, out newEntryState);
      ...
      out RelatedEnd relatedEnd)
    {
      ...
      if (sourceMember != null && targetMember != null)
      {
        ...
        if (wrappedEntity.EntityKey == (EntityKey) null)
          flag = true;
        else if (this.Workspace.MetadataOptimization.FindCSpaceAssociationSet(associationType, sourceEndName, wrappedEntity.EntityKey.EntitySetName, wrappedEntity.EntityKey.EntityContainerName, out EntitySet _) != null)
          flag = true;
        if (flag)
        {
    ...
    private void SetIsLoadedForSpan(RelatedEnd relatedEnd, bool forceToTrue)
    {
      if (!forceToTrue)
      {
        ...
        if (relatedEnd is EntityReference entityReference2)
          forceToTrue &= entityReference2.EntityKey == (EntityKey) null;
      }
    ...
    public TColumn GetColumnValueWithErrorHandling<TColumn>(int ordinal) => new Shaper.ColumnErrorHandlingValueReader<TColumn>().GetValue(this.Reader, ordinal);
    ...
    public HierarchyId GetHierarchyIdColumnValue(int ordinal) => new HierarchyId(this.Reader.GetValue(ordinal).ToString());

    protected virtual DbSpatialDataReader CreateSpatialDataReader() => SpatialHelpers.CreateSpatialDataReader(this.Workspace, this.Reader);

    public DbGeography GetGeographyColumnValue(int ordinal) => this.Streaming ? this._spatialReader.Value.GetGeography(ordinal) : (DbGeography) this.Reader.GetValue(ordinal);

    public DbGeometry GetGeometryColumnValue(int ordinal) => this.Streaming ? this._spatialReader.Value.GetGeometry(ordinal) : (DbGeometry) this.Reader.GetValue(ordinal);
    ...
    {
      AssociationEndMember otherAssociationEnd = MetadataHelper.GetOtherAssociationEnd(targetMember);
      AssociationSet cspaceAssociationSet = this.Context.MetadataWorkspace.MetadataOptimization.FindCSpaceAssociationSet((AssociationType) otherAssociationEnd.DeclaringType, otherAssociationEnd.Name, sourceKey.EntitySetName, sourceKey.EntityContainerName, out EntitySet _);
    ...
    private void UpdateEntry<TEntity>(IEntityWrapper wrappedEntity, EntityEntry existingEntry)
    {
      ...
      if (type != existingEntry.WrappedEntity.IdentityType)
      {
        ...
        throw new NotSupportedException(System.Data.Entity.Resources.Strings.Materializer_RecyclingEntity((object) TypeHelpers.GetFullName(entityKey.EntityContainerName, entityKey.EntitySetName), (object) type.FullName, (object) existingEntry.WrappedEntity.IdentityType.FullName));
      }
      if (EntityState.Added == existingEntry.State)
        throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Materializer_AddedEntityAlreadyExists((object) type.FullName));
    ...
    internal abstract class ErrorHandlingValueReader<T>
    {
      ...
      private static T GetTypedValueDefault(DbDataReader reader, int ordinal)
      {
        ...
        return underlyingType != (Type) null && underlyingType.IsEnum() ? (T) Shaper.ErrorHandlingValueReader<T>.GetGenericTypedValueDefaultMethod(underlyingType).Invoke((object) null, new object[2]
        {
          (object) reader,
          (object) ordinal
        }) : (T) CodeGenEmitter.GetReaderMethod(typeof (T), out bool _).Invoke((object) reader, new object[1]
        {
          (object) ordinal
        });
      }

      public static MethodInfo GetGenericTypedValueDefaultMethod(Type underlyingType) => typeof (Shaper.ErrorHandlingValueReader<>).MakeGenericType(underlyingType).GetOnlyDeclaredMethod("GetTypedValueDefault");

      private static object GetUntypedValueDefault(DbDataReader reader, int ordinal) => reader.GetValue(ordinal);
    ...
    private class ColumnErrorHandlingValueReader<TColumn> : Shaper.ErrorHandlingValueReader<TColumn>
    {
      ...
      protected override Exception CreateNullValueException() => (Exception) new InvalidOperationException(System.Data.Entity.Resources.Strings.Materializer_NullReferenceCast((object) typeof (TColumn)));

      protected override Exception CreateWrongTypeException(Type resultType) => (Exception) EntityUtil.ValueInvalidCast(resultType, typeof (TColumn));
    }

    private class PropertyErrorHandlingValueReader<TProperty> : 
      Shaper.ErrorHandlingValueReader<TProperty>
    {
      ...
      protected override Exception CreateNullValueException()
      {
        ...
        return (Exception) new ConstraintException(System.Data.Entity.Resources.Strings.Materializer_SetInvalidValue((object) type, (object) this._typeName, (object) this._propertyName, (object) "null"));
      }
      ...
      protected override Exception CreateWrongTypeException(Type resultType)
      {
        ...
        if ((object) type == null)
          type = typeof (TProperty);
        return (Exception) new InvalidOperationException(System.Data.Entity.Resources.Strings.Materializer_SetInvalidValue((object) type, (object) this._typeName, (object) this._propertyName, (object) resultType));
      }
    }
  }
}
