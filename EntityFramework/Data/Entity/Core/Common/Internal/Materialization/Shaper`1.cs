...
namespace System.Data.Entity.Core.Common.Internal.Materialization
{
  internal class Shaper<T> : Shaper
  {
    ...
    public virtual IDbEnumerator<T> GetEnumerator()
    {
      ...
      return this._isObjectQuery ? (IDbEnumerator<T>) new Shaper<T>.ObjectQueryNestedEnumerator(rowEnumerator) : (IDbEnumerator<T>) new Shaper<T>.RecordStateEnumerator(rowEnumerator);
    }
    ...
    {
      Shaper<T> shaper = this;
      bool flag;
      try
      {
        flag = await shaper.Reader.ReadAsync(cancellationToken).WithCurrentCulture<bool>();
      }
      catch (Exception ex)
      {
        shaper.HandleReaderException(ex);
        throw;
      }
      return flag;
    }
    ...
    private class SimpleEnumerator : 
      IDbEnumerator<T>,
      IEnumerator<T>,
      IDisposable,
      IEnumerator,
      ...
      IDbAsyncEnumerator
    {
      ...
      internal SimpleEnumerator(Shaper<T> shaper) => this._shaper = shaper;

      public T Current => this._shaper.RootCoordinator.Current;

      object IEnumerator.Current => (object) this._shaper.RootCoordinator.Current;

      object IDbAsyncEnumerator.Current => (object) this._shaper.RootCoordinator.Current;
    ...
    private class RowNestedResultEnumerator : 
      IDbEnumerator<Coordinator[]>,
      IEnumerator<Coordinator[]>,
      IDisposable,
      IEnumerator,
      IDbAsyncEnumerator<Coordinator[]>,
      IDbAsyncEnumerator
    {
      private readonly Shaper<T> _shaper;
      ...
      public Coordinator[] Current => this._current;

      object IEnumerator.Current => (object) this._current;

      object IDbAsyncEnumerator.Current => (object) this._current;
      ...
      internal Coordinator<T> RootCoordinator => this._shaper.RootCoordinator;
    }

    private class ObjectQueryNestedEnumerator : 
      IDbEnumerator<T>,
      IEnumerator<T>,
      IDisposable,
      IEnumerator,
      ...
      IDbAsyncEnumerator
    {
      ...
      public T Current => this._previousElement;
...
label_5:
      ...
      public void Reset() => this._rowEnumerator.Reset();
    ...
    private class RecordStateEnumerator : 
      IDbEnumerator<RecordState>,
      IEnumerator<RecordState>,
      IDisposable,
      IEnumerator,
      IDbAsyncEnumerator<RecordState>,
      IDbAsyncEnumerator
    {
      private readonly Shaper<T>.RowNestedResultEnumerator _rowEnumerator;
      ...
      public RecordState Current => this._current;
