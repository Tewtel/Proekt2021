    ...
    internal class TranslatorVisitor : ColumnMapVisitorWithResults<TranslatorResult, TranslatorArg>
    {
        ...
        TranslatorArg arg)
      {
        ...
        else
        {
          ...
          ConstructorInfo constructorForType = DelegateFactory.GetConstructorForType(this.DetermineClrType((EdmType) edmType));
          List<MemberBinding> propertyBindings = this.CreatePropertyBindings((StructuredColumnMap) columnMap, edmType.Properties);
          expression2 = (Expression) Expression.MemberInit(Expression.New(constructorForType), (IEnumerable<MemberBinding>) propertyBindings);
          if (expression1 != null)
        ...
        ReadOnlyMetadataCollection<EdmProperty> properties)
      {
        ...
        for (int index = 0; index < columnMap.Properties.Length; ++index)
        {
          ...
          if (columnMap.Properties[index] is ScalarColumnMap property3)
          {
            string propertyName = methodInfo.Name.Substring(4);
            Expression withErrorHandling = CodeGenEmitter.Emit_Shaper_GetPropertyValueWithErrorHandling(propertyType, property3.ColumnPos, propertyName, methodInfo.DeclaringType.Name, property3.Type);
        ...
        RowType spanRowType)
      {
        ...
        for (int key = 1; key < columnMap.Properties.Length; ++key)
        {
          AssociationEndMember associationEndMember = spanMap[key];
          TranslatorResult translatorResult1 = Translator.TranslatorVisitor.AcceptWithMappedType(this, columnMap.Properties[key]);
          Expression expression2 = translatorResult1.Expression;
          if (translatorResult1 is CollectionTranslatorResult translatorResult3)
          {
            Expression toGetCoordinator = translatorResult3.ExpressionToGetCoordinator;
        ...
        else
        {
          ...
          if (elementType2 != (Type) null)
          {
            ...
            if (collectionType == (Type) null)
              throw new InvalidOperationException(System.Data.Entity.Resources.Strings.ObjectQuery_UnableToMaterializeArbitaryProjectionType((object) arg.RequestedType));
      ...
      public static MethodInfo GetGenericElementsMethod(Type elementType) => typeof (Coordinator<>).MakeGenericType(elementType).GetOnlyDeclaredMethod("GetElements");
      ...
      private void ExitCoordinatorTranslateScope() => this._currentCoordinatorScratchpad = this._currentCoordinatorScratchpad.Parent;
      ...
      {
        EntityIdentity entityIdentity = columnMap.EntityIdentity;
        ConditionalExpression conditionalExpression = Expression.Condition(CodeGenEmitter.Emit_EntityKey_HasValue(entityIdentity.Keys), this.Emit_EntityKey_ctor(this, entityIdentity, (EdmType) ((RefType) columnMap.Type.EdmType).ElementType, true, out Expression _), (Expression) Expression.Constant((object) null, typeof (EntityKey)));
        int columnPos = ((ScalarColumnMap) entityIdentity.Keys[0]).ColumnPos;
        if (!this._streaming && !this.NullableColumns.Contains(columnPos))
          this.NullableColumns.Add(columnPos);
        Type requestedType = arg.RequestedType;
        return new TranslatorResult((Expression) conditionalExpression, requestedType);
      }
        ...
        TranslatorArg arg)
      {
        ...
        Expression expression1;
        if (Helper.IsSpatialType(type1, out PrimitiveTypeKind _))
        {
        ...
        else if (Helper.IsHierarchyIdType(type1))
        {
          expression1 = CodeGenEmitter.Emit_Conditional_NotDBNull(CodeGenEmitter.Emit_EnsureType(CodeGenEmitter.Emit_Shaper_GetHierarchyIdColumnValue(columnPos), requestedType), columnPos, requestedType);
        }
        else
        {
          bool isNullable;
      ...
      private int AllocateStateSlot() => this.StateSlotCount++;

      private Type DetermineClrType(TypeUsage typeUsage) => this.DetermineClrType(typeUsage.EdmType);
      ...
      private static ConstructorInfo GetConstructor(Type type) => !type.IsAbstract() ? DelegateFactory.GetConstructorForType(type) : (ConstructorInfo) null;
