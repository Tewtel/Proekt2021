...
namespace System.Data.Entity.Core.Common.EntitySql.AST
{
  internal sealed class Literal : Node
  {
    ...
    internal static Literal NewBooleanLiteral(bool value) => new Literal(value);
    ...
    internal bool IsNumber => this._literalKind == LiteralKind.Number;
    ...
    internal bool IsSignedNumber
    {
      get
      {
        if (!this.IsNumber)
          return false;
        return this._originalValue[0] == '-' || this._originalValue[0] == '+';
      }
    }

    internal bool IsString => this._literalKind == LiteralKind.String || this._literalKind == LiteralKind.UnicodeString;

    internal bool IsUnicodeString => this._literalKind == LiteralKind.UnicodeString;

    internal bool IsNullLiteral => this._literalKind == LiteralKind.Null;

    internal string OriginalValue => this._originalValue;

    internal void PrefixSign(string sign) => this._originalValue = sign + this._originalValue;
    ...
    private static object ConvertNumericLiteral(ErrorContext errCtx, string numericString)
    {
      ...
      if (-1 != startIndex)
      {
        ...
        switch (upperInvariant)
        {
          case "D":
            double result1;
            if (!double.TryParse(s, NumberStyles.Float, (IFormatProvider) CultureInfo.InvariantCulture, out result1))
            {
          ...
          case "F":
            float result2;
            if (!float.TryParse(s, NumberStyles.Float, (IFormatProvider) CultureInfo.InvariantCulture, out result2))
            {
          ...
          case "UL":
            ulong result4;
            if (!ulong.TryParse(s, NumberStyles.Integer, (IFormatProvider) CultureInfo.InvariantCulture, out result4))
            {
          ...
          case "U":
            uint result6;
            if (!uint.TryParse(s, NumberStyles.Integer, (IFormatProvider) CultureInfo.InvariantCulture, out result6))
            {
    ...
    private static byte[] ConvertBinaryLiteralValue(string binaryLiteralValue)
    {
      ...
      int length = num3 / 2;
      int num4 = (uint) (num3 % 2) > 0U ? 1 : 0;
      if (num4 != 0)
        ++length;
      byte[] numArray1 = new byte[length];
      int num5 = 0;
      if (num4 != 0)
        numArray1[num5++] = (byte) Literal.HexDigitToBinaryValue(binaryLiteralValue[num1++]);
      while (num1 < num2)
      {
        ...
        int index2 = num1;
        int num6 = index2 + 1;
        int num7 = Literal.HexDigitToBinaryValue(str1[index2]) << 4;
        string str2 = binaryLiteralValue;
        int index3 = num6;
        num1 = index3 + 1;
        int binaryValue = Literal.HexDigitToBinaryValue(str2[index3]);
        int num8 = (int) (byte) (num7 | binaryValue);
        numArray2[index1] = (byte) num8;
      }
      return numArray1;
    }
    ...
    private static Guid ConvertGuidLiteralValue(string guidLiteralValue) => new Guid(guidLiteralValue);
  }
}
