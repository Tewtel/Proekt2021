...
namespace System.Data.Entity.Core.Common
{
  internal static class TypeHelpers
  {
    ...
    internal static void AssertEdmType(TypeUsage typeUsage)
    {
      ...
      else if (TypeSemantics.IsPrimitiveType(typeUsage) && edmType is PrimitiveType primitiveType2 && primitiveType2.DataSpace != DataSpace.CSpace)
    ...
    internal static bool IsValidGroupKeyType(TypeUsage typeUsage) => TypeHelpers.IsSetComparableOpType(typeUsage);

    internal static bool IsValidDistinctOpType(TypeUsage typeUsage) => TypeHelpers.IsSetComparableOpType(typeUsage);
    ...
    internal static bool IsValidIsNullOpType(TypeUsage typeUsage) => TypeSemantics.IsReferenceType(typeUsage) || TypeSemantics.IsEntityType(typeUsage) || TypeSemantics.IsScalarType(typeUsage) || TypeSemantics.IsRowType(typeUsage);

    internal static bool IsValidInOpType(TypeUsage typeUsage) => TypeSemantics.IsReferenceType(typeUsage) || TypeSemantics.IsEntityType(typeUsage) || TypeSemantics.IsScalarType(typeUsage);
    ...
    internal static bool IsFacetValueConstant(TypeUsage type, string facetName) => Helper.GetFacet((IEnumerable<FacetDescription>) ((PrimitiveType) type.EdmType).FacetDescriptions, facetName).IsConstant;
    ...
    internal static CollectionType CreateCollectionType(TypeUsage elementType) => new CollectionType(elementType);

    internal static TypeUsage CreateCollectionTypeUsage(TypeUsage elementType) => TypeUsage.Create((EdmType) new CollectionType(elementType));
    ...
    internal static RefType CreateReferenceType(EntityTypeBase entityType) => new RefType((EntityType) entityType);

    internal static TypeUsage CreateReferenceTypeUsage(EntityType entityType) => TypeUsage.Create((EdmType) TypeHelpers.CreateReferenceType((EntityTypeBase) entityType));
    ...
    internal static RowType CreateKeyRowType(EntityTypeBase entityType)
    {
      ...
      return keyValuePairList.Count >= 1 ? TypeHelpers.CreateRowType((IEnumerable<KeyValuePair<string, TypeUsage>>) keyValuePairList) : throw new ArgumentException(Strings.Cqt_Metadata_EntityTypeEmptyKeyMembersInvalid, nameof (entityType));
    }

    internal static TypeUsage GetPrimitiveTypeUsageForScalar(TypeUsage scalarType) => !TypeSemantics.IsEnumerationType(scalarType) ? scalarType : TypeHelpers.CreateEnumUnderlyingTypeUsage(scalarType);

    internal static TypeUsage CreateEnumUnderlyingTypeUsage(TypeUsage enumTypeUsage) => TypeUsage.Create((EdmType) Helper.GetUnderlyingEdmTypeForEnumType(enumTypeUsage.EdmType), (IEnumerable<Facet>) enumTypeUsage.Facets);

    internal static TypeUsage CreateSpatialUnionTypeUsage(TypeUsage spatialTypeUsage) => TypeUsage.Create((EdmType) Helper.GetSpatialNormalizedPrimitiveType(spatialTypeUsage.EdmType), (IEnumerable<Facet>) spatialTypeUsage.Facets);

    internal static IBaseList<EdmMember> GetAllStructuralMembers(TypeUsage type) => TypeHelpers.GetAllStructuralMembers(type.EdmType);
    ...
    internal static IEnumerable GetDeclaredStructuralMembers(TypeUsage type) => TypeHelpers.GetDeclaredStructuralMembers(type.EdmType);
    ...
    internal static TypeUsage GetElementTypeUsage(TypeUsage type)
    {
      ...
      return TypeSemantics.IsReferenceType(type) ? TypeUsage.Create((EdmType) ((RefType) type.EdmType).ElementType) : (TypeUsage) null;
    }
    ...
    internal static string GetFullName(string qualifier, string name) => !string.IsNullOrEmpty(qualifier) ? string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0}.{1}", (object) qualifier, (object) name) : string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0}", (object) name);
    ...
    internal static TypeUsage GetLiteralTypeUsage(PrimitiveTypeKind primitiveTypeKind) => TypeHelpers.GetLiteralTypeUsage(primitiveTypeKind, true);
    ...
    internal static bool IsCanonicalFunction(EdmFunction function) => function.DataSpace == DataSpace.CSpace && function.NamespaceName == "Edm";
  }
}
