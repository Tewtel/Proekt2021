...
namespace System.Data.Entity.Core.Common.CommandTrees.Internal
{
  internal class ViewSimplifier
  {
    ...
    private ViewSimplifier(EntitySetBase viewTarget) => this.extent = viewTarget;
    ...
    private DbExpression AddFkRelatedEntityRefs(DbExpression viewConstructor)
    {
      ...
      List<AssociationSet> list1 = targetSet.EntityContainer.BaseEntitySets.Where<EntitySetBase>((Func<EntitySetBase, bool>) (es => es.BuiltInTypeKind == BuiltInTypeKind.AssociationSet)).Cast<AssociationSet>().Where<AssociationSet>((Func<AssociationSet, bool>) (assocSet => assocSet.ElementType.IsForeignKey && assocSet.AssociationSetEnds.Any<AssociationSetEnd>((Func<AssociationSetEnd, bool>) (se => se.EntitySet == targetSet)))).ToList<AssociationSet>();
      if (list1.Count == 0)
      {
        this.doNotProcess = true;
        return (DbExpression) null;
    ...
    private static DbExpression SimplifyCaseStatement(DbExpression expression)
    {
      ...
      return !flag ? (DbExpression) null : (DbExpression) DbExpressionBuilder.Case((IEnumerable<DbExpression>) dbExpressionList, (IEnumerable<DbExpression>) dbCaseExpression.Then, dbCaseExpression.Else);
    }
    ...
    private static bool TrySimplifyPredicate(DbExpression predicate, out DbExpression simplified)
    {
      ...
      if (dbCaseExpression.Then.Count != 1 && dbCaseExpression.Then[0].ExpressionKind == DbExpressionKind.Constant)
        return false;
      DbConstantExpression constantExpression1 = (DbConstantExpression) dbCaseExpression.Then[0];
      bool flag = true;
      if (!flag.Equals(constantExpression1.Value))
        return false;
      if (dbCaseExpression.Else != null)
      {
        if (dbCaseExpression.Else.ExpressionKind != DbExpressionKind.Constant)
          return false;
        DbConstantExpression constantExpression2 = (DbConstantExpression) dbCaseExpression.Else;
        flag = true;
        if (flag.Equals(constantExpression2.Value))
          return false;
      }
    ...
    private static DbExpression CollapseNestedProjection(DbExpression expression)
    {
      ...
      return projectionCollapser.IsDoomed ? (DbExpression) null : (DbExpression) expression1.Input.Project(projection3);
    }
    ...
    private class ProjectionCollapser : DefaultExpressionVisitor
    {
      ...
      internal DbExpression CollapseProjection(DbExpression expression) => this.VisitExpression(expression);

      public override DbExpression Visit(DbPropertyExpression property)
      {
        System.Data.Entity.Utilities.Check.NotNull<DbPropertyExpression>(property, nameof (property));
        return property.Instance.ExpressionKind == DbExpressionKind.VariableReference && this.IsOuterBindingVarRef((DbVariableReferenceExpression) property.Instance) ? this.m_varRefMemberBindings[property.Property.Name] : base.Visit(property);
      }

      public override DbExpression Visit(DbVariableReferenceExpression varRef)
      {
        System.Data.Entity.Utilities.Check.NotNull<DbVariableReferenceExpression>(varRef, nameof (varRef));
        if (this.IsOuterBindingVarRef(varRef))
          this.m_doomed = true;
        return base.Visit(varRef);
      }

      private bool IsOuterBindingVarRef(DbVariableReferenceExpression varRef) => varRef.VariableName == this.m_outerBinding.VariableName;

      internal bool IsDoomed => this.m_doomed;
    }
  }
}
