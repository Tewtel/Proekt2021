...
namespace System.Data.Entity.Core.Common.Utils
{
  ...
  {
    internal static void FormatTraceLine(string format, params object[] args) => Trace.WriteLine(string.Format((IFormatProvider) CultureInfo.InvariantCulture, format, args));

    internal static void StringTrace(string arg) => Trace.Write(arg);

    internal static void StringTraceLine(string arg) => Trace.WriteLine(arg);
      ...
      Func<TNode, IEnumerable<TNode>> getImmediateSubNodes)
    {
      ...
      while (nodes.Count > 0)
      {
        ...
        {
          List<TNode> nodeList = new List<TNode>(getImmediateSubNodes(node));
