  ...
  internal abstract class ModificationFunctionMappingTranslator
  {
    ...
    private sealed class EntitySetTranslator : ModificationFunctionMappingTranslator
    {
      ...
      private Tuple<EntityTypeModificationFunctionMapping, ModificationFunctionMapping> GetFunctionMapping(
        ExtractedStateEntry stateEntry)
      {
        EntityType key = stateEntry.Current == null ? (EntityType) stateEntry.Original.StructuralType : (EntityType) stateEntry.Current.StructuralType;
        EntityTypeModificationFunctionMapping typeMapping = this.m_typeMappings[key];
        ...
        switch (stateEntry.State)
        {
          ...
          case EntityState.Modified:
            ...
            EntityUtil.ValidateNecessaryModificationFunctionMapping(mapping, "Update", stateEntry.Source, "EntityType", key.Name);
            break;
          case EntityState.Added:
            mapping = typeMapping.InsertFunctionMapping;
            EntityUtil.ValidateNecessaryModificationFunctionMapping(mapping, "Insert", stateEntry.Source, "EntityType", key.Name);
            break;
          case EntityState.Deleted:
            mapping = typeMapping.DeleteFunctionMapping;
            EntityUtil.ValidateNecessaryModificationFunctionMapping(mapping, "Delete", stateEntry.Source, "EntityType", key.Name);
            break;
          default:
        ...
        Dictionary<AssociationEndMember, IEntityStateEntry> originalReferenceEnds)
      {
        foreach (ModificationFunctionParameterBinding parameterBinding in functionMapping.ParameterBindings)
        {
          ...
          if (parameterBinding.MemberPath.AssociationSetEnd != null)
          {
            ...
            if ((parameterBinding.IsCurrent ? (currentReferenceEnds.TryGetValue(associationEndMember, out stateEntry1) ? 1 : 0) : (originalReferenceEnds.TryGetValue(associationEndMember, out stateEntry1) ? 1 : 0)) == 0)
            {
              if (associationEndMember.RelationshipMultiplicity == RelationshipMultiplicity.One)
                throw new UpdateException(System.Data.Entity.Resources.Strings.Update_MissingRequiredRelationshipValue((object) stateEntry.Source.EntitySet.Name, (object) parameterBinding.MemberPath.AssociationSetEnd.ParentAssociationSet.Name), (Exception) null, command.GetStateEntries(translator).Cast<ObjectStateEntry>().Distinct<ObjectStateEntry>());
