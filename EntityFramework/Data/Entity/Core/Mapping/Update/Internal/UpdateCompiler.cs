...
namespace System.Data.Entity.Core.Mapping.Update.Internal
{
  internal sealed class UpdateCompiler
  {
    ...
    internal UpdateCompiler(UpdateTranslator translator) => this.m_translator = translator;
      ...
      for (int index1 = 0; index1 < processor.Table.ElementType.Properties.Count; ++index1)
      {
        ...
        if (flag3)
        {
          ...
          if (-1 != identifier)
          {
            if (this.m_translator.KeyManager.HasPrincipals(identifier))
              throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Update_GeneratedDependent((object) property.Name));
            ...
            if (generatedPattern != StoreGeneratedPattern.Identity && processor.IsKeyProperty(index1))
              throw new NotSupportedException(System.Data.Entity.Resources.Strings.Update_NotSupportedComputedKeyColumn((object) "StoreGeneratedPattern", (object) "Computed", (object) "Identity", (object) property.Name, (object) property.DeclaringType.FullName));
          }
        }
        ...
        if (!flag1 && !insertMode && generatedPattern == StoreGeneratedPattern.Identity)
        {
          ...
          if (!ByValueEqualityComparer.Default.Equals(memberValue.GetSimpleValue(), result.GetSimpleValue()))
            throw new InvalidOperationException(System.Data.Entity.Resources.Strings.Update_ModifyingIdentityColumn((object) "Identity", (object) property.Name, (object) property.DeclaringType.FullName));
          flag1 = true;
        }
        if (!flag1)
      ...
      PropagatorResult value)
    {
      ...
      return valueExpression.ExpressionKind == DbExpressionKind.Null ? (DbExpression) propertyExpression.IsNull() : (DbExpression) propertyExpression.Equal(valueExpression);
    }
    ...
    private static bool HasFlag(PropagatorResult input, PropagatorFlags flags) => input != null && (uint) (flags & input.PropagatorFlags) > 0U;

    private static DbExpressionBinding GetTarget(TableChangeProcessor processor) => processor.Table.Scan().BindAs("target");
  }
}
