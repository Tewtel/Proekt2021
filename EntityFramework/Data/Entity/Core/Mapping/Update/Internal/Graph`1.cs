...
namespace System.Data.Entity.Core.Mapping.Update.Internal
{
  internal class Graph<TVertex>
  {
    ...
    internal IEnumerable<TVertex> Vertices => (IEnumerable<TVertex>) this.m_vertices;
    ...
    internal IEnumerable<KeyValuePair<TVertex, TVertex>> Edges
    {
      get
      {
        ...
        {
          KeyValuePair<TVertex, HashSet<TVertex>> successors = successor;
          foreach (TVertex vertex in successors.Value)
            yield return new KeyValuePair<TVertex, TVertex>(successors.Key, vertex);
          successors = new KeyValuePair<TVertex, HashSet<TVertex>>();
        }
      }
    }

    internal void AddVertex(TVertex vertex) => this.m_vertices.Add(vertex);
      ...
      out IEnumerable<TVertex> remainder)
    {
      ...
      while (0 < sortedSet.Count)
      {
        ...
        {
          foreach (TVertex key in vertexSet)
          {
            if (--this.m_predecessorCounts[key] == 0)
              sortedSet.Add(key);
          }
