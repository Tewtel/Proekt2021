  ...
  internal class UpdateCommandOrderer : Graph<UpdateCommand>
  {
      ...
      out KeyToListMap<EntitySetBase, ReferentialConstraint> targetMap)
    {
      ...
      foreach (EntityContainer container in containers)
      {
        ...
        {
          if (baseEntitySet is AssociationSet associationSet2)
          {
            AssociationSetEnd associationSetEnd1 = (AssociationSetEnd) null;
            AssociationSetEnd associationSetEnd2 = (AssociationSetEnd) null;
            if (2 == associationSet2.AssociationSetEnds.Count)
            {
              AssociationType elementType = associationSet2.ElementType;
              bool flag = false;
              ...
              foreach (ReferentialConstraint referentialConstraint2 in elementType.ReferentialConstraints)
              {
                if (!flag)
                  flag = true;
                associationSetEnd1 = associationSet2.AssociationSetEnds[referentialConstraint2.ToRole.Name];
                associationSetEnd2 = associationSet2.AssociationSetEnds[referentialConstraint2.FromRole.Name];
    ...
    private void AddServerGenDependencies()
    {
      ...
      foreach (UpdateCommand vertex in this.Vertices)
      {
        foreach (int outputIdentifier in vertex.OutputIdentifiers)
        {
          ...
          {
            throw new UpdateException(System.Data.Entity.Resources.Strings.Update_AmbiguousServerGenIdentifier, (Exception) ex, vertex.GetStateEntries(this._translator).Cast<ObjectStateEntry>().Distinct<ObjectStateEntry>());
          }
        }
      }
      foreach (UpdateCommand vertex in this.Vertices)
    ...
    private void AddForeignKeyDependencies() => this.AddForeignKeyEdges(this.DetermineForeignKeyPredecessors());
      ...
      KeyToListMap<EntityKey, UpdateCommand> requiredMap)
    {
      foreach (KeyValuePair<EntityKey, List<UpdateCommand>> keyValuePair in requiredMap.KeyValuePairs)
      {
        ...
        foreach (UpdateCommand enumerateValue in producedMap.EnumerateValues(key))
        {
          ...
          {
            if (enumerateValue != to && (enumerateValue.Kind == UpdateCommandKind.Function || to.Kind == UpdateCommandKind.Function))
    ...
    private class ForeignKeyValueComparer : IEqualityComparer<UpdateCommandOrderer.ForeignKeyValue>
    {
      ...
      internal ForeignKeyValueComparer(IEqualityComparer<CompositeKey> baseComparer) => this._baseComparer = baseComparer;
      ...
      public int GetHashCode(UpdateCommandOrderer.ForeignKeyValue obj) => this._baseComparer.GetHashCode(obj.Key);
    }
  }
}
