...
namespace System.Data.Entity.Core.Mapping.Update.Internal
{
  internal class UndirectedGraph<TVertex> : InternalBase
  {
    ...
    internal IEnumerable<TVertex> Vertices => this.m_graph.Vertices;

    internal IEnumerable<KeyValuePair<TVertex, TVertex>> Edges => this.m_graph.Edges;

    internal void AddVertex(TVertex vertex) => this.m_graph.AddVertex(vertex);
    ...
    internal override void ToCompactString(StringBuilder builder) => builder.Append((object) this.m_graph);

    private class ComponentNum
    {
      internal int componentNum;

      internal ComponentNum(int compNum) => this.componentNum = compNum;

      public override string ToString() => StringUtil.FormatInvariant("{0}", (object) this.componentNum);
    }
  }
}
