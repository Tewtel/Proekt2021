...
namespace System.Data.Entity.Core.Mapping.Update.Internal
{
  internal class UpdateTranslator
  {
    ...
    internal MetadataWorkspace MetadataWorkspace => this.Connection.GetMetadataWorkspace();

    internal virtual KeyManager KeyManager { get; private set; }

    internal ViewLoader ViewLoader => this.MetadataWorkspace.GetUpdateViewLoader();

    internal RecordConverter RecordConverter => this._recordConverter;

    internal virtual EntityConnection Connection => this._adapter.Connection;

    internal virtual int? CommandTimeout => this._adapter.CommandTimeout;

    public virtual DbInterceptionContext InterceptionContext => this._interceptionContext;
...
label_13:
        ...
        if (MetadataHelper.GetEntityTypeForEnd((AssociationEndMember) referentialConstraint.ToRole).IsAssignableFrom(extendedDataRecord.DataRecordInfo.RecordType.EdmType))
        {
          ...
          if ((EntityKey) null != principalKey)
          {
            ...
            if (!this._stateManager.TryGetEntityStateEntry(principalKey, out stateEntry1) && currentValues && this.KeyManager.TryGetTempKey(principalKey, out tempKey))
              principalKey = !((EntityKey) null == tempKey) ? tempKey : throw EntityUtil.Update(System.Data.Entity.Resources.Strings.Update_AmbiguousForeignKey((object) referentialConstraint.ToRole.DeclaringType.FullName), (Exception) null, stateEntry);
            ...
            for (int count = referentialConstraint.FromProperties.Count; index < count; ++index)
            {
              ...
              if (currentValues && stateEntry1 != null && stateEntry1.State == EntityState.Deleted && (stateEntry.State == EntityState.Added || stateEntry.State == EntityState.Modified))
                throw EntityUtil.Update(System.Data.Entity.Resources.Strings.Update_InsertingOrUpdatingReferenceToDeletedEntity((object) associationSet.ElementType.FullName), (Exception) null, stateEntry, stateEntry1);
    ...
    internal virtual int Update()
    {
      ...
      try
      {
        foreach (UpdateCommand updateCommand in updateCommands)
          this.ValidateRowsAffected((source = updateCommand).Execute(identifierValues, generatedValues), source);
      }
      catch (Exception ex)
      {
        if (ex.RequiresContext())
          throw new UpdateException(System.Data.Entity.Resources.Strings.Update_GeneralExecutionException, ex, this.DetermineStateEntriesFromSource(source).Cast<ObjectStateEntry>().Distinct<ObjectStateEntry>());
        throw;
      }
    ...
    internal virtual async Task<int> UpdateAsync(CancellationToken cancellationToken)
    {
      ...
      try
      {
        foreach (UpdateCommand updateCommand in updateCommands)
        {
          ...
          this.ValidateRowsAffected(await updateCommand.ExecuteAsync(identifierValues, generatedValues, cancellationToken).WithCurrentCulture<long>(), source);
        }
      }
      catch (Exception ex)
      {
    ...
    private void ValidateRowsAffected(long rowsAffected, UpdateCommand source)
    {
      if (rowsAffected == 0L)
      {
        ...
        throw new OptimisticConcurrencyException(System.Data.Entity.Resources.Strings.Update_ConcurrencyError((object) rowsAffected), (Exception) null, entriesFromSource.Cast<ObjectStateEntry>().Distinct<ObjectStateEntry>());
      }
    }
    ...
    {
      return source == null ? Enumerable.Empty<System.Data.Entity.Core.IEntityStateEntry>() : (IEnumerable<System.Data.Entity.Core.IEntityStateEntry>) source.GetStateEntries(this);
    }
    ...
    private IEnumerable<EntitySetBase> GetDynamicModifiedExtents() => (IEnumerable<EntitySetBase>) this._changes.Keys;

    private IEnumerable<EntitySetBase> GetFunctionModifiedExtents() => (IEnumerable<EntitySetBase>) this._functionChanges.Keys;

    private IEnumerable<UpdateCommand> ProduceDynamicCommands()
    {
      UpdateTranslator updateTranslator = this;
      UpdateCompiler updateCompiler = new UpdateCompiler(updateTranslator);
      Set<EntitySet> set = new Set<EntitySet>();
      foreach (EntitySetBase dynamicModifiedExtent in updateTranslator.GetDynamicModifiedExtents())
      {
        Set<EntitySet> affectedTables = updateTranslator.ViewLoader.GetAffectedTables(dynamicModifiedExtent, updateTranslator.MetadataWorkspace);
        if (affectedTables.Count == 0)
          throw EntityUtil.Update(System.Data.Entity.Resources.Strings.Update_MappingNotFound((object) dynamicModifiedExtent.Name), (Exception) null);
        foreach (EntitySet element in affectedTables)
          set.Add(element);
      }
      foreach (EntitySet table in set)
      {
        DbQueryCommandTree cqtView = updateTranslator.Connection.GetMetadataWorkspace().GetCqtView((EntitySetBase) table);
        ChangeNode changeNode = Propagator.Propagate(updateTranslator, table, cqtView);
        foreach (UpdateCommand compileCommand in new TableChangeProcessor(table).CompileCommands(changeNode, updateCompiler))
      ...
      ModificationFunctionMapping functionMapping)
    {
      ...
      if (!this._modificationFunctionCommandDefinitions.TryGetValue(functionMapping, out commandDefinition))
      {
        ...
        this._modificationFunctionCommandDefinitions.Add(functionMapping, commandDefinition);
      }
    ...
    {
      UpdateTranslator translator1 = this;
      foreach (EntitySetBase functionModifiedExtent in translator1.GetFunctionModifiedExtents())
      {
        ModificationFunctionMappingTranslator translator = translator1.ViewLoader.GetFunctionMappingTranslator(functionModifiedExtent, translator1.MetadataWorkspace);
        if (translator != null)
        {
          foreach (ExtractedStateEntry functionModification in translator1.GetExtentFunctionModifications(functionModifiedExtent))
          {
            FunctionUpdateCommand functionUpdateCommand = translator.Translate(translator1, functionModification);
            if (functionUpdateCommand != null)
              yield return (UpdateCommand) functionUpdateCommand;
          }
        }
        translator = (ModificationFunctionMappingTranslator) null;
      }
    }
      ...
      IEnumerable<UpdateCommand> remainder)
    {
      ...
      throw new UpdateException(System.Data.Entity.Resources.Strings.Update_ConstraintCycle, (Exception) null, source.Cast<ObjectStateEntry>().Distinct<ObjectStateEntry>());
    }
    ...
    internal DbCommand CreateCommand(DbModificationCommandTree commandTree)
    {
      try
      {
        return (DbCommand) new InterceptableDbCommand(this._providerServices.CreateCommand((DbCommandTree) commandTree, this._interceptionContext), this._interceptionContext);
      }
      catch (Exception ex)
      {
        if (ex.RequiresContext())
    ...
    internal void SetParameterValue(DbParameter parameter, TypeUsage typeUsage, object value) => this._providerServices.SetParameterValue(parameter, typeUsage, value);
      ...
      foreach (KeyValuePair<EntityKey, AssociationSet> requiredEntity in this._requiredEntities)
      {
        ...
        if (!this._knownEntityKeys.Contains(key))
        {
          ...
          if (!this._stateManager.TryGetEntityStateEntry(key, out stateEntry) || stateEntry.IsKeyEntry)
            throw EntityUtil.Update(System.Data.Entity.Resources.Strings.Update_MissingEntity((object) requiredEntity.Value.Name, (object) TypeHelpers.GetFullName(key.EntityContainerName, key.EntitySetName)), (Exception) null);
    ...
    private static void VerifyExtent(MetadataWorkspace workspace, EntitySetBase extent)
    {
      ...
      if (entityContainer1 == null || entityContainer2 == null || entityContainer1 != entityContainer2)
        throw EntityUtil.Update(System.Data.Entity.Resources.Strings.Update_WorkspaceMismatch, (Exception) null);
    }
    ...
    private class RelationshipConstraintValidator
    {
      ...
      internal void RegisterEntity(System.Data.Entity.Core.IEntityStateEntry stateEntry)
      {
        ...
        foreach (AssociationSet referencingAssociationSet in this.GetReferencingAssociationSets(entitySet))
        {
          ...
          foreach (AssociationSetEnd associationSetEnd1 in associationSetEnds)
          {
            ...
            {
              if (associationSetEnd2.CorrespondingAssociationEndMember != associationSetEnd1.CorrespondingAssociationEndMember && associationSetEnd2.EntitySet.EdmEquals((MetadataItem) entitySet) && (MetadataHelper.GetLowerBoundOfMultiplicity(associationSetEnd1.CorrespondingAssociationEndMember.RelationshipMultiplicity) != 0 && MetadataHelper.GetEntityTypeForEnd(associationSetEnd2.CorrespondingAssociationEndMember).IsAssignableFrom((EdmType) entityType)))
                this.m_impliedRelationships.Add(new UpdateTranslator.RelationshipConstraintValidator.DirectionalRelationship(entityKey, associationSetEnd1.CorrespondingAssociationEndMember, associationSetEnd2.CorrespondingAssociationEndMember, referencingAssociationSet, stateEntry), stateEntry);
            }
          }
        }
      }

      private static EntityType GetEntityType(DbDataRecord dbDataRecord) => (EntityType) (dbDataRecord as IExtendedDataRecord).DataRecordInfo.RecordType.EdmType;
        ...
        System.Data.Entity.Core.IEntityStateEntry stateEntry)
      {
        ...
        foreach (AssociationSetEnd associationSetEnd1 in associationSetEnds)
        {
          foreach (AssociationSetEnd associationSetEnd2 in associationSetEnds)
          {
            if (associationSetEnd2.CorrespondingAssociationEndMember != associationSetEnd1.CorrespondingAssociationEndMember)
          ...
          if (1 == num && 1 == boundOfMultiplicity1)
          {
            ...
            if (1 == nullable.GetValueOrDefault() & nullable.HasValue)
            {
              ...
              if (!this.m_impliedRelationships.TryGetValue(key, out entityStateEntry) || flag && EntityState.Added != entityStateEntry.State || !flag && EntityState.Deleted != entityStateEntry.State)
                throw EntityUtil.Update(System.Data.Entity.Resources.Strings.Update_MissingRequiredEntity((object) key.AssociationSet.Name, (object) key.StateEntry.State, (object) key.ToEnd.Name), (Exception) null, key.StateEntry);
            }
          }
        }
      }
      ...
      private IEnumerable<AssociationSet> GetReferencingAssociationSets(
        EntitySet entitySet)
      {
        ...
        if (!this.m_referencingRelationshipSets.TryGetValue(entitySet, out associationSetList))
        {
          ...
          {
            if (baseEntitySet is AssociationSet associationSet3 && !associationSet3.ElementType.IsForeignKey)
            {
              foreach (AssociationSetEnd associationSetEnd in associationSet3.AssociationSetEnds)
              {
                if (associationSetEnd.EntitySet.Equals((object) entitySet))
                {
                  associationSetList.Add(associationSet3);
                  break;
                }
              }
            }
      ...
      private class DirectionalRelationship : 
        IEquatable<UpdateTranslator.RelationshipConstraintValidator.DirectionalRelationship>
      {
        ...
        internal IEnumerable<UpdateTranslator.RelationshipConstraintValidator.DirectionalRelationship> GetEquivalenceSet()
        {
          ...
          UpdateTranslator.RelationshipConstraintValidator.DirectionalRelationship current = directionalRelationship;
          do
          {
            yield return current;
            current = current._equivalenceSetLinkedListNext;
          }
          while (current != directionalRelationship);
        }
        ...
        public override bool Equals(object obj) => this.Equals(obj as UpdateTranslator.RelationshipConstraintValidator.DirectionalRelationship);
