...
namespace System.Data.Entity.Core.Mapping.Update.Internal
{
  internal class KeyManager
  {
    ...
    internal int GetCliqueIdentifier(int identifier)
    {
      ...
      return partition != null ? partition.PartitionId : identifier;
    }
    ...
    internal void RegisterIdentifierOwner(PropagatorResult owner) => this._identifiers[owner.Identifier].Owner = owner;
      ...
      foreach (int principal in this.GetPrincipals(identifier))
      {
        ...
        if (owner != null)
        {
          if (flag)
          {
            ...
            throw new ConstraintException(System.Data.Entity.Resources.Strings.Update_ReferentialConstraintIntegrityViolation);
        }
      }
      if (flag)
    ...
    internal IEnumerable<int> GetPrincipals(int identifier) => this.WalkGraph(identifier, (Func<KeyManager.IdentifierInfo, KeyManager.LinkedList<int>>) (info => info.References), true);

    internal IEnumerable<int> GetDirectReferences(int identifier)
    {
      foreach (int num in KeyManager.LinkedList<int>.Enumerate(this._identifiers[identifier].References))
        yield return num;
    }

    internal IEnumerable<int> GetDependents(int identifier) => this.WalkGraph(identifier, (Func<KeyManager.IdentifierInfo, KeyManager.LinkedList<int>>) (info => info.ReferencedBy), false);
      ...
      bool leavesOnly)
    {
      ...
      {
        int index = stack.Pop();
        KeyManager.LinkedList<int> current = successorFunction(this._identifiers[index]);
        if (current != null)
        {
          foreach (int num in KeyManager.LinkedList<int>.Enumerate(current))
            stack.Push(num);
          if (!leavesOnly)
            yield return index;
        }
    ...
    internal bool HasPrincipals(int identifier) => this._identifiers[identifier].References != null;
    ...
    internal bool TryGetTempKey(EntityKey valueKey, out EntityKey tempKey) => this._valueKeyToTempKey.TryGetValue(valueKey, out tempKey);
      ...
      foreach (int node1 in KeyManager.LinkedList<int>.Enumerate(this._identifiers[node].References))
      {
        switch (color[node1])
        {
          ...
          case 2:
            ...
            throw new UpdateException(System.Data.Entity.Resources.Strings.Update_CircularRelationships, (Exception) null, source.Cast<ObjectStateEntry>().Distinct<ObjectStateEntry>());
          default:
            continue;
        }
      }
    ...
    private sealed class LinkedList<T>
    {
      ...
      internal static void Add(ref KeyManager.LinkedList<T> list, T value) => list = new KeyManager.LinkedList<T>(value, list);
    }
