...
namespace System.Data.Entity.Core.Mapping.ViewGeneration
{
  internal class ViewgenContext : InternalBase
  {
    ...
    internal ViewTarget ViewTarget => this.m_viewTarget;

    internal MemberMaps MemberMaps => this.m_memberMaps;

    internal EntitySetBase Extent => this.m_extent;

    internal ConfigViewGenerator Config => this.m_config;

    internal CqlIdentifiers CqlIdentifiers => this.m_identifiers;

    internal EdmItemCollection EdmItemCollection => this.m_edmItemCollection;

    internal FragmentQueryProcessor LeftFragmentQP => this.m_leftFragmentQP;

    internal FragmentQueryProcessor RightFragmentQP => this.m_rightFragmentQP;

    internal List<LeftCellWrapper> AllWrappersForExtent => this.m_cellWrappers;

    internal EntityContainerMapping EntityContainerMapping => this.m_entityContainerMapping;

    internal bool TryGetCachedRewriting(FragmentQuery query, out Tile<FragmentQuery> rewriting) => this.m_rewritingCache.TryGetValue(query, out rewriting);

    internal void SetCachedRewriting(FragmentQuery query, Tile<FragmentQuery> rewriting) => this.m_rewritingCache[query] = rewriting;
    ...
    private void CheckConcurrencyControlTokens()
    {
      ...
      foreach (EdmMember member in forTypeHierarchy)
      {
        if (!member.DeclaringType.IsAssignableFrom((EdmType) elementType))
          ExceptionHelpers.ThrowMappingException(new ErrorLog.Record(ViewGenErrorCode.ConcurrencyDerivedClass, System.Data.Entity.Resources.Strings.ViewGen_Concurrency_Derived_Class((object) member.Name, (object) member.DeclaringType.Name, (object) this.m_extent), (IEnumerable<LeftCellWrapper>) this.m_cellWrappers, string.Empty), this.m_config);
      ...
      foreach (LeftCellWrapper cellWrapper in this.m_cellWrappers)
      {
        ...
        if (set.Count > 0)
        {
          ...
          stringBuilder.AppendLine(System.Data.Entity.Resources.Strings.ViewGen_Concurrency_Invalid_Condition((object) MemberPath.PropertiesToUserString((IEnumerable<MemberPath>) set, false), (object) this.m_extent.Name));
    ...
    internal override void ToCompactString(StringBuilder builder) => LeftCellWrapper.WrappersToStringBuilder(builder, this.m_cellWrappers, "Left Cell Wrappers");
    ...
    internal class OneToOneFkAssociationsForEntitiesFilter
    {
      ...
      {
        return associationSets.Where<AssociationSet>((Func<AssociationSet, bool>) (a => a.ElementType.IsForeignKey && a.ElementType.AssociationEndMembers.All<AssociationEndMember>((Func<AssociationEndMember, bool>) (aem => aem.RelationshipMultiplicity == RelationshipMultiplicity.One && entityTypes.Contains(aem.GetEntityType())))));
      }
    }
  }
}
