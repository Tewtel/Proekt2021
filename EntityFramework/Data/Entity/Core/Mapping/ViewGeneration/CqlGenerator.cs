...
namespace System.Data.Entity.Core.Mapping.ViewGeneration
{
  internal sealed class CqlGenerator : InternalBase
  {
    ...
    private int TotalSlots => this.m_projectedSlotMap.Count + this.m_numBools;
    ...
    internal DbQueryCommandTree GenerateCqt() => DbQueryCommandTree.FromValidExpression(this.m_mappingItemCollection.Workspace, DataSpace.SSpace, this.GenerateCqlBlockTree().AsCqt(true), true, false);
    ...
    private bool[] GetRequiredSlots()
    {
      ...
      foreach (CaseStatement caseStatement in this.m_caseStatements.Values)
      {
        if ((caseStatement.MemberPath.IsPartOfKey ? 0 : (!caseStatement.DependsOnMemberValue ? 1 : 0)) != 0)
    ...
    private MemberPath GetOutputMemberPath(int slotNum) => this.m_projectedSlotMap.GetMemberPath(slotNum, this.TotalSlots - this.m_projectedSlotMap.Count);

    private int GetSlotIndex(MemberPath member, EdmMember child) => this.m_projectedSlotMap.IndexOf(new MemberPath(member, child));
