...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.CqlGeneration
{
  internal abstract class CqlBlock : InternalBase
  {
    ...
    protected ReadOnlyCollection<CqlBlock> Children => this.m_children;

    protected BoolExpression WhereClause => this.m_whereClause;

    internal string CqlAlias => this.m_blockAlias;
    ...
    internal QualifiedSlot QualifySlotWithBlockAlias(int slotNum) => new QualifiedSlot(this, this.m_slots[slotNum].SlotValue);

    internal ProjectedSlot SlotValue(int slotNum) => this.m_slots[slotNum].SlotValue;

    internal System.Data.Entity.Core.Mapping.ViewGeneration.Structures.MemberPath MemberPath(
      int slotNum)
    {
      return this.m_slots[slotNum].OutputMember;
    }

    internal bool IsProjected(int slotNum) => this.m_slots[slotNum].IsProjected;
    ...
    protected DbExpression GenerateProjectionCqt(DbExpression row, bool isTopLevel) => isTopLevel ? this.Slots.Where<SlotInfo>((Func<SlotInfo, bool>) (slot => slot.IsRequiredByParent)).Single<SlotInfo>().AsCqt(row) : (DbExpression) DbExpressionBuilder.NewRow(this.Slots.Where<SlotInfo>((Func<SlotInfo, bool>) (slot => slot.IsRequiredByParent)).Select<SlotInfo, KeyValuePair<string, DbExpression>>((Func<SlotInfo, KeyValuePair<string, DbExpression>>) (slot => new KeyValuePair<string, DbExpression>(slot.CqlFieldAlias, slot.AsCqt(row)))));

    internal void SetJoinTreeContext(IList<string> parentQualifiers, string leafQualifier) => this.m_joinTreeContext = new CqlBlock.JoinTreeContext(parentQualifiers, leafQualifier);

    internal DbExpression GetInput(DbExpression row) => this.m_joinTreeContext == null ? row : this.m_joinTreeContext.FindInput(row);
