...
namespace System.Data.Entity.Core.Mapping.ViewGeneration
{
  internal class CellCreator : InternalBase
  {
    ...
    internal CqlIdentifiers Identifiers => this.m_identifiers;
    ...
    private void ExpandCells(List<Cell> cells)
    {
      ...
      {
        Cell cell = cell1;
        foreach (MemberPath memberPath in set1)
        {
          IEnumerable<MemberPath> elements = cell.SQuery.GetProjectedPositions(memberPath).Select<int, MemberPath>((Func<int, MemberPath>) (pos => ((MemberProjectedSlot) cell.CQuery.ProjectedSlotAt(pos)).MemberPath));
      ...
      foreach (Cell originalCell in cells.ToArray())
      {
        foreach (MemberPath memberPath in set1)
        {
          Set<MemberPath> set2 = dictionary[memberPath];
          if (originalCell.SQuery.GetProjectedMembers().Contains<MemberPath>(memberPath))
          {
            Cell result = (Cell) null;
            if (this.TryCreateAdditionalCellWithCondition(originalCell, memberPath, true, ViewTarget.UpdateView, out result))
              cells.Add(result);
            if (this.TryCreateAdditionalCellWithCondition(originalCell, memberPath, false, ViewTarget.UpdateView, out result))
              cells.Add(result);
          }
          else
          {
            foreach (MemberPath memberToExpand in originalCell.CQuery.GetProjectedMembers().Intersect<MemberPath>((IEnumerable<MemberPath>) set2))
            {
              Cell result = (Cell) null;
              if (this.TryCreateAdditionalCellWithCondition(originalCell, memberToExpand, true, ViewTarget.QueryView, out result))
                cells.Add(result);
              if (this.TryCreateAdditionalCellWithCondition(originalCell, memberToExpand, false, ViewTarget.QueryView, out result))
                cells.Add(result);
            }
          }
        }
      ...
      List<Cell> cells)
    {
      ...
      CellQuery cQuery = new CellQuery(projectedSlotList1, literal, memberPath1, CellQuery.SelectDistinct.No);
      CellQuery cellQuery = new CellQuery(projectedSlotList2, sQueryWhereClause, memberPath2, fragmentMap.IsSQueryDistinct ? CellQuery.SelectDistinct.Yes : CellQuery.SelectDistinct.No);
      CellLabel cellLabel = new CellLabel(fragmentMap);
      CellQuery sQuery = cellQuery;
      CellLabel label = cellLabel;
      int currentCellNumber = this.m_currentCellNumber;
      Cell cs = Cell.CreateCS(cQuery, sQuery, label, currentCellNumber);
      ++this.m_currentCellNumber;
      cells.Add(cs);
    }
    ...
    private static bool IsBooleanMember(MemberPath path) => path.EdmType is PrimitiveType edmType && edmType.PrimitiveTypeKind == PrimitiveTypeKind.Boolean;

    internal override void ToCompactString(StringBuilder builder) => builder.Append(nameof (CellCreator));
  }
}
