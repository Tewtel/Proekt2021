...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.QueryRewriting
{
  internal class QueryRewriter
  {
    ...
    internal ViewgenContext ViewgenContext => this._context;

    internal Dictionary<MemberPath, CaseStatement> CaseStatements => this._caseStatements;

    internal BoolExpression TopLevelWhereClause => this._topLevelWhereClause;

    internal CellTreeNode BasicView => this._basicView.MakeCopy();

    internal List<LeftCellWrapper> UsedCells => this._usedCells;

    private IEnumerable<FragmentQuery> FragmentQueries => (IEnumerable<FragmentQuery>) this._fragmentQueries;
        ...
        if (domainValue != Constant.Undefined)
        {
          ...
          if (!this.FindRewritingAndUsedViews((IEnumerable<MemberPath>) this._keyAttributes, and, outputUsedViews, out rewriting))
          {
            if (!ErrorPatternMatcher.FindMappingErrors(this._context, this._domainMap, this._errorLog))
            {
              ...
              {
                string viewGenExtent = System.Data.Entity.Resources.Strings.ViewGen_Extent;
                builder.AppendLine(System.Data.Entity.Resources.Strings.ViewGen_Cannot_Recover_Types((object) viewGenExtent, (object) str));
              }
              else
              {
                string viewGenEntities = System.Data.Entity.Resources.Strings.ViewGen_Entities;
                builder.AppendLine(System.Data.Entity.Resources.Strings.ViewGen_Cannot_Disambiguate_MultiConstant((object) viewGenEntities, (object) str));
              }
          ...
          else if (domainValue is TypeConstant typeConstant)
          {
      ...
      IEnumerable<MemberPath> nonConditionalScalarAttributes)
    {
      ...
      foreach (MemberPath conditionalScalarAttribute in nonConditionalScalarAttributes)
      {
        ...
        stringList.Add(leafEdmMember.DeclaringType.Name + "." + leafEdmMember?.ToString());
      }
      return stringList;
    }
      ...
      ErrorLog errorLog)
    {
      ...
      builder.AppendLine(System.Data.Entity.Resources.Strings.ViewGen_Cannot_Recover_Attributes((object) commaSeparatedString, (object) viewGenExtent, (object) str));
        ...
        foreach (Constant domainValue in list)
        {
          ...
          else
          {
            ...
            else if (!QueryRewriter.IsDefaultValue(domainValue, member) && !ErrorPatternMatcher.FindMappingErrors(this._context, this._domainMap, this._errorLog))
            {
              ...
              string str2 = this._context.ViewTarget == ViewTarget.QueryView ? System.Data.Entity.Resources.Strings.ViewGen_Entities : System.Data.Entity.Resources.Strings.ViewGen_Tuples;
              if (this._context.ViewTarget == ViewTarget.QueryView)
                builder.AppendLine(System.Data.Entity.Resources.Strings.Viewgen_CannotGenerateQueryViewUnderNoValidation((object) str1));
              else
                builder.AppendLine(System.Data.Entity.Resources.Strings.ViewGen_Cannot_Disambiguate_MultiConstant((object) str2, (object) str1));
      ...
      Tile<FragmentQuery> unionCaseRewriting)
    {
      ...
      else
      {
        ...
        stringBuilder.AppendLine(System.Data.Entity.Resources.Strings.ViewGen_No_Default_Value_For_Configuration((object) currentPath.PathToString(new bool?(false))));
    ...
    internal void EnsureExtentIsFullyMapped(HashSet<FragmentQuery> outputUsedViews)
    {
      ...
      else
      {
        if (this._config.IsValidationEnabled)
        {
          ...
          {
            if (member.IsScalarType() && !member.IsPartOfKey && (!this._domainMap.IsConditionMember(member) && !Domain.TryGetDefaultValueForMemberPath(member, out Constant _)))
            {
              ...
              {
                FragmentQuery fragmentQuery = leftCellWrapper.FragmentQuery;
                FragmentQuery query = new FragmentQuery(fragmentQuery.Description, fragmentQuery.FromVariable, (IEnumerable<MemberPath>) memberPathSet, fragmentQuery.Condition);
                Tile<FragmentQuery> tile = (Tile<FragmentQuery>) QueryRewriter.CreateTile(FragmentQuery.Create((IEnumerable<MemberPath>) this._keyAttributes, BoolExpression.CreateNot(fragmentQuery.Condition)));
                if (!this.RewriteQuery((Tile<FragmentQuery>) QueryRewriter.CreateTile(query), tile, out Tile<FragmentQuery> _, out IEnumerable<MemberPath> _, false))
        ...
        foreach (Tile<FragmentQuery> view in this._views)
        {
          ...
          Tile<FragmentQuery> rewriting;
          if (!this.RewriteQuery(toFill, tile, out rewriting, out IEnumerable<MemberPath> _, true))
          {
            ...
            this._errorLog.AddEntry(new ErrorLog.Record(ViewGenErrorCode.ImpossibleCondition, System.Data.Entity.Resources.Strings.Viewgen_QV_RewritingNotFound((object) leftCellWrapper.RightExtent.ToString()), leftCellWrapper.Cells, string.Empty));
          }
          else
    ...
    {
      return this.FindRewritingAndUsedViews(attributes, whereClause, outputUsedViews, out rewriting, out IEnumerable<MemberPath> _);
    }
      ...
      out Tile<FragmentQuery> rewriting)
    {
      if (this._context.TryGetCachedRewriting(toFill.Query, out rewriting))
        return true;
      int num = this._qp.RewriteQuery(toFill, toAvoid, views, out rewriting) ? 1 : 0;
      if (num == 0)
        return num != 0;
    ...
    private static TileNamed<FragmentQuery> CreateTile(FragmentQuery query) => new TileNamed<FragmentQuery>(query);
    ...
    private static BoolExpression TileToBoolExpr(Tile<FragmentQuery> tile)
    {
      switch (tile.OpKind)
      {
        ...
        case TileOpKind.Named:
          ...
          return namedQuery.Condition.IsAlwaysTrue() ? BoolExpression.True : namedQuery.FromVariable;
        default:
    ...
    private static bool IsDefaultValue(Constant domainValue, MemberPath path)
    {
      if (domainValue.IsNull() && path.IsNullable)
        return true;
      return path.DefaultValue != null && (domainValue as ScalarConstant).Value == path.DefaultValue;
    }

    private static Set<MemberPath> GetVariables(FragmentQuery query) => new Set<MemberPath>(query.Condition.VariableConstraints.Where<DomainConstraint<BoolLiteral, Constant>>((Func<DomainConstraint<BoolLiteral, Constant>, bool>) (domainConstraint => domainConstraint.Variable.Identifier is MemberRestriction && !domainConstraint.Variable.Domain.All<Constant>((Func<Constant, bool>) (constant => domainConstraint.Range.Contains(constant))))).Select<DomainConstraint<BoolLiteral, Constant>, MemberPath>((Func<DomainConstraint<BoolLiteral, Constant>, MemberPath>) (domainConstraint => ((MemberRestriction) domainConstraint.Variable.Identifier).RestrictedMemberSlot.MemberPath)), MemberPath.EqualityComparer);

    private bool IsTrue(FragmentQuery query) => !this._context.LeftFragmentQP.IsSatisfiable(FragmentQuery.Create(BoolExpression.CreateNot(query.Condition)));

    [Conditional("DEBUG")]
    private void PrintStatistics(RewritingProcessor<Tile<FragmentQuery>> qp) => qp.GetStatistics(out int _, out int _, out int _, out int _, out int _);
