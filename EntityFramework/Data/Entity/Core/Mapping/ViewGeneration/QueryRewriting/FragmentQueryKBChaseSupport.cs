...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.QueryRewriting
{
  internal class FragmentQueryKBChaseSupport : FragmentQueryKB
  {
    ...
    internal FragmentQueryKBChaseSupport() => this._chase = new FragmentQueryKBChaseSupport.AtomicConditionRuleChase(this);
    ...
    internal BoolExpr<DomainConstraint<BoolLiteral, Constant>> Chase(
      BoolExpr<DomainConstraint<BoolLiteral, Constant>> expression)
    {
      return this.Implications.Count != 0 ? this._chase.Chase(FragmentQueryKBChaseSupport.Normalizer.ToNnfAndSplitRange(expression)) : expression;
    }
      ...
      BoolExpr<DomainConstraint<BoolLiteral, Constant>> implies)
    {
      ...
      bool flag = true;
      foreach (TermExpr<DomainConstraint<BoolLiteral, Constant>> key in new Set<TermExpr<DomainConstraint<BoolLiteral, Constant>>>((IEnumerable<TermExpr<DomainConstraint<BoolLiteral, Constant>>>) this.Implications.Keys))
      {
        BoolExpr<DomainConstraint<BoolLiteral, Constant>> expr2 = queryKbChaseSupport.Chase(this.Implications[key]);
        ...
        this.Implications[key] = new Converter<DomainConstraint<BoolLiteral, Constant>>(expr2, IdentifierService<DomainConstraint<BoolLiteral, Constant>>.Instance.CreateConversionContext()).Dnf.Expr;
      }
      if (flag)
    ...
    private static class Normalizer
    {
      ...
      private class NegatedTreeVisitor : 
        Visitor<DomainConstraint<BoolLiteral, Constant>, BoolExpr<DomainConstraint<BoolLiteral, Constant>>>
      {
      ...
      private class NegatedNnfSplitCountEstimator : 
        TermCounter<DomainConstraint<BoolLiteral, Constant>>
      {
      ...
      private class DnfTreeVisitor : BasicVisitor<DomainConstraint<BoolLiteral, Constant>>
      {
          ...
          AndExpr<DomainConstraint<BoolLiteral, Constant>> expression)
        {
          ...
          {
            if (child is OrExpr<DomainConstraint<BoolLiteral, Constant>> orExpr1)
              source.Add(new Set<BoolExpr<DomainConstraint<BoolLiteral, Constant>>>(orExpr1.Children));
            else
              set.Add(child);
          }
    ...
    private class AtomicConditionRuleChase
    {
      ...
      private class NonNegatedDomainConstraintTreeVisitor : 
        BasicVisitor<DomainConstraint<BoolLiteral, Constant>>
      {
