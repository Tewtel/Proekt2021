...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.QueryRewriting
{
    ...
    where T_Tile : class
  {
    ...
    public RewritingProcessor(System.Data.Entity.Core.Mapping.ViewGeneration.QueryRewriting.TileProcessor<T_Tile> tileProcessor) => this.m_tileProcessor = tileProcessor;

    internal System.Data.Entity.Core.Mapping.ViewGeneration.QueryRewriting.TileProcessor<T_Tile> TileProcessor => this.m_tileProcessor;
    ...
    internal override T_Tile GetArg1(T_Tile tile) => this.m_tileProcessor.GetArg1(tile);

    internal override T_Tile GetArg2(T_Tile tile) => this.m_tileProcessor.GetArg2(tile);

    internal override TileOpKind GetOpKind(T_Tile tile) => this.m_tileProcessor.GetOpKind(tile);
    ...
    public bool IsDisjointFrom(T_Tile a, T_Tile b) => this.m_tileProcessor.IsEmpty(this.Join(a, b));

    internal bool IsContainedIn(T_Tile a, T_Tile b) => this.IsEmpty(this.AntiSemiJoin(a, b));

    internal bool IsEquivalentTo(T_Tile a, T_Tile b) => this.IsContainedIn(a, b) & this.IsContainedIn(b, a);
    ...
    public void AddError() => ++this.m_numErrors;
    ...
    public static IEnumerable<IEnumerable<T>> Permute<T>(IEnumerable<T> list)
    {
      ...
      foreach (T obj in list)
      {
        ...
        foreach (IEnumerable<T> rest1 in RewritingProcessor<T_Tile>.Permute<T>(rest))
          yield return RewritingProcessor<T_Tile>.Concat<T>(value, rest1);
        value = default (T);
      }
      if (rest == null)
        yield return list;
