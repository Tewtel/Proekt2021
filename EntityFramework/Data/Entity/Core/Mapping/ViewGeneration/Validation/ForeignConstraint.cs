...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Validation
{
  internal class ForeignConstraint : InternalBase
  {
    ...
    internal EntitySet ParentTable => this.m_parentTable;

    internal EntitySet ChildTable => this.m_childTable;

    internal IEnumerable<MemberPath> ChildColumns => (IEnumerable<MemberPath>) this.m_childColumns;

    internal IEnumerable<MemberPath> ParentColumns => (IEnumerable<MemberPath>) this.m_parentColumns;
      ...
      EntityContainer container)
    {
      ...
      {
        if (baseEntitySet is AssociationSet i_fkeySet1)
        {
          Dictionary<string, EntitySet> dictionary = new Dictionary<string, EntitySet>();
          foreach (AssociationSetEnd associationSetEnd in i_fkeySet1.AssociationSetEnds)
            dictionary.Add(associationSetEnd.Name, associationSetEnd.EntitySet);
          foreach (ReferentialConstraint referentialConstraint in i_fkeySet1.ElementType.ReferentialConstraints)
          {
            ...
            ForeignConstraint foreignConstraint = new ForeignConstraint(i_fkeySet1, i_parentTable, i_childTable, referentialConstraint.FromProperties, referentialConstraint.ToProperties);
      ...
      ConfigViewGenerator config)
    {
      ...
      {
        ErrorLog.Record record = new ErrorLog.Record(ViewGenErrorCode.ForeignKeyMissingTableMapping, System.Data.Entity.Resources.Strings.ViewGen_Foreign_Key_Missing_Table_Mapping((object) this.ToUserString(), (object) this.ChildTable.Name), (IEnumerable<LeftCellWrapper>) parentRewriter.UsedCells, string.Empty);
        errorLog.AddEntry(record);
      }
      else if (parentRewriter == null)
      {
        ErrorLog.Record record = new ErrorLog.Record(ViewGenErrorCode.ForeignKeyMissingTableMapping, System.Data.Entity.Resources.Strings.ViewGen_Foreign_Key_Missing_Table_Mapping((object) this.ToUserString(), (object) this.ParentTable.Name), (IEnumerable<LeftCellWrapper>) childRewriter.UsedCells, string.Empty);
        errorLog.AddEntry(record);
      }
      else
      {
      ...
      ErrorLog errorLog)
    {
      ...
      if (FragmentQueryProcessor.Merge(viewgenContext1.RightFragmentQP, viewgenContext2.RightFragmentQP).IsContainedIn(basicView1.RightFragmentQuery, basicView2.RightFragmentQuery))
        return;
      string message = System.Data.Entity.Resources.Strings.ViewGen_Foreign_Key_Not_Guaranteed_InCSpace((object) this.ToUserString());
      ...
      ConfigViewGenerator config)
    {
      ...
      foreach (Cell cell in cells)
      {
        if (cell.SQuery.Extent.Equals((object) this.ChildTable))
        {
          ...
          if (relationEndForColumns == null || this.CheckParentColumnsForForeignKey(cell, cells, relationEndForColumns, ref errorList))
          {
            ...
            {
              flag1 = ForeignConstraint.CheckConstraintWhenOnlyParentMapped((AssociationSet) cell.CQuery.Extent, relationEndForColumns, childRewriter, parentRewriter);
              if (flag1)
                break;
            }
          }
      ...
      else
      {
        if (flag1)
          return;
        string message = System.Data.Entity.Resources.Strings.ViewGen_Foreign_Key_Missing_Relationship_Mapping((object) this.ToUserString());
        List<LeftCellWrapper> wrappersFromContext1 = ForeignConstraint.GetWrappersFromContext(viewgenContext2, (EntitySetBase) this.ParentTable);
        ...
        Set<LeftCellWrapper> set = new Set<LeftCellWrapper>((IEnumerable<LeftCellWrapper>) wrappersFromContext1);
      ...
      ConfigViewGenerator config)
    {
      ...
      {
        ErrorLog.Record record = new ErrorLog.Record(ViewGenErrorCode.ForeignKeyUpperBoundMustBeOne, System.Data.Entity.Resources.Strings.ViewGen_Foreign_Key_UpperBound_MustBeOne((object) this.ToUserString(), (object) cell.CQuery.Extent.Name, (object) parentEnd.Name), cell, string.Empty);
        errorLog.AddEntry(record);
        flag = false;
      }
      if (!MemberPath.AreAllMembersNullable(this.ChildColumns) && parentEnd.RelationshipMultiplicity != RelationshipMultiplicity.One)
      {
        ErrorLog.Record record = new ErrorLog.Record(ViewGenErrorCode.ForeignKeyLowerBoundMustBeOne, System.Data.Entity.Resources.Strings.ViewGen_Foreign_Key_LowerBound_MustBeOne((object) this.ToUserString(), (object) cell.CQuery.Extent.Name, (object) parentEnd.Name), cell, string.Empty);
        errorLog.AddEntry(record);
        flag = false;
      }
      ...
      IEnumerable<MemberPath> columns)
    {
      ...
      if (cell.CQuery.Extent is AssociationSet extent)
      {
        foreach (AssociationSetEnd associationSetEnd in extent.AssociationSetEnds)
        {
