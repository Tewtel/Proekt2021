  ...
  internal class RewritingValidator
  {
      ...
      foreach (LeftCellWrapper leftCellWrapper in this._viewgenContext.AllWrappersForExtent)
      {
        ...
        if (cellTreeNode2 != null)
        {
          ...
          if (!this.CheckEquivalence(cellTreeNode1.RightFragmentQuery, sQueryTree.RightFragmentQuery, literal, out unsatisfiedConstraint))
          {
            ...
            this.ReportConstraintViolation(System.Data.Entity.Resources.Strings.ViewGen_CQ_PartitionConstraint((object) str), unsatisfiedConstraint, ViewGenErrorCode.PartitionConstraintViolation, cellTreeNode1.GetLeaves().Concat<LeftCellWrapper>((IEnumerable<LeftCellWrapper>) sQueryTree.GetLeaves()));
          }
      ...
      bool complementElse)
    {
      ...
      foreach (MemberPath conditionMember in this._domainMap.ConditionMembers(this._viewgenContext.Extent))
      {
        ...
        for (int index = 0; index < constantList.Count; ++index)
        {
          Constant domainValue = constantList[index];
          RewritingValidator.MemberValueBinding key = new RewritingValidator.MemberValueBinding(conditionMember, domainValue);
          ...
          if (this._viewgenContext.TryGetCachedRewriting(QueryRewriter.CreateMemberConditionQuery(conditionMember, domainValue, this._keyAttributes, this._domainMap), out rewriting))
          {
            ...
            dictionary[key] = cellTree;
            if (index < constantList.Count - 1)
        ...
        if (complementElse && constantList.Count > 1)
        {
          ...
          RewritingValidator.MemberValueBinding key = new RewritingValidator.MemberValueBinding(conditionMember, constant);
          dictionary[key] = (CellTreeNode) new OpCellTreeNode(this._viewgenContext, CellTreeOpType.LASJ, new CellTreeNode[2]
          {
            this._basicView,
      ...
      BoolExpression inExtentCondition)
    {
      foreach (MemberPath conditionMember in this._domainMap.ConditionMembers(this._viewgenContext.Extent))
      {
        ...
        if (wrapper.RightCellQuery.ProjectedSlotAt(slotNum) is MemberProjectedSlot memberProjectedSlot2)
        {
          foreach (Constant constant in this._domainMap.GetDomain(conditionMember))
          {
            ...
            if (memberValueTrees.TryGetValue(new RewritingValidator.MemberValueBinding(conditionMember, constant), out cellTreeNode1))
            {
              ...
              if (!this.CheckEquivalence(cQuery, cellTreeNode3.RightFragmentQuery, inExtentCondition, out unsatisfiedConstraint))
                this.ReportConstraintViolation(System.Data.Entity.Resources.Strings.ViewGen_CQ_DomainConstraint((object) memberProjectedSlot2.ToUserString()), unsatisfiedConstraint, ViewGenErrorCode.DomainConstraintViolation, cellTreeNode3.GetLeaves().Concat<LeftCellWrapper>((IEnumerable<LeftCellWrapper>) new LeftCellWrapper[1]
                {
                  wrapper
                }));
            }
      ...
      foreach (MemberPath nonConditionMember in this._domainMap.NonConditionMembers(this._viewgenContext.Extent))
      {
        ...
        if (!nonConditionMember.IsNullable & flag)
        {
          ...
          if (query != null && this._viewgenContext.RightFragmentQP.IsSatisfiable(query))
            this._errorLog.AddEntry(new ErrorLog.Record(ViewGenErrorCode.NullableMappingForNonNullableColumn, System.Data.Entity.Resources.Strings.Viewgen_NullableMappingForNonNullableColumn((object) wrapper.LeftExtent.ToString(), (object) nonConditionMember.ToFullString()), wrapper.Cells, ""));
        }
      }
    }
    ...
    private class WhereClauseVisitor : Visitor<DomainConstraint<BoolLiteral, Constant>, CellTreeNode>
    {
      ...
      internal CellTreeNode GetCellTreeNode(BoolExpression whereClause) => whereClause.Tree.Accept<CellTreeNode>((Visitor<DomainConstraint<BoolLiteral, Constant>, CellTreeNode>) this);
        ...
        AndExpr<DomainConstraint<BoolLiteral, Constant>> expression)
      {
        ...
        return opCellTreeNode.Children.Count != 0 ? (CellTreeNode) opCellTreeNode : this._topLevelTree;
      }
      ...
      {
        RewritingValidator.WhereClauseVisitor whereClauseVisitor = this;
        foreach (BoolExpr<DomainConstraint<BoolLiteral, Constant>> child in children)
          yield return child.Accept<CellTreeNode>((Visitor<DomainConstraint<BoolLiteral, Constant>, CellTreeNode>) whereClauseVisitor);
      }
    }
    ...
    internal class DomainConstraintVisitor : CellTreeNode.SimpleCellTreeVisitor<bool, bool>
    {
      ...
      internal override bool VisitLeaf(LeafCellTreeNode node, bool dummy)
      {
        ...
        {
          for (int index = 0; index < rightCellQuery1.NumProjectedSlots; ++index)
          {
            if (rightCellQuery1.ProjectedSlotAt(index) is MemberProjectedSlot memberProjectedSlot5 && rightCellQuery2.ProjectedSlotAt(index) is MemberProjectedSlot memberProjectedSlot6)
            {
              MemberPath projectedSlot = this.m_viewgenContext.MemberMaps.ProjectedSlotMap[index];
              if (!projectedSlot.IsPartOfKey && !MemberPath.EqualityComparer.Equals(memberProjectedSlot5.MemberPath, memberProjectedSlot6.MemberPath))
                memberPathList.Add(projectedSlot);
            }
          }
        }
        if (memberPathList.Count > 0)
          this.m_errorLog.AddEntry(new ErrorLog.Record(ViewGenErrorCode.NonKeyProjectedWithOverlappingPartitions, System.Data.Entity.Resources.Strings.ViewGen_NonKeyProjectedWithOverlappingPartitions((object) MemberPath.PropertiesToUserString((IEnumerable<MemberPath>) memberPathList, false)), (IEnumerable<LeftCellWrapper>) new LeftCellWrapper[2]
          {
            this.m_wrapper,
    ...
    private struct MemberValueBinding : IEquatable<RewritingValidator.MemberValueBinding>
    {
      ...
      public override string ToString() => string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0}={1}", (object) this.Member, (object) this.Value);

      public bool Equals(RewritingValidator.MemberValueBinding other) => MemberPath.EqualityComparer.Equals(this.Member, other.Member) && Constant.EqualityComparer.Equals(this.Value, other.Value);
    }
  }
}
