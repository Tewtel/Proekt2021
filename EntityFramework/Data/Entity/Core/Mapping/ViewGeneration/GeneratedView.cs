  ...
  internal sealed class GeneratedView : InternalBase
  {
      ...
      out GeneratedView generatedView)
    {
      ...
      else
      {
        ...
        if (!(commandTree.Query.ResultType.EdmType is CollectionType edmType2) || !setMapping.Set.ElementType.IsAssignableFrom(edmType2.TypeUsage.EdmType))
        {
    ...
    internal string eSQL => this.m_eSQL;
      ...
      Command targetIqtCommand)
    {
      if (this.m_internalTreeNode == null)
      {
        ...
        System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert(command.Root.Op.OpType == OpType.PhysicalProject, "Expected a physical projectOp at the root of the tree - found " + command.Root.Op.OpType.ToString());
    ...
    {
      int num = this.m_type != this.m_extent.ElementType ? 1 : 0;
      if (num != 0)
