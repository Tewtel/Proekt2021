...
namespace System.Data.Entity.Core.Mapping.ViewGeneration
{
  internal class CellTreeSimplifier : InternalBase
  {
    ...
    private CellTreeSimplifier(ViewgenContext context) => this.m_viewgenContext = context;

    internal static CellTreeNode MergeNodes(CellTreeNode rootNode) => new CellTreeSimplifier(rootNode.ViewgenContext).SimplifyTreeByMergingNodes(rootNode);
      ...
      List<CellTreeNode> nodes)
    {
      ...
      {
        if (!(node is OpCellTreeNode opCellTreeNode2))
          return (Set<LeafCellTreeNode>) null;
        if (cellTreeOpType == CellTreeOpType.Leaf)
          cellTreeOpType = opCellTreeNode2.OpType;
        else if (!CellTreeNode.IsAssociativeOp(opCellTreeNode2.OpType) || cellTreeOpType != opCellTreeNode2.OpType)
          return (Set<LeafCellTreeNode>) null;
        Set<LeafCellTreeNode> other = new Set<LeafCellTreeNode>(LeafCellTreeNode.EqualityComparer);
        foreach (CellTreeNode child in opCellTreeNode2.Children)
        {
          if (!(child is LeafCellTreeNode element4))
            return (Set<LeafCellTreeNode>) null;
          other.Add(element4);
        }
        if (set == null)
      ...
      return set.Count == 0 ? (Set<LeafCellTreeNode>) null : set;
    }
      ...
      List<CellTreeNode> nodes)
    {
      ...
      foreach (CellTreeNode node in nodes)
      {
        if (node is LeafCellTreeNode leafCellTreeNode1)
          keyToListMap.Add(leafCellTreeNode1.LeftCellWrapper.RightCellQuery.Extent, (CellTreeNode) leafCellTreeNode1);
        else
      ...
      List<CellTreeNode> nodes)
    {
      ...
      for (int index = 1; index < nodes.Count; ++index)
      {
        ...
        if (node is LeafCellTreeNode leafCellTreeNode2)
          keyToListMap.Add(leafCellTreeNode2.LeftCellWrapper.RightCellQuery.Extent, (CellTreeNode) leafCellTreeNode2);
        else
          cellTreeNodeList2.Add(node);
      }
      if (nodes[0] is LeafCellTreeNode node1)
      {
    ...
    internal override void ToCompactString(StringBuilder builder) => this.m_viewgenContext.MemberMaps.ProjectedSlotMap.ToCompactString(builder);
  }
}
