...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Structures
{
  internal class CellQuery : InternalBase
  {
    ...
    internal CellQuery.SelectDistinct SelectDistinctFlag => this.m_selectDistinct;

    internal EntitySetBase Extent => this.m_extentMemberPath.Extent;

    internal int NumProjectedSlots => this.m_projectedSlots.Length;

    internal ProjectedSlot[] ProjectedSlots => this.m_projectedSlots;

    internal List<BoolExpression> BoolVars => this.m_boolExprs;

    internal int NumBoolVars => this.m_boolExprs.Count;

    internal BoolExpression WhereClause => this.m_whereClause;

    internal MemberPath SourceExtentMemberPath => this.m_extentMemberPath;

    internal BasicCellRelation BasicCellRelation => this.m_basicCellRelation;

    internal IEnumerable<MemberRestriction> Conditions => this.GetConjunctsFromOriginalWhereClause();

    internal ProjectedSlot ProjectedSlotAt(int slotNum) => this.m_projectedSlots[slotNum];
    ...
    internal ErrorLog.Record CheckForDuplicateFields(CellQuery cQuery, Cell sourceCell)
    {
      ...
      foreach (MemberProjectedSlot key in keyToListMap.Keys)
      {
        ...
        if (cSideSlotIndexes.Count > 1 && !cQuery.AreSlotsEquivalentViaRefConstraints(cSideSlotIndexes))
        {
          ...
          {
            stringBuilder1 = new StringBuilder(System.Data.Entity.Resources.Strings.ViewGen_Duplicate_CProperties((object) this.Extent.Name));
          ...
          stringBuilder1.AppendLine(System.Data.Entity.Resources.Strings.ViewGen_Duplicate_CProperties_IsMapped((object) key.ToUserString(), (object) stringBuilder2.ToString()));
        }
      }
      return !flag ? (ErrorLog.Record) null : new ErrorLog.Record(ViewGenErrorCode.DuplicateCPropertiesMapped, stringBuilder1.ToString(), sourceCell, string.Empty);
    }

    private bool AreSlotsEquivalentViaRefConstraints(ReadOnlyCollection<int> cSideSlotIndexes) => this.Extent is AssociationSet && cSideSlotIndexes.Count <= 2 && ((MemberProjectedSlot) this.m_projectedSlots[cSideSlotIndexes[0]]).MemberPath.IsEquivalentViaRefConstraint(((MemberProjectedSlot) this.m_projectedSlots[cSideSlotIndexes[1]]).MemberPath);
      ...
      foreach (MemberRestriction condition in this.Conditions)
      {
        if (condition.Domain.ContainsNotNull() && MemberProjectedSlot.GetSlotForMember((IEnumerable<ProjectedSlot>) this.m_projectedSlots, condition.RestrictedMemberSlot.MemberPath) == null)
        {
          ...
          if (this.Extent is EntitySet)
          {
            ...
            if (rightExtent != null)
            {
              foreach (AssociationSet associationSet in (cellQuery.Extent as EntitySet).AssociationSets.Where<AssociationSet>((Func<AssociationSet, bool>) (association => association.AssociationSetEnds.Any<AssociationSetEnd>((Func<AssociationSetEnd, bool>) (end => end.CorrespondingAssociationEndMember.RelationshipMultiplicity == RelationshipMultiplicity.One && MetadataHelper.GetOppositeEnd(end).EntitySet.EdmEquals((MetadataItem) rightExtent))))))
              {
          ...
          if (flag2)
          {
            stringBuilder.AppendLine(System.Data.Entity.Resources.Strings.ViewGen_NotNull_No_Projected_Slot((object) condition.RestrictedMemberSlot.MemberPath.PathToString(new bool?(false))));
            flag1 = true;
          }
        }
      }
      return !flag1 ? (ErrorLog.Record) null : new ErrorLog.Record(ViewGenErrorCode.NotNullNoProjectedSlot, stringBuilder.ToString(), sourceCell, string.Empty);
    }
    ...
    {
      ProjectedSlot[] projectedSlotArray = this.m_projectedSlots;
      for (int index = 0; index < projectedSlotArray.Length; ++index)
      {
        if (projectedSlotArray[index] is MemberProjectedSlot memberProjectedSlot1)
          yield return memberProjectedSlot1;
      }
      projectedSlotArray = (ProjectedSlot[]) null;
    }
    ...
    internal List<int> GetProjectedPositions(MemberPath member)
    {
      ...
      for (int index = 0; index < this.m_projectedSlots.Length; ++index)
      {
        if (this.m_projectedSlots[index] is MemberProjectedSlot projectedSlot1 && MemberPath.EqualityComparer.Equals(member, projectedSlot1.MemberPath))
          intList.Add(index);
      }
      return intList;
    }
    ...
    internal List<int> GetAssociationEndSlots(AssociationEndMember endMember)
    {
      ...
      for (int index = 0; index < this.m_projectedSlots.Length; ++index)
      {
        if (this.m_projectedSlots[index] is MemberProjectedSlot projectedSlot1 && projectedSlot1.MemberPath.RootEdmMember.Equals((object) endMember))
          intList.Add(index);
      }
      return intList;
    }
    ...
    internal void UpdateWhereClause(MemberDomainMap domainMap)
    {
      ...
      foreach (BoolExpression atom in this.WhereClause.Atoms)
      {
        ...
        int num = !(asLiteral is ScalarRestriction scalarRestriction2) || scalarRestriction2.Domain.Contains(Constant.Null) || scalarRestriction2.Domain.Contains(Constant.NotNull) ? 0 : (!scalarRestriction2.Domain.Contains(Constant.Undefined) ? 1 : 0);
        if (num != 0)
    ...
    internal BoolExpression GetBoolVar(int varNum) => this.m_boolExprs[varNum];
    ...
    internal IEnumerable<MemberRestriction> GetConjunctsFromWhereClause() => CellQuery.GetConjunctsFromWhereClause(this.m_whereClause);

    internal IEnumerable<MemberRestriction> GetConjunctsFromOriginalWhereClause() => CellQuery.GetConjunctsFromWhereClause(this.m_originalWhereClause);
    ...
    internal void GetIdentifiers(CqlIdentifiers identifiers)
    {
      foreach (ProjectedSlot projectedSlot in this.m_projectedSlots)
      {
        if (projectedSlot is MemberProjectedSlot memberProjectedSlot1)
          memberProjectedSlot1.MemberPath.GetIdentifiers(identifiers);
      }
