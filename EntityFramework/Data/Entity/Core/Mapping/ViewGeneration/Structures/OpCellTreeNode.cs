...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Structures
{
  internal class OpCellTreeNode : CellTreeNode
  {
    ...
    internal override CellTreeOpType OpType => this.m_opType;
    ...
    internal override MemberDomainMap RightDomainMap => this.m_children[0].RightDomainMap;

    internal override Set<MemberPath> Attributes => this.m_attrs;

    internal override List<CellTreeNode> Children => this.m_children;

    internal override int NumProjectedSlots => this.m_children[0].NumProjectedSlots;

    internal override int NumBoolSlots => this.m_children[0].NumBoolSlots;
    ...
    internal void Add(CellTreeNode child) => this.Insert(this.m_children.Count, child);

    internal void AddFirst(CellTreeNode child) => this.Insert(0, child);
      ...
      ref List<WithRelationship> withRelationships)
    {
      ...
      foreach (CellTreeNode child in this.Children)
      {
        ...
        {
          if (requiredSlots[index] && !projectedSlots[index])
          {
            if (this.IsBoolSlot(index))
            {
              slotInfoArray[index] = new SlotInfo(true, true, (ProjectedSlot) new BooleanProjectedSlot(BoolExpression.False, identifiers, this.SlotToBoolIndex(index)), (MemberPath) null);
            }
            else
            {
              MemberPath outputMember = cqlBlock.MemberPath(index);
              slotInfoArray[index] = new SlotInfo(true, true, (ProjectedSlot) new ConstantProjectedSlot(Constant.Null), outputMember);
            }
          }
          else
            slotInfoArray[index] = cqlBlock.Slots[index];
        }
      ...
      for (int index3 = 1; index3 < children.Count; ++index3)
      {
        ...
        foreach (int keySlot in this.KeySlots)
        {
          if (!this.ViewgenContext.Config.IsValidationEnabled && (!cqlBlock.IsProjected(keySlot) || !children[0].IsProjected(keySlot)))
          {
            ...
            errorLog.AddEntry(new ErrorLog.Record(ViewGenErrorCode.NoJoinKeyOrFKProvidedInMapping, System.Data.Entity.Resources.Strings.Viewgen_NoJoinKeyOrFK, (IEnumerable<LeftCellWrapper>) this.ViewgenContext.AllWrappersForExtent, string.Empty));
