...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Structures
{
  internal class BoolExpression : InternalBase
  {
    ...
    internal BoolExpression Create(BoolLiteral literal) => new BoolExpression(literal.GetDomainBoolExpression(this.m_memberDomainMap), this.m_memberDomainMap);

    internal static BoolExpression CreateNot(BoolExpression expression) => new BoolExpression(ExprType.Not, (IEnumerable<BoolExpression>) new BoolExpression[1]
    {
      expression
    });

    internal static BoolExpression CreateAnd(params BoolExpression[] children) => new BoolExpression(ExprType.And, (IEnumerable<BoolExpression>) children);

    internal static BoolExpression CreateOr(params BoolExpression[] children) => new BoolExpression(ExprType.Or, (IEnumerable<BoolExpression>) children);

    internal static BoolExpression CreateAndNot(BoolExpression e1, BoolExpression e2) => BoolExpression.CreateAnd(e1, BoolExpression.CreateNot(e2));
    ...
    internal IEnumerable<BoolExpression> Atoms
    {
      get
      {
        foreach (BoolExpr<DomainConstraint<BoolLiteral, Constant>> term in BoolExpression.TermVisitor.GetTerms(this.m_tree, false))
          yield return new BoolExpression(term, this.m_memberDomainMap);
      }
    }

    internal BoolLiteral AsLiteral => !(this.m_tree is TermExpr<DomainConstraint<BoolLiteral, Constant>> tree) ? (BoolLiteral) null : BoolExpression.GetBoolLiteral(tree);
    ...
    internal bool IsTrue => this.m_tree.ExprType == ExprType.True;

    internal bool IsFalse => this.m_tree.ExprType == ExprType.False;
    ...
    internal bool IsSatisfiable() => !this.IsUnsatisfiable();
    ...
    internal BoolExpr<DomainConstraint<BoolLiteral, Constant>> Tree => this.m_tree;

    internal IEnumerable<DomainConstraint<BoolLiteral, Constant>> VariableConstraints => LeafVisitor<DomainConstraint<BoolLiteral, Constant>>.GetLeaves(this.m_tree);

    internal IEnumerable<DomainVariable<BoolLiteral, Constant>> Variables => this.VariableConstraints.Select<DomainConstraint<BoolLiteral, Constant>, DomainVariable<BoolLiteral, Constant>>((Func<DomainConstraint<BoolLiteral, Constant>, DomainVariable<BoolLiteral, Constant>>) (domainConstraint => domainConstraint.Variable));
    ...
    internal IEnumerable<MemberRestriction> MemberRestrictions
    {
      get
      {
        ...
        {
          if (variable.Identifier is MemberRestriction identifier1)
    ...
    internal StringBuilder AsEsql(StringBuilder builder, string blockAlias) => BoolExpression.AsEsqlVisitor.AsEsql(this.m_tree, builder, blockAlias);

    internal DbExpression AsCqt(DbExpression row) => BoolExpression.AsCqtVisitor.AsCqt(this.m_tree, row);
      ...
      bool writeRoundtrippingMessage)
    {
      ...
      {
        builder.AppendLine(System.Data.Entity.Resources.Strings.Viewgen_ConfigurationErrorMsg((object) blockAlias));
        builder.Append("  ");
      }
      return BoolExpression.AsUserStringVisitor.AsUserString(this.m_tree, builder, blockAlias);
    }

    internal override void ToCompactString(StringBuilder builder) => BoolExpression.CompactStringVisitor.ToBuilder(this.m_tree, builder);

    internal BoolExpression RemapBool(Dictionary<MemberPath, MemberPath> remap) => new BoolExpression(BoolExpression.RemapBoolVisitor.RemapExtentTreeNodes(this.m_tree, this.m_memberDomainMap, remap), this.m_memberDomainMap);
    ...
    internal BoolExpression MakeCopy() => this.Create(this.m_tree.Accept<BoolExpr<DomainConstraint<BoolLiteral, Constant>>>((Visitor<DomainConstraint<BoolLiteral, Constant>, BoolExpr<DomainConstraint<BoolLiteral, Constant>>>) BoolExpression._copyVisitorInstance));
    ...
    private bool IsFinal() => this.m_memberDomainMap != null && BoolExpression.IsFinalVisitor.IsFinal(this.m_tree);
    ...
    private class BoolComparer : IEqualityComparer<BoolExpression>
    {
      public bool Equals(BoolExpression left, BoolExpression right)
      {
        if (left == right)
          return true;
        return left != null && right != null && left.m_tree.Equals(right.m_tree);
      }

      public int GetHashCode(BoolExpression expression) => expression.m_tree.GetHashCode();
    }
    ...
    private class IsFinalVisitor : Visitor<DomainConstraint<BoolLiteral, Constant>, bool>
    {
      ...
      internal override bool VisitTerm(
        TermExpr<DomainConstraint<BoolLiteral, Constant>> expression)
      {
        return !(BoolExpression.GetBoolLiteral(expression) is MemberRestriction boolLiteral) || boolLiteral.IsComplete;
      }
        ...
        TreeExpr<DomainConstraint<BoolLiteral, Constant>> expression)
      {
        ...
        foreach (BoolExpr<DomainConstraint<BoolLiteral, Constant>> child in expression.Children)
        {
          switch (child)
          {
            case FalseExpr<DomainConstraint<BoolLiteral, Constant>> _:
            case TrueExpr<DomainConstraint<BoolLiteral, Constant>> _:
              continue;
            default:
    ...
    private sealed class AsCqtVisitor : BoolExpression.AsCqlVisitor<DbExpression>
    {
      ...
      private AsCqtVisitor(DbExpression row) => this.m_row = row;
    ...
    private abstract class AsCqlVisitor<T_Return> : 
      Visitor<DomainConstraint<BoolLiteral, Constant>, T_Return>
    {
      private bool m_skipIsNotNull;

      protected AsCqlVisitor() => this.m_skipIsNotNull = true;
    ...
    private class AsUserStringVisitor : 
      Visitor<DomainConstraint<BoolLiteral, Constant>, StringBuilder>
    {
      private readonly StringBuilder m_builder;
      private readonly string m_blockAlias;
      private bool m_skipIsNotNull;
        ...
        TermExpr<DomainConstraint<BoolLiteral, Constant>> expression)
      {
        ...
        switch (boolLiteral)
        {
          case ScalarRestriction _:
          case TypeRestriction _:
            return boolLiteral.AsUserString(this.m_builder, System.Data.Entity.Resources.Strings.ViewGen_EntityInstanceToken, this.m_skipIsNotNull);
          default:
      ...
      {
        this.m_skipIsNotNull = false;
        if (expression.Child is TermExpr<DomainConstraint<BoolLiteral, Constant>> child)
    ...
    private class TermVisitor : 
      Visitor<DomainConstraint<BoolLiteral, Constant>, IEnumerable<TermExpr<DomainConstraint<BoolLiteral, Constant>>>>
    {
      ...
      private IEnumerable<TermExpr<DomainConstraint<BoolLiteral, Constant>>> VisitTreeNode(
        TreeExpr<DomainConstraint<BoolLiteral, Constant>> expression)
      {
        BoolExpression.TermVisitor termVisitor = this;
        foreach (BoolExpr<DomainConstraint<BoolLiteral, Constant>> child in expression.Children)
        {
          foreach (TermExpr<DomainConstraint<BoolLiteral, Constant>> termExpr in child.Accept<IEnumerable<TermExpr<DomainConstraint<BoolLiteral, Constant>>>>((Visitor<DomainConstraint<BoolLiteral, Constant>, IEnumerable<TermExpr<DomainConstraint<BoolLiteral, Constant>>>>) termVisitor))
      ...
      Visitor<DomainConstraint<BoolLiteral, Constant>, StringBuilder>
    {
      ...
      private CompactStringVisitor(StringBuilder builder) => this.m_builder = builder;
