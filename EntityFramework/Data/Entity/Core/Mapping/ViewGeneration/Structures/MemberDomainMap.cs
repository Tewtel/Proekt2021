...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Structures
{
  internal class MemberDomainMap : InternalBase
  {
        ...
        foreach (MemberRestriction memberRestriction in extentCell.GetLeftQuery(viewTarget).GetConjunctsFromWhereClause())
        {
          ...
          if (set.Contains(Constant.Null) || !memberRestriction.Domain.Values.All<Constant>((Func<Constant, bool>) (conditionConstant => conditionConstant.Equals((object) Constant.NotNull))))
          {
            if (set.Count <= 0 || !set.Contains(Constant.Null) && memberRestriction.Domain.Values.Contains<Constant>(Constant.Null))
              ExceptionHelpers.ThrowMappingException(new ErrorLog.Record(ViewGenErrorCode.InvalidCondition, System.Data.Entity.Resources.Strings.ViewGen_InvalidCondition((object) memberPath.PathToString(new bool?(false))), extentCell, string.Empty), config);
    ...
    internal bool IsProjectedConditionMember(MemberPath memberPath) => this.m_projectedConditionMembers.Contains(memberPath);
    ...
    internal MemberDomainMap MakeCopy() => new MemberDomainMap(this.m_conditionDomainMap.ToDictionary<KeyValuePair<MemberPath, Set<Constant>>, MemberPath, Set<Constant>>((Func<KeyValuePair<MemberPath, Set<Constant>>, MemberPath>) (p => p.Key), (Func<KeyValuePair<MemberPath, Set<Constant>>, Set<Constant>>) (p => new Set<Constant>((IEnumerable<Constant>) p.Value, Constant.EqualityComparer))), this.m_nonConditionDomainMap, this.m_edmItemCollection);

    internal void ExpandDomainsToIncludeAllPossibleValues() => this.ExpandDomainsIfNeeded(this.m_conditionDomainMap);
      ...
      foreach (Cell cell in cells)
      {
        ...
        for (int slotNum = 0; slotNum < cquery.NumProjectedSlots; ++slotNum)
        {
          ...
          if (memberProjectedSlot1 != null && memberProjectedSlot2 != null)
          {
            ...
            queryDomainMap.GetDomainInternal(memberPath1).Unite(updateDomainMap.GetDomainInternal(memberPath2).Where<Constant>((Func<Constant, bool>) (constant => !constant.IsNull() && !(constant is NegatedConstant))));
    ...
    private static void ExpandNegationsInDomainMap(Dictionary<MemberPath, Set<Constant>> domainMap)
    {
      foreach (MemberPath key in domainMap.Keys.ToArray<MemberPath>())
        domainMap[key] = Domain.ExpandNegationsInDomain((IEnumerable<Constant>) domainMap[key]);
    }

    internal bool IsConditionMember(MemberPath path) => this.m_conditionDomainMap.ContainsKey(path);
    ...
    internal void AddSentinel(MemberPath path) => this.GetDomainInternal(path).Add(Constant.AllOtherConstants);

    internal void RemoveSentinel(MemberPath path) => this.GetDomainInternal(path).Remove(Constant.AllOtherConstants);

    internal IEnumerable<Constant> GetDomain(MemberPath path) => (IEnumerable<Constant>) this.GetDomainInternal(path);
