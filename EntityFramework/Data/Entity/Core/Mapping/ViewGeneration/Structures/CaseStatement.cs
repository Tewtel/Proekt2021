...
namespace System.Data.Entity.Core.Mapping.ViewGeneration.Structures
{
  internal sealed class CaseStatement : InternalBase
  {
    ...
    internal List<CaseStatement.WhenThen> Clauses => this.m_clauses;

    internal ProjectedSlot ElseValue => this.m_elseValue;
    ...
    internal IEnumerable<EdmType> InstantiatedTypes
    {
      get
      {
        ...
        EdmType type1;
        if (CaseStatement.TryGetInstantiatedType(this.m_elseValue, out type1))
          yield return type1;
      }
    }
    ...
    {
      type = (EdmType) null;
      if (!(slot is ConstantProjectedSlot constantProjectedSlot) || !(constantProjectedSlot.CellConstant is TypeConstant cellConstant))
        return false;
    ...
    internal void Simplify()
    {
      ...
      foreach (CaseStatement.WhenThen clause in this.m_clauses)
      {
        if (clause.Value is ConstantProjectedSlot constantProjectedSlot1 && (constantProjectedSlot1.CellConstant.IsNull() || constantProjectedSlot1.CellConstant.IsUndefined()))
        {
          flag = true;
        }
        else
      ...
      IEnumerable<WithRelationship> withRelationships)
    {
      ...
      return this.Clauses.Count > 0 ? (DbExpression) DbExpressionBuilder.Case((IEnumerable<DbExpression>) dbExpressionList1, (IEnumerable<DbExpression>) dbExpressionList2, elseExpression) : elseExpression;
    }
    ...
    internal sealed class WhenThen : InternalBase
    {
      ...
      internal BoolExpression Condition => this.m_condition;

      internal ProjectedSlot Value => this.m_value;

      internal CaseStatement.WhenThen ReplaceWithQualifiedSlot(CqlBlock block) => new CaseStatement.WhenThen(this.m_condition, this.m_value.DeepQualify(block));
