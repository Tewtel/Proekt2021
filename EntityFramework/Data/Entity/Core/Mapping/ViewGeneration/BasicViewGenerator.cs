...
namespace System.Data.Entity.Core.Mapping.ViewGeneration
{
  internal class BasicViewGenerator : InternalBase
  {
    ...
    private FragmentQueryProcessor LeftQP => this.m_viewgenContext.LeftFragmentQP;
    ...
    private CellTreeNode ConvertUnionsToNormalizedLOJs(CellTreeNode rootNode)
    {
      ...
      {
        if (cellTreeNode is OpCellTreeNode opCellTreeNode4 && opCellTreeNode4.OpType == CellTreeOpType.Union)
          cellTreeNodeList.AddRange((IEnumerable<CellTreeNode>) opCellTreeNode4.Children);
        else
      ...
      {
        if (child is LeafCellTreeNode leaf1)
        {
          EntitySetBase leafNodeTable = (EntitySetBase) BasicViewGenerator.GetLeafNodeTable(leaf1);
          if (leafNodeTable != null)
            keyToListMap1.Add((EntitySet) leafNodeTable, leaf1);
        }
      ...
      EntitySet principal)
    {
      foreach (Tuple<AssociationSet, ReferentialConstraint> foreignKeyPrincipal in principal.ForeignKeyPrincipals)
      {
        ...
        {
          int index = 0;
          while (index < keyMembers.Count && keyMembers[index].EdmEquals((MetadataItem) toProperties[index]))
            ++index;
          if (index == keyMembers.Count)
    ...
    private static EntitySet GetLeafNodeTable(LeafCellTreeNode leaf) => leaf.LeftCellWrapper.RightCellQuery.Extent as EntitySet;
    ...
    private bool IsDisjoint(CellTreeNode n1, CellTreeNode n2)
    {
      ...
      if (!(this.m_viewgenContext.ViewTarget == ViewTarget.UpdateView & flag) || rightFragmentQuery)
        return flag | rightFragmentQuery;
      if (ErrorPatternMatcher.FindMappingErrors(this.m_viewgenContext, this.m_domainMap, this.m_errorLog))
        return false;
      StringBuilder builder = new StringBuilder(System.Data.Entity.Resources.Strings.Viewgen_RightSideNotDisjoint((object) this.m_viewgenContext.Extent.ToString()));
      builder.AppendLine();
    ...
    private bool IsEquivalentTo(CellTreeNode n1, CellTreeNode n2) => this.IsContainedIn(n1, n2) && this.IsContainedIn(n2, n1);

    internal override void ToCompactString(StringBuilder builder) => this.m_projectedSlotMap.ToCompactString(builder);
  }
}
