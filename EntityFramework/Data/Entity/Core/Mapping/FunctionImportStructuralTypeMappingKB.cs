  ...
  internal sealed class FunctionImportStructuralTypeMappingKB
  {
      ...
      string sourceLocation)
    {
      ...
      foreach (KeyValuePair<EntityType, List<LineInfo>> keyValuePair in unreachableEntityTypes.KeyValuePairs)
      {
        ...
        EdmSchemaError edmSchemaError = new EdmSchemaError(System.Data.Entity.Resources.Strings.Mapping_FunctionImport_UnreachableType((object) keyValuePair.Key.FullName, (object) commaSeparatedString), 2076, EdmSchemaErrorSeverity.Error, sourceLocation, lineInfo.LineNumber, lineInfo.LinePosition);
      ...
      foreach (KeyValuePair<EntityType, List<LineInfo>> keyValuePair in unreachableIsTypeOfs.KeyValuePairs)
      {
        LineInfo lineInfo = keyValuePair.Value.First<LineInfo>();
        string commaSeparatedString = StringUtil.ToCommaSeparatedString((IEnumerable) keyValuePair.Value.Select<LineInfo, int>((Func<LineInfo, int>) (li => li.LineNumber)));
        EdmSchemaError edmSchemaError = new EdmSchemaError(System.Data.Entity.Resources.Strings.Mapping_FunctionImport_UnreachableIsTypeOf((object) ("IsTypeOf(" + keyValuePair.Key.FullName + ")"), (object) commaSeparatedString), 2076, EdmSchemaErrorSeverity.Error, sourceLocation, lineInfo.LineNumber, lineInfo.LinePosition);
        errors.Add(edmSchemaError);
        flag = false;
      }
      return flag;
      ...
      Vertex[] mappingConditions)
    {
      ...
      {
        if (!converter.Solver.And(((IEnumerable<Vertex>) vertexArray).Select<Vertex, Vertex>((Func<Vertex, int, Vertex>) ((typeCondition, ordinal) => ordinal != i ? converter.Solver.Not(typeCondition) : typeCondition))).IsZero())
