...
namespace System.Data.Entity.Core.Mapping
{
  ...
  public sealed class FunctionImportMappingComposable : FunctionImportMapping
  {
    ...
    {
      System.Data.Entity.Utilities.Check.NotNull<FunctionImportResultMapping>(resultMapping, nameof (resultMapping));
      ...
      if (!functionImport.IsComposableAttribute)
        throw new ArgumentException(System.Data.Entity.Resources.Strings.NonComposableFunctionCannotBeMappedAsComposable((object) nameof (functionImport)));
      if (!targetFunction.IsComposableAttribute)
        throw new ArgumentException(System.Data.Entity.Resources.Strings.NonComposableFunctionCannotBeMappedAsComposable((object) nameof (targetFunction)));
      EdmType returnType;
      if (!MetadataHelper.TryGetFunctionImportReturnType<EdmType>(functionImport, 0, out returnType))
        throw new ArgumentException(System.Data.Entity.Resources.Strings.InvalidReturnTypeForComposableFunction);
      ...
      if (tvfReturnType1 == null)
        throw new ArgumentException(System.Data.Entity.Resources.Strings.Mapping_FunctionImport_ResultMapping_InvalidSType((object) functionImport.Identity), nameof (functionImport));
      ...
      this._containerMapping = mapping != null ? mapping._containerMapping : throw new InvalidOperationException(parsingErrors.Count > 0 ? parsingErrors[0].Message : string.Empty);
      ...
      : base(functionImport, targetFunction)
    {
      ...
      if (!TypeSemantics.IsScalarType(returnType) && (structuralTypeMappings == null || structuralTypeMappings.Count == 0))
        throw new ArgumentException(System.Data.Entity.Resources.Strings.StructuralTypeMappingsMustNotBeNullForFunctionImportsReturningNonScalarValues);
    ...
    public FunctionImportResultMapping ResultMapping => this._resultMapping;
    ...
    internal ReadOnlyCollection<Tuple<StructuralType, List<ConditionPropertyMapping>, List<PropertyMapping>>> StructuralTypeMappings => this.m_structuralTypeMappings != null ? new ReadOnlyCollection<Tuple<StructuralType, List<ConditionPropertyMapping>, List<PropertyMapping>>>((IList<Tuple<StructuralType, List<ConditionPropertyMapping>, List<PropertyMapping>>>) this.m_structuralTypeMappings) : (ReadOnlyCollection<Tuple<StructuralType, List<ConditionPropertyMapping>, List<PropertyMapping>>>) null;

    internal EdmProperty[] TvfKeys => this.m_targetFunctionKeys;
      ...
      IList<System.Data.Entity.Core.Query.InternalTrees.Node> targetIqtArguments)
    {
      if (this.m_internalTreeNode == null)
      {
        ...
        System.Data.Entity.Core.Query.PlanCompiler.PlanCompiler.Assert(root.Op.OpType == OpType.PhysicalProject, "Expected a physical projectOp at the root of the tree - found " + root.Op.OpType.ToString());
    ...
    {
      FunctionImportMappingComposable mappingComposable = this;
      foreach (FunctionParameter parameter in mappingComposable.TargetFunction.Parameters)
      {
        FunctionParameter targetParameter = parameter;
        FunctionParameter functionParameter = mappingComposable.FunctionImport.Parameters.Single<FunctionParameter>((Func<FunctionParameter, bool>) (p => p.Name == targetParameter.Name));
        yield return (DbExpression) mappingComposable.m_commandParameters[mappingComposable.FunctionImport.Parameters.IndexOf(functionParameter)];
      }
    }
      ...
      out DiscriminatorMap discriminatorMap)
    {
      ...
      if (this.m_structuralTypeMappings.Count == 1)
      {
        ...
        List<PropertyMapping> propertyMappingList = structuralTypeMapping.Item3;
        ...
        List<PropertyMapping> propertyMappings = propertyMappingList;
        DbVariableReferenceExpression variable = input.Variable;
        DbExpression structuralTypeMappingView = FunctionImportMappingComposable.GenerateStructuralTypeMappingView(structuralType, propertyMappings, (DbExpression) variable);
      ...
      DbExpression row)
    {
      ...
      return !condition.IsNull.Value ? (DbExpression) columnRef.IsNull().Not() : (DbExpression) columnRef.IsNull();
    }
      ...
      DbExpression storeFunctionInvoke)
    {
      ...
      Func<DbExpression, DbExpression> scalarView = (Func<DbExpression, DbExpression>) (row =>
      {
        ...
        return TypeSemantics.IsEqual(functionImportReturnType.TypeUsage, column.TypeUsage) ? (DbExpression) propertyExpression : (DbExpression) propertyExpression.CastTo(functionImportReturnType.TypeUsage);
      });
    ...
    private sealed class FunctionViewOpCopier : OpCopier
    {
        ...
        System.Data.Entity.Core.Query.InternalTrees.Node n)
      {
        ...
        return op.Var.VarType == VarType.Parameter && this.m_viewArguments.TryGetValue(((ParameterVar) op.Var).ParameterName, out n1) ? OpCopier.Copy(this.m_destCmd, n1) : base.Visit(op, n);
      }
    }
  }
}
