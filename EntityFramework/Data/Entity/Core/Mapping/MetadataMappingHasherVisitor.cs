...
namespace System.Data.Entity.Core.Mapping
{
  internal class MetadataMappingHasherVisitor : BaseMetadataMappingVisitor
  {
    ...
    protected override void Visit(PropertyMapping propertyMapping) => base.Visit(propertyMapping);
    ...
    protected override void Visit(EntitySetBase entitySetBase) => base.Visit(entitySetBase);
    ...
    protected override void Visit(EdmType edmType) => base.Visit(edmType);
    ...
    internal string HashValue => this.m_hashSourceBuilder.ComputeHash();
    ...
    private void AddSeenObjectToHashBuilder(int instanceIndex) => this.m_hashSourceBuilder.AppendLine("Instance Reference: " + instanceIndex.ToString());

    private void AddObjectStartDumpToHashBuilder(object o, int objectIndex) => this.m_hashSourceBuilder.AppendObjectStartDump(o, objectIndex);

    private void AddObjectEndDumpToHashBuilder() => this.m_hashSourceBuilder.AppendObjectEndDump();

    private void AddObjectContentToHashBuilder(object content)
    {
      if (content != null)
      {
        if (content is IFormattable formattable2)
          this.m_hashSourceBuilder.AppendLine(formattable2.ToString((string) null, (IFormatProvider) CultureInfo.InvariantCulture));
        else
