...
namespace Newtonsoft.Json.Schema
{
  ...
  internal class JsonSchemaWriter
  {
    ...
    private void WriteType(string propertyName, JsonWriter writer, JsonSchemaType type)
    {
      ...
      {
        IEnumerator<JsonSchemaType> enumerator = EnumUtils.GetFlagsValues<JsonSchemaType>(type).Where<JsonSchemaType>((Func<JsonSchemaType, bool>) (v => (uint) v > 0U)).GetEnumerator();
