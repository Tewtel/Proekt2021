...
namespace Newtonsoft.Json.Converters
{
  ...
  public class XmlNodeConverter : JsonConverter
  {
        ...
        default:
          ...
          for (int index1 = 0; index1 < node.ChildNodes.Count; ++index1)
          {
            ...
            else
            {
              ...
              {
                if (!(obj is List<IXmlNode> xmlNodeList7))
                {
                  xmlNodeList7 = new List<IXmlNode>()
                  {
                    (IXmlNode) obj
                  };
                  dictionary[propertyName2] = (object) xmlNodeList7;
                }
                xmlNodeList7.Add(childNode);
              }
            }
          }
          if (dictionary == null)
          ...
          using (Dictionary<string, object>.Enumerator enumerator = dictionary.GetEnumerator())
          {
            while (enumerator.MoveNext())
            {
              ...
              if (current.Value is List<IXmlNode> groupedNodes5)
                this.WriteGroupedNodes(writer, manager, writePropertyName, groupedNodes5, current.Key);
              else
    ...
    private static string? ConvertTokenToXmlValue(JsonReader reader)
    {
      ...
      {
        case JsonToken.Integer:
          return reader.Value is BigInteger bigInteger2 ? bigInteger2.ToString((IFormatProvider) CultureInfo.InvariantCulture) : XmlConvert.ToString(Convert.ToInt64(reader.Value, (IFormatProvider) CultureInfo.InvariantCulture));
        case JsonToken.Float:
          if (reader.Value is Decimal num3)
            return XmlConvert.ToString(num3);
          return reader.Value is float num4 ? XmlConvert.ToString(num4) : XmlConvert.ToString(Convert.ToDouble(reader.Value, (IFormatProvider) CultureInfo.InvariantCulture));
        case JsonToken.String:
        ...
        case JsonToken.Null:
          return (string) null;
        case JsonToken.Date:
          if (reader.Value is DateTimeOffset dateTimeOffset2)
            return XmlConvert.ToString(dateTimeOffset2);
      ...
      XmlNamespaceManager manager)
    {
      ...
      {
        if (childNode is IXmlElement element3 && element3.LocalName == propertyName)
        {
          this.AddJsonArrayAttribute(element3, document);
          break;
        }
      }
    }
        ...
        switch (reader.TokenType)
        {
          case JsonToken.PropertyName:
            ...
            if (!StringUtils.IsNullOrEmpty(str1))
            {
              ...
              {
                case '$':
                  if (str1 != null && (str1 == "$values" || str1 == "$id" || (str1 == "$ref" || str1 == "$type") || str1 == "$value"))
                  {
            ...
            if (reader.TokenType == JsonToken.StartArray)
            {
              ...
              if (num == 1 && this.WriteArrayAttribute)
              {
                ...
                using (List<IXmlNode>.Enumerator enumerator = currentNode.ChildNodes.GetEnumerator())
                {
                  while (enumerator.MoveNext())
                  {
                    if (enumerator.Current is IXmlElement current9 && current9.LocalName == localName && current9.NamespaceUri == str2)
                    {
                      this.AddJsonArrayAttribute(current9, document);
                      break;
                    }
                  }
                  break;
