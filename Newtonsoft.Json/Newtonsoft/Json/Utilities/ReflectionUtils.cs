  ...
  internal static class ReflectionUtils
  {
      ...
      BindingFlags bindingAttr)
    {
      ...
      foreach (IGrouping<string, MemberInfo> source2 in source1.GroupBy<MemberInfo, string>((Func<MemberInfo, string>) (m => m.Name)))
      {
        ...
        {
          List<MemberInfo> source3 = new List<MemberInfo>();
          foreach (MemberInfo memberInfo1 in (IEnumerable<MemberInfo>) source2)
          {
            MemberInfo memberInfo = memberInfo1;
            if (source3.Count == 0)
              source3.Add(memberInfo);
            else if ((!ReflectionUtils.IsOverridenGenericMember(memberInfo, bindingAttr) || memberInfo.Name == "Item") && !source3.Any<MemberInfo>((Func<MemberInfo, bool>) (m => m.DeclaringType == memberInfo.DeclaringType)))
              source3.Add(memberInfo);
          }
          memberInfoList.AddRange((IEnumerable<MemberInfo>) source3);
        }
      }
      ...
      bool inherit)
    {
      ...
      if ((object) element1 == null)
      {
        ...
        if ((object) element2 == null)
        {
          ...
          {
            if (obj is ParameterInfo element10)
              return !(attributeType != (Type) null) ? Attribute.GetCustomAttributes(element10, inherit) : Attribute.GetCustomAttributes(element10, attributeType, inherit);
      ...
      BindingFlags bindingAttr)
    {
      ...
      {
        IEnumerable<FieldInfo> fieldInfos = ((IEnumerable<FieldInfo>) targetType.GetFields(bindingAttr1)).Where<FieldInfo>((Func<FieldInfo, bool>) (f => f.IsPrivate));
          ...
          else
          {
            ...
            if (initialProperties.IndexOf<PropertyInfo>((Func<PropertyInfo, bool>) (p =>
            {
              if (!(p.Name == subTypeProperty.Name) || !p.IsVirtual())
                return false;
              Type declaringType = p.GetBaseDefinition()?.DeclaringType;
              if ((object) declaringType == null)
                declaringType = p.DeclaringType;
              return declaringType.IsAssignableFrom(subTypePropertyDeclaringType);
            })) == -1)
              initialProperties.Add(subTypeProperty);
          }
        }
