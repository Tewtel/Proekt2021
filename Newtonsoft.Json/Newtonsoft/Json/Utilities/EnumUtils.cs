...
namespace Newtonsoft.Json.Utilities
{
  internal static class EnumUtils
  {
      ...
      bool disallowNumber)
    {
      ...
      ulong num1 = 0;
      int num2;
      for (int index2 = index1; index2 <= value.Length; index2 = num2 + 1)
      {
        num2 = value.IndexOf(',', index2);
        if (num2 == -1)
          num2 = value.Length;
        int num3 = num2;
        while (index2 < num2 && char.IsWhiteSpace(value[index2]))
          ++index2;
        while (num3 > index2 && char.IsWhiteSpace(value[num3 - 1]))
          --num3;
        int valueSubstringLength = num3 - index2;
        int? nullable = EnumUtils.MatchName(value, names, resolvedNames, index2, valueSubstringLength, StringComparison.Ordinal);
        if (!nullable.HasValue)
          nullable = EnumUtils.MatchName(value, names, resolvedNames, index2, valueSubstringLength, StringComparison.OrdinalIgnoreCase);
        if (!nullable.HasValue)
        {
