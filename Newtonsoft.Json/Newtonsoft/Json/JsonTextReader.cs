...
label_8:
          ...
          return true;
label_9:
          await jsonTextReader.ParseCommentAsync(!ignoreComments, cancellationToken).ConfigureAwait(false);
        }
...
label_17:
      if (!jsonTextReader.SupportMultipleContent || jsonTextReader.Depth != 0)
        throw JsonReaderException.Create((JsonReader) jsonTextReader, "After parsing a value an unexpected character was encountered: {0}.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) c));
      jsonTextReader.SetStateBasedOnCurrent();
      return false;
    }
...
label_35:
      ...
      CancellationToken cancellationToken)
    {
      ...
      do
      {
        char ch1 = jsonTextReader._chars[charPos++];
        if (ch1 <= '\r')
        {
        ...
        else if (ch1 != '"' && ch1 != '\'')
        {
          if (ch1 == '\\')
          {
            ...
            char ch2 = jsonTextReader._chars[charPos];
            ++charPos;
            char writeChar;
            switch (ch2)
            {
              ...
              default:
                jsonTextReader._charPos = charPos;
                throw JsonReaderException.Create((JsonReader) jsonTextReader, "Bad JSON escape sequence: {0}.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) ("\\" + ch2.ToString())));
            }
            jsonTextReader.EnsureBufferNotEmpty();
      ...
      while (true)
      {
        do
        {
          do
          {
            ...
            while (await jsonTextReader.ReadDataAsync(true, cancellationToken).ConfigureAwait(false) != 0);
...
label_16:
        ...
        while (!await jsonTextReader.EnsureCharsAsync(0, true, cancellationToken).ConfigureAwait(false) || jsonTextReader._chars[jsonTextReader._charPos] != '/');
        break;
label_19:
        if (!singlelineComment)
        {
...
label_6:;
    ...
    {
      JsonTextReader jsonTextReader = this;
      ConfiguredTaskAwaitable configuredTaskAwaitable = await jsonTextReader.MatchValueWithTrailingSeparatorAsync("new", cancellationToken).ConfigureAwait(false) ? jsonTextReader.EatWhitespaceAsync(cancellationToken).ConfigureAwait(false) : throw JsonReaderException.Create((JsonReader) jsonTextReader, "Unexpected content while parsing JSON.");
      await configuredTaskAwaitable;
      int initialPosition = jsonTextReader._charPos;
      char c;
      while (true)
...
label_11:
      switch (c)
      {
        ...
        case '\r':
          endPosition = jsonTextReader._charPos;
          configuredTaskAwaitable = jsonTextReader.ProcessCarriageReturnAsync(true, cancellationToken).ConfigureAwait(false);
          await configuredTaskAwaitable;
          break;
        default:
          if (char.IsWhiteSpace(c))
          ...
          if (c != '(')
            throw JsonReaderException.Create((JsonReader) jsonTextReader, "Unexpected character while parsing constructor: {0}.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) c));
          endPosition = jsonTextReader._charPos;
          break;
      }
label_20:
      ...
      configuredTaskAwaitable = jsonTextReader.EatWhitespaceAsync(cancellationToken).ConfigureAwait(false);
      await configuredTaskAwaitable;
      if (jsonTextReader._chars[jsonTextReader._charPos] != '(')
    ...
    {
      JsonTextReader jsonTextReader = this;
      char ch = jsonTextReader._chars[jsonTextReader._charPos];
      char quoteChar;
      switch (ch)
      {
        case '"':
        case '\'':
          ...
          break;
        default:
          if (!jsonTextReader.ValidIdentifierChar(ch))
      ...
      string propertyName = jsonTextReader.PropertyNameTable == null ? jsonTextReader._stringReference.ToString() : jsonTextReader.PropertyNameTable.Get(jsonTextReader._stringReference.Chars, jsonTextReader._stringReference.StartIndex, jsonTextReader._stringReference.Length) ?? jsonTextReader._stringReference.ToString();
...
label_6:
          ...
          return (object) null;
label_7:
          await jsonTextReader.ParseStringAsync(ch, readType, cancellationToken).ConfigureAwait(false);
...
label_10:
          await jsonTextReader.ParseNumberAsync(ReadType.Read, cancellationToken).ConfigureAwait(false);
          bool flag;
          if (jsonTextReader.Value is BigInteger bigInteger2)
          {
            i = bigInteger2;
            flag = i != 0L;
          }
          else
            flag = Convert.ToBoolean(jsonTextReader.Value, (IFormatProvider) CultureInfo.InvariantCulture);
...
label_5:
          ...
          return (byte[]) null;
label_6:
          ConfiguredTaskAwaitable configuredTaskAwaitable = jsonTextReader.ParseStringAsync(ch, ReadType.ReadAsBytes, cancellationToken).ConfigureAwait(false);
          await configuredTaskAwaitable;
...
label_27:
    ...
    /// <summary>Gets or sets the reader's property name table.</summary>
    ...
    private void ParseReadNumber(ReadType readType, char firstChar, int initialPosition)
    {
      ...
      switch (readType)
      {
        ...
        case ReadType.ReadAsInt32:
          ...
          {
            int num2;
            switch (ConvertUtils.Int32TryParse(this._stringReference.Chars, this._stringReference.StartIndex, this._stringReference.Length, out num2))
            {
              case ParseResult.Success:
                obj = (object) num2;
                break;
