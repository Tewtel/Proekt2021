...
namespace System.Data.Entity.SqlServer
{
  internal sealed class SqlSpatialDataReader : DbSpatialDataReader
  {
    ...
    public override bool IsGeographyColumn(int ordinal) => this._geographyColumns[ordinal];

    public override bool IsGeometryColumn(int ordinal) => this._geometryColumns[ordinal];

    private void EnsureGeographyColumn(int ordinal)
    {
      if (!this.IsGeographyColumn(ordinal))
        throw new InvalidDataException(System.Data.Entity.SqlServer.Resources.Strings.SqlProvider_InvalidGeographyColumn((object) this._reader.GetDataTypeName(ordinal)));
    }

    private void EnsureGeometryColumn(int ordinal)
    {
      if (!this.IsGeometryColumn(ordinal))
        throw new InvalidDataException(System.Data.Entity.SqlServer.Resources.Strings.SqlProvider_InvalidGeometryColumn((object) this._reader.GetDataTypeName(ordinal)));
    }
