...
namespace System.Data.Entity.SqlServer.SqlGen
{
  internal static class DmlSqlGenerator
  {
      ...
      bool upperCaseKeywords = true)
    {
      ...
      DmlSqlGenerator.ExpressionTranslator translator = new DmlSqlGenerator.ExpressionTranslator(commandText, (DbModificationCommandTree) tree, tree.Returning != null, sqlGenerator);
    ...
    {
      SqlStringBuilder commandText = new SqlStringBuilder(256);
      commandText.UpperCaseKeywords = upperCaseKeywords;
      DmlSqlGenerator.ExpressionTranslator expressionTranslator = new DmlSqlGenerator.ExpressionTranslator(commandText, (DbModificationCommandTree) tree, false, sqlGenerator, createParameters: createParameters);
      ...
      bool createParameters = true)
    {
      ...
      DmlSqlGenerator.ExpressionTranslator translator = new DmlSqlGenerator.ExpressionTranslator(commandText, (DbModificationCommandTree) tree, tree.Returning != null, sqlGenerator, createParameters: createParameters);
    ...
    internal static string GenerateMemberTSql(EdmMember member) => SqlGenerator.QuoteIdentifier(member.Name);
      ...
      bool useGeneratedValuesVariable)
    {
      ...
      else
      {
        ...
        foreach (EdmMember keyMember in target.ElementType.KeyMembers)
        {
          ...
          {
            if (flag)
              throw new NotSupportedException(System.Data.Entity.SqlServer.Resources.Strings.Update_NotSupportedServerGenKey((object) target.Name));
            if (!DmlSqlGenerator.IsValidScopeIdentityColumnType(keyMember.TypeUsage))
              throw new InvalidOperationException(System.Data.Entity.SqlServer.Resources.Strings.Update_NotSupportedIdentityType((object) keyMember.Name, (object) keyMember.TypeUsage.ToString()));
    ...
    {
      if (!SqlProviderServices.UseScopeIdentity || typeUsage.EdmType.BuiltInTypeKind != BuiltInTypeKind.PrimitiveType)
        return false;
      string name = typeUsage.EdmType.Name;
      if (name == "tinyint" || name == "smallint" || (name == "int" || name == "bigint"))
        return true;
      Facet facet;
      return (name == "decimal" || name == "numeric") && typeUsage.Facets.TryGetValue("Scale", false, out facet) && Convert.ToInt32(facet.Value, (IFormatProvider) CultureInfo.InvariantCulture) == 0;
    }
    ...
    internal class ExpressionTranslator : BasicExpressionVisitor
    {
      ...
      internal List<SqlParameter> Parameters => this._parameters;

      internal Dictionary<EdmMember, SqlParameter> MemberValues => this._memberValues;
      ...
      internal static string GetParameterName(int index) => "@" + index.ToString((IFormatProvider) CultureInfo.InvariantCulture);
      ...
      public override void Visit(DbScanExpression expression)
      {
        ...
        if (expression.Target.GetMetadataPropertyValue<string>("DefiningQuery") != null)
        {
          ...
          throw new UpdateException(System.Data.Entity.SqlServer.Resources.Strings.Update_SqlEntitySetWithoutDmlFunctions((object) expression.Target.Name, (object) str, (object) "ModificationFunctionMapping"));
        }
