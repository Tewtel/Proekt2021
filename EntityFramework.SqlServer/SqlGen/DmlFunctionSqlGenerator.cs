...
namespace System.Data.Entity.SqlServer.SqlGen
{
  internal class DmlFunctionSqlGenerator
  {
    ...
    public DmlFunctionSqlGenerator(SqlGenerator sqlGenerator) => this._sqlGenerator = sqlGenerator;
    ...
    public string GenerateInsert(ICollection<DbInsertCommandTree> commandTrees)
    {
      ...
      stringBuilder.Append(DmlSqlGenerator.GenerateInsertSql(insertCommandTree1, this._sqlGenerator, out parameters, false, createParameters: false));
      ...
      {
        stringBuilder.Append(DmlSqlGenerator.GenerateInsertSql(tree, this._sqlGenerator, out parameters, false, createParameters: false));
        stringBuilder.AppendLine();
      }
      ...
      DbInsertCommandTree commandTree)
    {
      ...
      if (list.Any<EdmProperty>())
      {
        ...
        DmlSqlGenerator.ExpressionTranslator translator = new DmlSqlGenerator.ExpressionTranslator(commandText, (DbModificationCommandTree) commandTree, true, this._sqlGenerator, (ICollection<EdmProperty>) entityType.KeyProperties);
      ...
      string rowsAffectedParameter)
    {
      ...
      List<SqlParameter> parameters;
      sql.AppendLine(DmlSqlGenerator.GenerateUpdateSql(commandTrees.First<DbUpdateCommandTree>(), this._sqlGenerator, out parameters, false));
      foreach (DbUpdateCommandTree tree in commandTrees.Skip<DbUpdateCommandTree>(1))
      {
        sql.Append(DmlSqlGenerator.GenerateUpdateSql(tree, this._sqlGenerator, out parameters, false));
      ...
      string rowsAffectedParameter)
    {
      StringBuilder sql = new StringBuilder();
      List<SqlParameter> parameters;
      sql.AppendLine(DmlSqlGenerator.GenerateDeleteSql(commandTrees.First<DbDeleteCommandTree>(), this._sqlGenerator, out parameters));
      sql.AppendLine();
      foreach (DbDeleteCommandTree tree in commandTrees.Skip<DbDeleteCommandTree>(1))
      {
        sql.AppendLine(DmlSqlGenerator.GenerateDeleteSql(tree, this._sqlGenerator, out parameters));
        sql.AppendLine("AND @@ROWCOUNT > 0");
        sql.AppendLine();
      }
      DmlFunctionSqlGenerator.AppendSetRowsAffected(sql, rowsAffectedParameter);
    ...
    private sealed class ReturningSelectSqlGenerator : BasicExpressionVisitor
    {
      ...
      {
        string str = SqlGenerator.GetTargetTSql(scanExpression.Target) + " AS " + (this._currentTableAlias = "t" + this._aliasCount++.ToString());
      ...
      public override void Visit(DbParameterReferenceExpression expression) => this._where.Append("@" + expression.ParameterName);
